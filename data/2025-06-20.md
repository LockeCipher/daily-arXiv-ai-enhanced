...
TL;DR: 提出了一种基于扩散模型的一次性人脸素描合成方法，解决了传统方法在数据稀缺时的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 传统人脸素描合成方法依赖大量数据，面临数据稀缺和高成本问题，需一种更高效的方法。

Method: 利用扩散模型优化文本指令，通过梯度优化生成素描，并引入新数据集OS-Sketch进行评测。

Result: 实验表明，该方法在一次性场景下能生成高质量素描，且比其他方法更便捷、适用性更广。

Conclusion: 该方法在数据稀缺时表现优异，具有实际应用潜力，数据集将公开。
...
TL;DR: 本文提出了一种基于神经不确定性地图的新型主动视角选择方法（UPNet），用于高效且准确的3D物体重建，显著减少了计算开销并提升了速度。
...
Method: 使用轻量级前馈深度神经网络UPNet预测不确定性地图，通过启发式方法选择最具信息量的视角，并训练3D神经渲染模型。

Result: 使用一半的视角数量即可达到与上限相当的精度，计算速度提升400倍，资源消耗减少50%以上。

Conclusion: UPNet方法在减少计算资源的同时保持了高精度，且无需额外训练即可泛化到新物体类别。
...
Method: 提出DPO-Kernels方法，包括混合损失、核化表示和发散选择；引入DETONATE基准和AQI指标。

Result: DPO-Kernels通过HT-SR保持强泛化性能，DETONATE基准包含10万对图像。
...
TL;DR: 本文提出了一种针对医学图像扩散模型的频率校准重建误差（FCRE）方法，以改进成员推断攻击（MIA）的性能。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在医学图像生成中的应用引发了隐私担忧，现有MIA方法因图像固有难度和扩散模型在高频细节重建上的不足而效果有限。

Method: 通过分析反向扩散过程，计算中频范围的重建误差，并结合结构相似性指数评分，排除高频和低频区域的干扰。
...
<details>
  <summary>Details</summary>
Motivation: 传统风格迁移方法存在多种问题，且不同任务框架不统一。StyleWallfacer旨在通过统一框架实现高效、无漂移的风格注入和文本控制。

Method: 1. 基于BLIP和CLIP空间的语义风格注入方法；2. 基于人类反馈的数据增强策略；3. 训练自由的三重扩散过程，通过自注意力层特征操作实现风格注入。

Result: 实现了高质量的图像驱动风格迁移和文本驱动风格化，首次在风格迁移过程中实现图像颜色编辑。

Conclusion: StyleWallfacer框架在风格迁移领域取得了突破性成果，支持多种任务并保持内容一致性。
...
TL;DR: Echo-DND是一种新型双噪声扩散模型，用于超声心动图中左心室分割，结合高斯和伯努利噪声，并通过多尺度融合和空间一致性校准提升精度，性能优于现有SOTA模型。
...
TL;DR: 提出了一种基于扩散模型的两阶段融合网络（DM-FNet），用于多模态医学图像融合，通过增强特征识别能力和自适应处理不同模态图像，显著提升了融合质量。
...
Method: 采用两阶段扩散模型：第一阶段训练UNet进行图像重建，第二阶段将噪声图像输入融合网络，结合三种关键融合模块自适应处理不同模态图像。
...
TL;DR: 视频-SALMONN 2是一种先进的视听大语言模型，通过低秩适应（LoRA）和定向偏好优化（DPO）提升视频（带音频）字幕生成。提出多轮DPO（MrDPO）方法，显著降低字幕错误率28%，并在7B参数规模下超越GPT-4o和Gemini-1.5-Pro。
...
Method: 提出视频-SALMONN 2模型，结合LoRA和DPO优化；引入多轮DPO（MrDPO）方法，周期性更新参考模型并融合LoRA模块，结合真实字幕指导。

Result: MrDPO显著提升字幕准确性，错误率降低28%；7B参数模型在视频字幕任务中超越GPT-4o和Gemini-1.5-Pro。

Conclusion: 视频-SALMONN 2在视频字幕任务中表现优异，参数效率高，代码已开源。
...
TL;DR: RA-NeRF是一种新方法，能够在复杂相机轨迹下预测高精度相机位姿，结合NeRF和流驱动的位姿调节，显著提升了3D重建和SLAM任务的性能。
...
Conclusion: RA-NeRF在复杂相机轨迹下表现出高效和鲁棒性，为3D重建和SLAM任务提供了新解决方案。
...
Result: 在HEVC ClassB和UVG数据集上，MSNeRV在INR方法中表现最优，压缩效率在动态场景中超过VTM-23.7。
...
<details>
  <summary>Details</summary>
Motivation: 开源预训练模型的应用潜力巨大，但若训练数据不可用，其效用会下降。现有数据无关图像合成方法因缺乏自然图像的先验知识，生成的样本偏离训练数据分布。

Method: DDIS利用文本到图像扩散模型作为图像先验，通过领域对齐指导（DAG）和优化类别对齐标记（CAT）嵌入，生成与训练数据分布一致的图像。

Result: 在PACS和ImageNet上的实验表明，DDIS优于现有方法，生成的样本更贴近训练数据分布，在数据无关应用中达到SOTA性能。

Conclusion: DDIS通过扩散模型和领域对齐指导，显著提升了数据无关图像合成的质量，为预训练模型的应用提供了更可靠的解决方案。
...
TL;DR: 本教程以Hunyuan3D 2.1为例，提供了处理3D数据、训练3D生成模型及评估性能的完整指南，旨在降低3D AIGC领域的门槛。


<details>
  <summary>Details</summary>
Motivation: 3D AIGC领域因数据收集、处理和模型训练的复杂性，主要限于研究人员和开发者。本教程旨在通过Hunyuan3D 2.1系统解决这些挑战。

Method: 教程详细介绍了Hunyuan3D 2.1系统的两个核心组件：Hunyuan3D-DiT（形状生成）和Hunyuan3D-Paint（纹理合成），涵盖数据准备、模型架构、训练策略、评估指标和部署。

Result: 通过教程，用户可以掌握开发或微调适用于游戏、虚拟现实和工业设计的3D生成模型的能力。

Conclusion: 本教程为3D AIGC领域的实践者提供了全面的指导，有助于推动3D生成技术的广泛应用。
...
Method: 1) 使用Object-AnchorNet从3D HOI数据集中重建稀疏3D HOI关键帧；2) 提出ContactDM，通过接触感知编码器和注意力机制插值生成4D HOI序列。

Result: 在OMOMO和3D-FUTURE数据集上取得最优结果，展示了强大的泛化能力和高保真生成效果。

Conclusion: GenHOI通过两阶段方法有效解决了4D HOI合成的挑战，减少了数据依赖并提升了生成质量。
...
TL;DR: WinWinLay提出了一种无需训练的布局到图像生成方法，通过非局部注意力能量函数和自适应更新策略提升控制精度和真实性。
...
Method: 采用非局部注意力能量函数优化空间分布偏差，并引入基于Langevin动力学的自适应更新规则避免域外伪影。
...
TL;DR: Show-o2是一种改进的多模态模型，结合自回归建模和流匹配，支持跨图像和视频的多模态理解与生成。


<details>
  <summary>Details</summary>
Motivation: 构建统一的多模态表示，支持跨模态（文本、图像、视频）的理解与生成任务。

Method: 基于3D因果变分自编码器空间，通过双路径空间（时间）融合构建统一表示，结合自回归建模和流匹配。

Result: Show-o2模型在多模态任务中表现优异，支持文本、图像和视频的生成与理解。
...
TL;DR: 论文提出了一种名为DLoRAL的双LoRA学习范式，用于解决视频超分辨率中保持时空一致性的问题，通过结合Cross-Frame Retrieval模块和两种LoRA模块（C-LoRA和D-LoRA），实现了高效且高质量的视频恢复。


<details>
  <summary>Details</summary>
Motivation: 现有基于Stable Diffusion的Real-VSR方法在保持时空一致性时往往牺牲空间细节，导致视觉质量不佳。论文旨在解决这一问题，通过提取降噪鲁棒的时空一致性先验并增强视频细节。

Method: 提出DLoRAL范式，包括Cross-Frame Retrieval模块和两种LoRA模块（C-LoRA用于学习时空一致性，D-LoRA用于增强细节）。两阶段交替优化，最终合并LoRA分支以实现高效推理。

Result: 实验表明DLoRAL在精度和速度上均表现优异。

Conclusion: DLoRAL通过双LoRA学习范式，成功实现了视频超分辨率中时空一致性与细节增强的平衡，为Real-VSR任务提供了高效解决方案。
...
TL;DR: HOIDiNi是一个基于文本驱动的扩散框架，用于生成真实且合理的人-物交互（HOI）。通过优化预训练扩散模型的噪声空间，同时实现真实性和物理正确性。


<details>
  <summary>Details</summary>
Motivation: 当前方法在真实性和物理正确性之间存在权衡，HOIDiNi旨在通过两阶段方法解决这一问题。

Method: 采用扩散噪声优化（DNO）技术，分为物体中心阶段（选择手-物接触位置）和人体中心阶段（优化全身动作）。
...
TL;DR: 研究发现，多模态大语言模型（MLLMs）在视觉质量与性能之间存在矛盾：图像偏离人类感知的保真度时，模型表现反而更好。作者提出了一种轻量级适配模块VQ-TTT，动态调整输入图像以提升性能。


<details>
  <summary>Details</summary>
Motivation: 探索输入视觉质量如何影响MLLMs的表现，并解决现有视觉质量与模型需求之间的矛盾。

Method: 通过控制图像退化与风格变化，研究MLLMs的表现；提出VQ-TTT模块，动态调整输入图像频率内容并微调浅层视觉编码器。

Result: VQ-TTT显著提升了MLLMs在所有数据集上的平均准确率，无需外部模型或额外数据。

Conclusion: 研究重新定义了MLLMs对视觉输入的需求，强调适应性调整比普遍“干净”图像更重要。
...
Method: 联合估计反照率并合成重光照输出的单阶段方法，结合视频扩散模型的生成能力。

Result: 模型在合成和真实视频数据上表现出色，视觉保真度和时间一致性优于以往方法。

Conclusion: 该方法通过联合优化提升了场景理解和光照效果生成能力，具有广泛适用性。
...
TL;DR: Sekai是一个高质量的第一人称视角全球视频数据集，专为世界探索设计，包含丰富注释和多样场景。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成数据集不适合世界探索训练，存在地点有限、时长短、场景静态和缺乏注释等问题。

Method: 开发高效工具箱收集、预处理和注释视频，包含位置、场景、天气等信息，并训练交互式视频探索模型YUME。

Result: Sekai数据集包含5000多小时视频，覆盖100多个国家和750个城市，实验验证其质量。
...
TL;DR: ECAD是一种基于遗传算法的缓存调度方法，用于加速扩散模型的推理，无需修改网络参数或参考图像，显著提升速度并支持质量-延迟权衡控制。
...
Method: 提出ECAD，通过遗传算法学习高效的缓存调度，形成帕累托前沿，仅需少量校准提示。

Result: 在PixArt-alpha等模型上，ECAD优于现有方法（如COCO FID提升4.47），推理速度提升至2.58倍。

Conclusion: ECAD是一种可扩展、泛化性强的扩散模型加速方法，适用于不同模型和分辨率。
...
TL;DR: 论文提出了一种针对医疗内窥镜中模拟图像传感器的综合噪声模型，并开发了一种混合去噪系统，结合传统算法与学习技术，显著提升了图像质量。


<details>
  <summary>Details</summary>
Motivation: 微型内窥镜传感器因尺寸限制导致动态范围不足和噪声问题，影响图像质量，需开发高效去噪方法。

Method: 提出综合噪声模型，分析三类主要噪声，并设计混合去噪系统，结合传统算法与学习技术。

Result: 实验显示系统有效降噪，PSNR从21.16提升至33.05，且在FPGA平台上实现实时性能。

Conclusion: 该方法显著改善图像质量，适用于微型内窥镜传感器的噪声问题，具有实际应用价值。
...
TL;DR: 本文提出了一种针对360°视频的超分辨率模型S3PO，解决了传统VSR技术在360°视频中的失真问题，并创建了新的数据集360VDS。


<details>
  <summary>Details</summary>
Motivation: 360°视频在VR中广泛应用，但空间分辨率不足影响视觉质量，且传统VSR技术无法解决其投影失真问题。

Method: 提出S3PO模型，采用循环建模和注意力机制，结合专门的特征提取器和损失函数处理球形失真。

Result: S3PO在360°视频数据集上优于现有VSR和360°专用超分辨率模型。

Conclusion: S3PO通过针对性设计和优化，显著提升了360°视频的超分辨率效果。
