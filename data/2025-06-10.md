...
Motivation: 从视频中忠实重建纹理形状和物理属性是一个具有挑战性的问题，传统方法依赖高成本优化和超参数调整，限制了实用性和泛化性。

Method: Vid2Sim结合前馈神经网络和轻量级优化管道，先重建物理系统配置，再优化外观、几何和物理属性，支持高效无网格模拟。

Result: 实验表明，Vid2Sim在从视频数据中重建几何和物理属性时具有更高的准确性和效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（如NeRF和3DGS）在训练视角不足时，对未见或遮挡部分的渲染质量较低。环境中常存在重复元素，可利用这些重复性提升重建质量。

Method: 提出一种方法，对3DGS重建中的重复实例进行分割、配准，并允许实例间信息共享，同时考虑外观变化。

Result: 在合成和真实场景中验证了方法，显著提升了新视角合成的质量。

Conclusion: 通过利用重复元素，有效改善了场景重建和新视角合成的效果。
...
Method: 提出两种辅助一致性损失：1) 先验一致性正则化损失，确保非主体图像的扩散噪声与预训练模型一致；2) 主体一致性正则化损失，增强模型对噪声调制潜码的鲁棒性。

Result: 实验表明，结合这两种损失的微调方法在CLIP分数、背景多样性和视觉质量上优于DreamBooth。

Conclusion: 该方法有效解决了微调中的欠拟合和过拟合问题，同时提升了生成图像的多样性和主体保真度。
...
<details>
  <summary>Details</summary>
Motivation: 并行模拟中，收敛性和并行性通常被视为冲突目标。本文旨在解决这一问题，提出一种既能保持高并行性又能实现快速收敛的方法。

Method: 通过分析过冲现象，推导出理论上的二阶最优解，并将其转化为可预计算的形式。结合Cubature采样和全坐标公式化，实现了高效的预计算和运行时性能。

Result: 实验结果表明，该方法在收敛速度上优于现有GPU方法50至100倍，同时支持刚性和软材料的二阶收敛。

Conclusion: 该方法成功解决了并行模拟中收敛性与并行性的矛盾，为高质量模拟提供了高效解决方案。
...
Method: 使用自动编码器网络架构，将输入点云表面编码为稀疏体素网格，解码为基于SDF的几何和交叉场。

Result: 方法能在不到一秒内生成高质量交叉场，无需逐形状优化，并支持从草图等部分输入生成新形状的交叉场。

Conclusion: CrossGen在四边形网格生成任务中表现出高效性和高质量，适用于多种表面形状。
...
<details>
  <summary>Details</summary>
Motivation: 尽管3DGS在视图合成中表现出色，但在资源受限设备上实现实时渲染仍面临挑战。

Method: 采用轴定向光栅化减少计算冗余，引入神经排序替代硬件排序器，设计可重构处理阵列支持光栅化和神经网络推理。

Result: 实验显示，设计在保持渲染质量的同时，速度提升23.4~27.8倍，能耗降低28.8~51.4倍。

Conclusion: 该设计为资源受限设备上的高效3D渲染提供了可行方案，并计划开源以推动领域发展。
...
Method: 引入部分功能图（PAGs），从大语言模型中提取结构化HOI表示，指导三阶段合成：分解3D物体为几何部分，生成参考HOI视频并提取部分运动约束，优化4D HOI运动序列。

Result: 实验表明，该方法能灵活生成复杂多对象或多人的交互序列，显著提升零样本4D HOI生成的现实感和文本对齐。
...
<details>
  <summary>Details</summary>
Motivation: 为平坦表面的可视化提供一种直观且计算高效的方法。

Method: 采用光线步进技术，结合具体实现细节。

Result: 通过多个示例展示了方法的实用性，并提供了代码实现。

Conclusion: 该方法不仅高效，还可用于科普推广，相关代码已开源。
...
Method: 结合3D对象分割与高精度交互模拟，包括2D像素到3D高斯的快速映射、多材料物理属性分配和变形梯度约束。

Result: 实验显示PIG在视觉质量上优于现有技术，并推动了物理真实场景生成的新方向。

Conclusion: PIG通过精确分割和物理模拟，显著提升了3D高斯场景的交互质量和视觉效果。
...
<details>
  <summary>Details</summary>
Motivation: 动态3D高斯泼溅（3DGS）的神经推断效率低，限制了渲染速度和计算资源利用。

Method: 提出时间敏感剪枝和GroupFlow技术，减少高斯数量并聚类运动轨迹。

Result: 在多个数据集上实现10.37倍渲染加速、7.71倍模型压缩和2.71倍训练加速。

Conclusion: SpeeDe3DGS模块化设计可集成到任何动态3DGS框架，显著提升效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D数据压缩方法难以同时实现高压缩比和高质量重建，Squeeze3D旨在解决这一问题。

Method: 通过可训练的映射网络连接预训练编码器和生成模型的潜在空间，将3D数据压缩为紧凑潜在代码，再通过生成模型重建。

Result: 实验显示，Squeeze3D在纹理网格、点云和辐射场上的压缩比分别达到2187x、55x和619x，且视觉质量与现有方法相当。

Conclusion: Squeeze3D无需特定对象训练即可实现高效压缩和解压，支持多种3D数据格式，具有灵活性和高效性。
...
Method: 系统预训练2100万张面部图像，微调为LiveCAD模型，使用7004张来自1751名受试者的面部图像进行CAD风险评估。

Result: DigitalShadow能被动、无接触地生成个性化风险报告和健康建议。

Conclusion: 该系统以隐私为核心设计原则，支持本地部署，确保数据安全。
...
<details>
  <summary>Details</summary>
Motivation: 随着视觉变换器（ViTs）在计算机视觉任务中的成功应用，研究其在医学图像中的对抗性脆弱性具有重要意义。

Method: 通过投影梯度下降（PGD）生成对抗性水印，测试ViTs的脆弱性，并分析对抗训练的效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有健身追踪模型要么支持的运动种类有限，要么过于复杂难以部署，缺乏通用性。

Method: 开发了一个多任务运动分析模型，支持数百种运动的检测和重复计数，并构建了大规模数据集Olympia。
...
<details>
  <summary>Details</summary>
Motivation: 传统SLAM算法生成的地图分辨率低且不完整，而现有GS方法依赖场景优化，耗时且泛化能力差。

Method: 使用RGB-D图像识别主干预测高斯参数，集成3D语义分割，并通过全局定位后仅优化1次高斯泼溅来修正漂移和浮点。

Result: 在ScanNet上实现最先进的语义SLAM性能，高斯数量比其他GS方法少一个数量级，并在NYUv2和TUM RGB-D数据集上展示了零样本泛化能力。

Conclusion: 该方法高效、通用，且能生成高质量的3D语义地图。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法依赖大规模像素级标注，成本高且耗时，需要一种无需特定标注的解决方案。

Method: 整合音频、视觉和文本表征，通过多模态预训练模型连接策略实现分割。

Result: 在多个数据集上实现了最先进的零样本性能。
...
<details>
  <summary>Details</summary>
Motivation: 由于DNNs在交通标志识别中广泛应用且训练数据来源未知，确保模型安全免受攻击或中毒至关重要。

Method: 通过添加微小扰动进行误差最小化攻击，提出基于非线性变换的数据增强训练方法以提高模型鲁棒性。

Result: 攻击使DNNs预测准确率从99.90%降至10.6%，但提出的方法将其恢复至96.05%，检测模型识别攻击成功率超99%。
...
<details>
  <summary>Details</summary>
Motivation: 纺织品回收中，准确识别材料成分和检测污染物是提高效率和规模化的关键挑战。

Method: 使用预训练架构进行纺织品分类（EfficientNetB0表现最佳），结合Grounding DINO和Segment Anything Model进行零样本污染物分割。

Result: 分类准确率达81.25%，分割任务mIoU为0.90。

Conclusion: RGB图像结合现代深度学习技术可用于纺织品回收的关键预处理步骤。
...
<details>
  <summary>Details</summary>
Motivation: 现有监控系统因图像质量低和数据集偏见导致面部识别性能下降，尤其在肤色变化和部分遮挡情况下表现不佳。

Method: 利用深度学习方法（如自动编码器和生成对抗网络）生成多样化高质量面部数据，并集成图像增强模块。

Result: 在CelebA数据集上验证，平台提升了数据多样性和模型公平性。

Conclusion: 该工作减少了AI面部分析的偏见，提高了监控系统在复杂环境中的准确性和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 事件相机在运动任务中表现优异，但获取真实标注数据昂贵且困难，因此需要自监督方法。
...
Result: RARL在医学任务中表现优于监督微调（提升约7.78%），并在未见数据集上泛化能力更强（提升约27%）。

Conclusion: 推理引导学习和推理提示有助于提升医学视觉语言模型的透明性、准确性和资源效率。
...
Result: BLIP-2方法显著提升Recall@10至45.6%（衬衫）、40.1%（裙子）和50.4%（T恤），平均Recall@50达67.6%；Retrieval-DPO方法表现极差（0.02%）。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集在标注粒度、领域覆盖和逻辑规则上存在不足，限制了视觉解析在教育场景中的应用。

Method: 构建PhysLab数据集，包含620个长视频和多级标注，涵盖四种代表性物理实验。

Result: 数据集支持动作识别、物体检测和HOI分析等任务，并建立了强基线模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度从散焦（DFD）技术通常依赖多张图像，而单张散焦图像的深度估计问题因约束不足而研究较少。本文旨在通过暗通道先验和局部模糊与对比度变化的关系，提升单张图像DFD的性能。

Method: 利用暗通道捕捉模糊图像的局部统计和场景结构，结合局部散焦模糊与对比度变化的关系，通过对抗训练实现端到端深度估计。

Result: 在真实数据上的实验表明，暗通道先验的引入显著提升了单张图像DFD的深度估计效果。

Conclusion: 暗通道先验能有效提升单张散焦图像的深度估计性能，验证了方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D高斯散射方法在单目输入下优化时间长且泛化能力不足，限制了其在虚拟/增强现实等领域的应用。

Method: PGHM引入UV对齐的潜在身份图和分离的多头U-Net，分别编码主体特定特征并预测高斯属性，分解静态、姿态依赖和视角依赖组件。

Result: PGHM仅需约20分钟即可生成视觉质量相当的化身，显著优于从头优化的方法。

Conclusion: PGHM通过结合人体先验和高效设计，为单目化身创建提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型在洪水损害分类中表现不佳，因破坏特征不明显。Flood-DamageSense旨在填补这一空白。

Method: 结合SAR/InSAR、光学底图和洪水风险层，采用多模态Mamba架构，联合预测损害等级、洪水范围和建筑轮廓。

Result: 在Hurricane Harvey数据上，F1分数比现有方法提升19个百分点，尤其在轻微和中度损害类别中表现突出。
...
<details>
  <summary>Details</summary>
Motivation: 不孕症对生活质量影响显著，传统胚胎分级方法效率低下且耗时，需要更高效的技术支持。

Method: 采用CNN-LSTM混合架构，结合深度学习技术，通过XAI实现高精度且可解释的胚胎分类。

Result: 模型在胚胎分类中表现出高准确性，同时保持了可解释性。

Conclusion: 该框架为IVF中的胚胎选择提供了高效且透明的解决方案，有望提升成功率。
...
<details>
  <summary>Details</summary>
Motivation: 全向图像和视频超分辨率在虚拟现实和增强现实中至关重要，但现有数据集多为合成退化，缺乏真实世界失真。

Method: 通过引入新数据集360Insta，并结合公开数据集，对现有方法进行定性和定量评估。

Result: 360Insta填补了现有全向基准的空白，提升了超分辨率方法的泛化能力评估。
...
<details>
  <summary>Details</summary>
Motivation: 传统抛物线平均曲率流驱动的活动轮廓模型（PMCF-ACM）对初始曲线配置依赖性强，限制了其适应性。

Method: 提出HMCF-ACM和HDRF-ACM，利用可调初始速度场和边缘感知力调制，并通过加权四阶Runge-Kutta算法求解波方程。

Result: 实验表明，新方法在分割精度、噪声抵抗和数值稳定性上优于传统方法。

Conclusion: HMCF-ACM和HDRF-ACM通过任务自适应配置，显著提升了图像分割的性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决人类与野生动物冲突需要准确识别潜在威胁的动物，但现有分类模型在未知类别上表现不佳。

Method: 采用参数化的最近类均值（NCM）和非参数化的对比学习方法，利用预训练特征进行比较。

Result: NCM方法在AUPR-IN、AUPR-OUT和AUTC指标上分别提升2%、4%和22%。
...
<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在视觉和语言整合方面表现优异，但仍存在幻觉问题，即输出与图像内容不符。

Method: LPS利用局部视觉先验信息作为价值函数，在推理过程中修正解码过程。

Result: 在广泛使用的幻觉基准和噪声数据上，LPS显著减少了幻觉现象，尤其在噪声环境下表现突出。

Conclusion: LPS是一种简单、无需训练且高效的解码方法，能有效抑制多模态大语言模型的幻觉现象。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集缺乏食谱目标、分步指令和视觉内容之间的细粒度对齐，限制了食品计算领域的发展。

Method: 提出了RecipeGen数据集，包含26,453个食谱、196,724张图片和4,491个视频，并设计了领域特定的评估指标。

Result: RecipeGen覆盖了多样化的食材、烹饪过程、风格和菜品类型，并评估了代表性模型的表现。

Conclusion: RecipeGen为未来的食谱生成模型提供了基准和洞察，推动了食品计算领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法忽略结构建模在语义信息捕捉中的作用，本文旨在从SAR图像中推断目标组件及其结构关系。

Method: 基于结构描述符的两步算法框架：训练阶段从真实SAR图像检测2D关键点，学习其到3D层次结构的映射；测试阶段整合两步推断3D结构。

Result: 实验验证了各步骤的有效性，首次证明可从单视角SAR图像直接获取飞机目标的3D语义结构表示。

Conclusion: 该方法为SAR图像的高级信息检索提供了新思路，实现了从2D图像到3D语义结构的直接转换。
...
<details>
  <summary>Details</summary>
Motivation: 提供多样化的环境图像数据集，用于闭环算法的基准测试和深度学习训练。

Method: 使用高分辨率相机采集图像，每场景包含五张连续图像，并提供旋转和平移的真值数据。

Result: 数据集公开可用，适用于闭环算法的准确性和深度学习模型的训练。
...
<details>
  <summary>Details</summary>
Motivation: SO(3)旋转外推在计算机视觉和机器人学中至关重要，但面临噪声、稀疏观测、复杂动态和长期预测需求等挑战。

Method: 使用神经控制微分方程和Savitzky-Golay路径建模连续时间旋转动力学，学习潜在动态系统并保持旋转的几何结构。

Result: 在真实数据上的实验表明，相比现有方法，该方法具有更强的预测能力。

Conclusion: 该方法在复杂动态和长期预测任务中表现优越，为SO(3)旋转建模提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有风格迁移技术在保持身份特征方面表现不佳，尤其是在小面部或远距离面部图像中。

Method: 引入"马赛克恢复内容图像"技术和无需训练的内容一致性损失，增强身份保留和细节保护。

Result: 实验表明，该方法在保持高风格保真度和身份完整性方面显著优于基线模型。

Conclusion: 该方法无需重新训练或微调，即可在小面部或远距离面部条件下有效保持身份特征。
...
<details>
  <summary>Details</summary>
Motivation: 当前任务通用的提示分割方法在伪装物体分割中存在语义歧义和空间分离问题，导致分割效果不佳。

Method: 提出RDVP-MSD框架，结合区域约束的双流视觉提示（RDVP）和多模态逐步分解思维链（MSD-CoT），逐步消除语义歧义并优化视觉提示。

Result: 在多个伪装物体分割基准上取得最优分割结果，且推理速度更快。

Conclusion: RDVP-MSD无需训练即可显著提升分割准确性和效率，为伪装物体分割提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有3DGS模型依赖2D基础模型，导致视图不一致和开放词汇描述的不一致，阻碍了分层语义理解。

Method: 通过构建3D分层语义树和引入实例及部分对比损失，提升2D特征到3D特征的转换，实现视图一致的分层语义表示。

Result: 实验表明，Hi-LSplat在3D开放词汇分割和定位中表现优异，尤其在分层语义数据集上展现了强大的能力。

Conclusion: Hi-LSplat通过统一的3D分层语义表示，有效解决了视图不一致和开放词汇挑战，提升了3D场景的语义理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 研究VP在鲁棒源模型下的表现及其鲁棒性与泛化能力的权衡问题，并提出解决方案。

Method: 提出PBL策略，作为一种轻量级、即插即用的方法，与VP兼容。

Result: 实验表明PBL能有效继承鲁棒性并提升泛化能力。

Conclusion: PBL策略在多个数据集上验证了其普适性和有效性。
...
<details>
  <summary>Details</summary>
Motivation: 解决多图像生成中背景耦合与对象灵活性的矛盾，提升生成图像的一致性和多样性。

Method: 通过解耦模型交叉注意力模块中的背景和实体组件，并引入时间步相关的权重控制参数序列，优化这些参数以平衡背景耦合、文本对齐和视觉质量。

Result: 实验结果表明，该方法在背景耦合、文本对齐和视觉质量上优于现有方法。

Conclusion: 该方法有效解决了耦合图像生成中的关键问题，为多图像生成提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 解决内窥镜手术场景复杂性和传统深度学习模型在跨活动干扰中的性能不足问题。

Method: 结合低秩适应和任务共享适配器，引入空间感知多尺度注意力增强特征表示。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏视觉-时间推理能力，无法像人类专家一样识别上下文异常，因此探索VLM的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决3D高斯泼溅场景在风格化过程中难以匹配多风格、保持内存效率的问题。

Method: 采用二分匹配机制自动匹配风格图像与渲染图像局部区域，提出语义风格损失函数和局部-全局特征匹配技术。

Result: 实验表明，该方法在风格化效果和编辑灵活性上优于现有方法。

Conclusion: Multi-StyleGS在3D场景风格化中表现出色，兼具高效性和灵活性。
...
Conclusion: 神经网络可用于姿态估计，效果与现有技术相当，且更具成本效益。
...
<details>
  <summary>Details</summary>
Motivation: 卫星图像的语义分割对地球观测至关重要，但受限于标记数据不足。现有自监督方法（如MAE）专注于重建而非定位，而定位是分割任务的基础。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要预测其他代理的运动，现有编码器-解码器模型存在信息滞后问题，因此提出仅解码器模型以提升性能。

Method: 使用单一自回归模型编码历史轨迹并预测未来轨迹，引入‘过预测’策略以预测更长时域轨迹。

Result: DONUT在Argoverse 2单代理运动预测基准测试中优于基线模型，达到新最优结果。

Conclusion: 仅解码器模型在轨迹预测中表现优异，未来可进一步优化多代理交互场景。
...
Result: 在Reason-RFT-CoT Benchmark上性能提升达5%，优于现有最佳方法。
...
<details>
  <summary>Details</summary>
Motivation: ASD的早期诊断对干预至关重要，但传统方法依赖手工特征且缺乏透明性。

Method: 采用Vision Transformers和Vision Mamba的混合框架，结合注意力机制融合视觉、语音和面部线索，捕捉时空动态。

Result: 模型在Saliency4ASD数据集上表现优于现有方法，准确率0.96，F1分数0.95，灵敏度0.97，特异性0.94。
...
### [53] [NSD-Imagery: A benchmark dataset for extending fMRI vision decoding methods to mental imagery](https://arxiv.org/abs/2506.06898)
*Reese Kneeland,Paul S. Scotti,Ghislain St-Yves,Jesse Breedlove,Kendrick Kay,Thomas Naselaris*

Main category: cs.CV

TL;DR: NSD-Imagery是一个与人类fMRI活动配对的心理图像基准数据集，用于评估现有模型在心理图像重建上的表现，填补了现有NSD数据集的空白。
...
### [54] [KNN-Defense: Defense against 3D Adversarial Point Clouds using Nearest-Neighbor Search](https://arxiv.org/abs/2506.06906)
...
### [55] [Gaussian Mapping for Evolving Scenes](https://arxiv.org/abs/2506.06909)
*Vladimir Yugay,Thies Kersten,Luca Carlone,Theo Gevers,Martin R. Oswald,Lukas Schmid*

Main category: cs.CV

TL;DR: 论文提出了一种动态场景适应机制和关键帧管理机制，用于解决长期动态场景中的3D重建问题，并在合成和真实数据集上验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 当前基于3D高斯泼溅的系统在静态场景中表现优异，但在长期动态场景（如场景在视野外变化）中表现不足，需要一种机制来持续更新3D表示并保持几何和语义一致性。

Method: 引入动态场景适应机制以持续更新3D表示，并提出关键帧管理机制以丢弃过时观测同时保留尽可能多的信息。

Result: 在合成和真实数据集上，提出的GaME方法比现有技术更准确。

Conclusion: GaME方法有效解决了长期动态场景中的3D重建问题，提升了新视角合成的性能。
...
### [56] [Sleep Stage Classification using Multimodal Embedding Fusion from EOG and PSM](https://arxiv.org/abs/2506.06912)
*Olivier Papillon,Rafik Goubran,James Green,Julien Larivière-Chartier,Caitlin Higginson,Frank Knoefel,Rébecca Robillard*

Main category: cs.CV

TL;DR: 研究提出了一种基于ImageBind的多模态嵌入深度学习模型，结合PSM和双通道EOG信号进行睡眠阶段分类，显著提高了准确性。


<details>
  <summary>Details</summary>
Motivation: 传统PSG依赖EEG，复杂且难以在家中使用，因此探索PSM和EOG作为替代方案。

Method: 利用ImageBind模型整合PSM和双通道EOG数据，首次实现两者的融合分类。

Result: 经过微调的ImageBind模型优于现有单通道EOG、纯PSM及其他多模态方法，且在无微调时表现良好。

Conclusion: 预训练多模态嵌入模型可有效用于睡眠阶段分类，接近依赖复杂EEG数据的系统。
...
### [57] [Reading in the Dark with Foveated Event Vision](https://arxiv.org/abs/2506.06918)
...
### [58] [How Important are Videos for Training Video LLMs?](https://arxiv.org/abs/2506.06928)
...
Method: 使用LongVU算法训练LLMs，并通过图像序列和问题微调模型。

Result: 图像训练的LLMs在TVBench上表现显著高于随机水平，且接近或优于视频训练的LLMs。

Conclusion: 当前视频训练方法未充分利用时间特征，需进一步研究图像训练LLMs的时间推理机制和视频训练瓶颈。
...
### [59] [Polar Hierarchical Mamba: Towards Streaming LiDAR Object Detection with Point Clouds as Egocentric Sequences](https://arxiv.org/abs/2506.06944)
*Mellon M. Zhang,Glen Chou,Saibal Mukhopadhyay*

Main category: cs.CV

TL;DR: PHiM是一种基于状态空间模型的新型架构，专为极坐标流式LiDAR设计，通过局部双向Mamba块和全局前向Mamba块实现高效检测，性能优于现有流式检测器。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要低延迟、高吞吐的实时感知，但传统LiDAR处理方法存在延迟问题，而流式方法因几何不匹配导致性能下降。

Method: PHiM采用局部双向Mamba块进行扇区内空间编码，全局前向Mamba块进行扇区间时序建模，替代传统卷积和位置编码。

Result: 在Waymo Open Dataset上，PHiM性能超越现有流式检测器10%，且吞吐量翻倍。
...
### [60] [LaTtE-Flow: Layerwise Timestep-Expert Flow-based Transformer](https://arxiv.org/abs/2506.06952)
*Ying Shen,Zhiyang Xu,Jiuhai Chen,Shizhe Diao,Jiaxin Zhang,Yuguang Yao,Joy Rimchala,Ismini Lourentzou,Lifu Huang*

Main category: cs.CV

TL;DR: LaTtE-Flow是一种新型高效的多模态模型，统一了图像理解和生成，通过分层时间步专家架构显著提升了生成效率。
...
Method: 基于预训练视觉语言模型，引入分层时间步专家流架构和残差注意力机制，提升生成效率。

Result: 在理解任务上表现优异，生成质量与现有模型相当，推理速度快6倍。

Conclusion: LaTtE-Flow在性能和效率上均优于现有统一模型，具有实际应用潜力。
...
### [61] [Task-driven real-world super-resolution of document scans](https://arxiv.org/abs/2506.06953)
*Maciej Zyrek,Tomasz Tarasiewicz,Jakub Sadel,Aleksandra Krzywon,Michal Kawulok*

Main category: cs.CV

TL;DR: 论文提出了一种针对光学字符识别任务优化的多任务学习框架，通过结合高级视觉任务的辅助损失函数，动态调整权重，提升了真实场景中的超分辨率性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在模拟数据集上表现良好，但在真实场景（如文档扫描）中泛化能力不足，因此需要一种任务驱动的优化方法。

Method: 采用多任务学习框架，结合文本检测、识别、关键点定位和色调一致性等辅助损失函数，并使用动态权重平均机制平衡目标。基于SRResNet架构实现。

Result: 实验表明，该方法在模拟和真实文档数据集上提升了文本检测性能（IoU指标），同时保持了图像保真度。

Conclusion: 多目标优化有助于缩小模拟训练与真实部署之间的差距，提升超分辨率模型在实际场景中的应用效果。
...
### [62] [AR-RAG: Autoregressive Retrieval Augmentation for Image Generation](https://arxiv.org/abs/2506.06962)
*Jingyuan Qi,Zhiyang Xu,Qifan Wang,Lifu Huang*

Main category: cs.CV

TL;DR: AR-RAG是一种通过自回归方式在图像生成过程中动态检索并融合相关视觉参考的新方法，解决了现有方法中的静态检索问题。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成方法通常基于静态检索，容易导致过度复制或风格偏差，AR-RAG旨在通过动态检索解决这些问题。

Method: 提出了两种框架：DAiD（直接合并预测与检索的分布）和FAiD（通过多尺度卷积平滑检索特征并增强生成）。

Result: 在多个基准测试（如Midjourney-30K、GenEval和DPG-Bench）上表现优于现有方法。

Conclusion: AR-RAG通过动态检索显著提升了图像生成的质量和灵活性。
...
### [63] [Dual-view Spatio-Temporal Feature Fusion with CNN-Transformer Hybrid Network for Chinese Isolated Sign Language Recognition](https://arxiv.org/abs/2506.06966)
*Siyuan Jing,Guangxue Wang,Haoyang Zhai,Qin Tao,Jun Yang,Bing Wang,Peng Jin*

Main category: cs.CV

TL;DR: 本文提出了一个双视角中国手语数据集NationalCSL-DP，并设计了一个CNN-Transformer网络作为基线模型，通过融合策略提升了孤立手语识别（ISLR）的性能。


<details>
  <summary>Details</summary>
Motivation: 现有手语数据集覆盖不全且多为单视角，难以处理手部遮挡问题，因此需要构建更全面的数据集和改进识别方法。

Method: 提出双视角数据集NationalCSL-DP，包含134140个视频，并设计CNN-Transformer网络及融合策略。

Result: 实验证明数据集和基线模型有效，融合策略显著提升ISLR性能，但序列模型难以学习双视角互补特征。

Conclusion: 双视角数据集和融合策略为ISLR提供了新方向，但需进一步优化模型以更好地利用多视角信息。
...
### [64] [Guiding Cross-Modal Representations with MLLM Priors via Preference Alignment](https://arxiv.org/abs/2506.06970)
*Pengfei Zhao,Rongbo Luan,Wei Zhang,Peng Wu,Sifeng He*

Main category: cs.CV

TL;DR: MAPLE利用MLLM的细粒度对齐先验，通过强化学习框架和新的RPA损失函数，显著提升了跨模态检索的细粒度性能。


<details>
  <summary>Details</summary>
Motivation: CLIP在多模态检索中存在模态差距，而现有MLLM检索器依赖粗粒度对齐机制，限制了其潜力。

Method: 提出MAPLE框架，包括自动偏好数据构建和RPA损失函数，用于细粒度跨模态表示学习。

Result: 实验表明，MAPLE在细粒度跨模态检索中取得了显著提升。

Conclusion: MAPLE通过偏好引导对齐，有效处理了语义细微差别。
...
### [65] [Hybrid Mesh-Gaussian Representation for Efficient Indoor Scene Reconstruction](https://arxiv.org/abs/2506.06988)
...
<details>
  <summary>Details</summary>
Motivation: 解决3D高斯点云在复杂纹理区域渲染效率低下的问题。

Method: 通过修剪和优化网格处理平坦区域，结合3D高斯点云和网格的联合优化，使用预热策略和透射感知监督。

Result: 在保持渲染质量的同时，显著提升了帧率并减少了高斯基元数量。

Conclusion: 混合表示方法在室内场景中有效提升了渲染效率。
...
### [66] [Boosting Adversarial Transferability via Commonality-Oriented Gradient Optimization](https://arxiv.org/abs/2506.06992)
...
<details>
  <summary>Details</summary>
Motivation: 理解Vision Transformers（ViTs）的特性需要有效的对抗样本，但现有方法因过拟合导致迁移性不足。

Method: COGO包含两部分：共性增强（CE）扰动中低频区域，个性抑制（IS）通过自适应阈值调整梯度权重。
...
### [67] [DM$^3$Net: Dual-Camera Super-Resolution via Domain Modulation and Multi-scale Matching](https://arxiv.org/abs/2506.06993)
*Cong Guan,Jiacheng Ying,Yuya Ieiri,Osamu Yoshie*

Main category: cs.CV

TL;DR: DM$^3$Net是一种基于域调制和多尺度匹配的双摄像头超分辨率网络，通过压缩全局表示和多尺度匹配模块提升图像质量，同时引入关键修剪减少内存和推理时间。


<details>
  <summary>Details</summary>
Motivation: 智能手机摄影中，双摄像头超分辨率技术实用性强，但现有方法在域差距和高频细节传递上存在不足。

Method: 提出DM$^3$Net，通过域调制学习压缩全局表示，设计多尺度匹配模块提升匹配精度，并引入关键修剪优化效率。
...
### [68] [Technical Report for ICRA 2025 GOOSE 3D Semantic Segmentation Challenge: Adaptive Point Cloud Understanding for Heterogeneous Robotic Systems](https://arxiv.org/abs/2506.06995)
*Xiaoya Zhang*
...
<details>
  <summary>Details</summary>
Motivation: 解决多机器人平台采集的户外3D点云数据的语义分割问题，特别是在异构数据环境下的适应性挑战。

Method: 采用Point Prompt Tuning（PPT）与Point Transformer v3（PTv3）结合，通过平台特定条件和跨数据集类别对齐策略处理异构数据。
...
### [69] [BePo: Leveraging Birds Eye View and Sparse Points for Efficient and Accurate 3D Occupancy Prediction](https://arxiv.org/abs/2506.07002)
*Yunxiao Shi,Hong Cai,Jisoo Jeong,Yinhao Zhu,Shizhong Han,Amin Ansari,Fatih Porikli*

Main category: cs.CV

TL;DR: 提出了一种结合BEV和稀疏点的3D占用预测方法BePo，通过双分支设计提升小物体和平面物体的检测效果，并在实验中验证了其优越性和高效性。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景理解方法在计算成本或检测效果上存在不足，BEV对小物体信息损失严重，稀疏点对平面或大物体效率低。

Method: 采用双分支设计：基于查询的稀疏点分支和BEV分支，通过交叉注意力共享信息，最终融合输出3D占用预测。

Result: 在Occ3D-nuScenes和Occ3D-Waymo基准测试中表现优越，推理速度与最新高效方法相当。

Conclusion: BePo通过结合BEV和稀疏点的优势，有效解决了现有方法的局限性，实现了高效且准确的3D占用预测。
...
### [70] [UNO: Unified Self-Supervised Monocular Odometry for Platform-Agnostic Deployment](https://arxiv.org/abs/2506.07013)
*Wentao Zhao,Yihe Niu,Yanbo Wang,Tianchen Deng,Shenghai Yuan,Zhenli Wang,Rui Guo,Jingchuan Wang*

Main category: cs.CV

TL;DR: UNO是一个统一的单目视觉里程计框架，能够在多种环境、平台和运动模式下实现鲁棒且自适应的姿态估计。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖于特定部署的调优或预定义的运动先验，难以适应多样化场景。UNO旨在通过通用框架解决这一问题。

Method: 采用Mixture-of-Experts策略进行局部状态估计，结合Gumbel-Softmax模块选择最优解码器，并通过统一后端结合深度先验和轻量级捆绑调整。

Result: 在KITTI、EuRoC-MAV和TUM-RGBD数据集上实现了最先进的性能。

Conclusion: UNO框架在多样化场景中表现出色，具有广泛的应用潜力。
...
### [71] [TABLET: Table Structure Recognition using Encoder-only Transformers](https://arxiv.org/abs/2506.07015)
*Qiyu Hou,Jun Wang*
...
<details>
  <summary>Details</summary>
Motivation: 解决大规模、密集表格结构识别的挑战，提供一种稳定且高效的解决方案。

Method: 将行和列分割任务转化为序列标注问题，利用双Transformer编码器；合并任务转化为网格分类问题，使用额外的Transformer编码器。
...
### [72] [MAGNET: A Multi-agent Framework for Finding Audio-Visual Needles by Reasoning over Multi-Video Haystacks](https://arxiv.org/abs/2506.07016)
*Sanjoy Chowdhury,Mohamed Elmoghany,Yohan Abeysinghe,Junjie Fei,Sayan Nag,Salman Khan,Mohamed Elhoseiny,Dinesh Manocha*

Main category: cs.CV

TL;DR: 论文提出了AV-HaystacksQA任务和AVHaystacks基准，用于评估大型多模态模型在多视频检索和时间定位任务中的能力，并提出了MAGNET框架和两个新评价指标STEM与MTGS。


<details>
  <summary>Details</summary>
Motivation: 现有视频问答基准局限于单一视频片段，无法满足实际应用中大规模音频-视觉检索和复杂推理的需求。

Method: 提出AVHaystacks基准和MAGNET多智能体框架，用于多视频检索和时间定位任务。

Result: MAGNET在BLEU@4和GPT评分上分别比基线方法提升了89%和65%。

Conclusion: AVHaystacks和MAGNET为多视频复杂推理任务提供了有效解决方案和评估工具。
...
### [73] [Interpretable and Reliable Detection of AI-Generated Images via Grounded Reasoning in MLLMs](https://arxiv.org/abs/2506.07045)
*Yikun Ji,Hong Yan,Jun Lan,Huijia Zhu,Weiqiang Wang,Qi Fan,Liqing Zhang,Jianfu Zhang*

Main category: cs.CV

TL;DR: 本文提出了一种基于多模态大语言模型（MLLMs）的方法，用于检测AI生成图像并提供可解释的视觉定位和文本解释。通过构建标注数据集和多阶段优化策略，模型在检测和定位性能上显著优于基线方法。
...
Method: 构建标注数据集，标注合成伪影；采用多阶段优化策略，平衡检测准确性、视觉定位和文本解释。

Result: 模型在检测AI生成图像和定位视觉缺陷方面表现优异，显著优于基线方法。

Conclusion: 通过数据标注和多阶段优化，MLLMs能够实现高效检测和可解释的视觉-文本推理。
...
### [74] [From Swath to Full-Disc: Advancing Precipitation Retrieval with Multimodal Knowledge Expansion](https://arxiv.org/abs/2506.07050)
...
<details>
  <summary>Details</summary>
Motivation: 现有红外降水反演算法精度低，而被动微波和雷达方法范围有限，因此需要一种能在大范围内实现高精度降水反演的技术。

Method: PRE-Net采用两阶段流程：1) Swath-Distilling阶段通过CoMWE技术将多模态知识迁移到红外模型；2) Full-Disc Adaptation阶段通过Self-MaskTune平衡多模态和红外知识。

Result: 实验表明，PRE-Net在PRE基准测试中显著优于PERSIANN-CCS、PDIR和IMERG等现有方法。

Conclusion: PRE-Net通过多模态知识扩展实现了高精度、大范围的红外降水反演，为相关领域提供了有效解决方案。
...
### [75] [A Layered Self-Supervised Knowledge Distillation Framework for Efficient Multimodal Learning on the Edge](https://arxiv.org/abs/2506.07055)
...
TL;DR: LSSKD框架通过中间特征图的辅助分类器生成自监督知识，无需依赖预训练教师网络，显著提升模型性能，且推理时无额外计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖预训练教师网络，而LSSKD旨在通过自监督知识蒸馏提升紧凑模型的性能，适用于计算资源有限的设备。

Method: 在中间特征图上添加辅助分类器，生成多样化的自监督知识，实现不同网络阶段的一对一知识转移。

Result: 在CIFAR-100和ImageNet上分别超越现有方法4.54%和0.32%，在少样本学习场景下也达到最优结果。

Conclusion: LSSKD有效提升模型泛化能力，适用于多模态感知和低计算资源环境，支持弱监督下的智能代理开发。
...
### [76] [D2R: dual regularization loss with collaborative adversarial generation for model robustness](https://arxiv.org/abs/2506.07056)
*Zhenyu Liu,Huizhi Liang,Rajiv Ranjan,Zhanxing Zhu,Vaclav Snasel,Varun Ojha*

Main category: cs.CV

TL;DR: 提出了一种双正则化损失（D2R Loss）方法和协作对抗生成（CAG）策略，用于增强深度神经网络的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在目标模型的损失函数引导和对抗样本生成协作方面存在不足，需要改进。

Method: D2R Loss通过两步优化增强模型鲁棒性，CAG通过梯度协作生成对抗样本。

Result: 在多个基准数据集和模型上验证了D2R Loss与CAG的高效性。

Conclusion: D2R Loss与CAG能显著提升模型的对抗鲁棒性。
...
### [77] [FLAIR-HUB: Large-scale Multimodal Dataset for Land Cover and Crop Mapping](https://arxiv.org/abs/2506.07080)
*Anatol Garioud,Sébastien Giordano,Nicolas David,Nicolas Gonthier*

Main category: cs.CV

TL;DR: FLAIR-HUB是法国IGN推出的多传感器土地覆盖数据集，结合六种模态数据，支持高精度土地覆盖和作物分类。


<details>
  <summary>Details</summary>
Motivation: 解决地球观测数据量大且异构带来的处理和标注挑战。

Method: 结合六种模态数据（如航空影像、Sentinel-1/2时间序列等），并通过深度学习方法（CNN、Transformer）进行多模态融合和多任务学习。

Result: 最佳土地覆盖分类准确率为78.2%，mIoU为65.8%，多模态融合效果显著。

Conclusion: FLAIR-HUB为土地覆盖和作物分类提供了高质量数据集，支持监督和多模态预训练。
...
### [78] [UCOD-DPL: Unsupervised Camouflaged Object Detection via Dynamic Pseudo-label Learning](https://arxiv.org/abs/2506.07087)
...
<details>
  <summary>Details</summary>
Motivation: 现有无监督伪装目标检测方法因伪标签噪声大和解码器简单导致性能较低，需解决模型易拟合错误知识及语义特征捕获不足的问题。

Method: 采用教师-学生框架，结合自适应伪标签模块（APM）、双分支对抗解码器（DBA）和二次细化机制（Look-Twice），动态优化伪标签并提升特征学习能力。
...
### [79] [SceneLCM: End-to-End Layout-Guided Interactive Indoor Scene Generation with Latent Consistency Model](https://arxiv.org/abs/2506.07091)
*Yangkai Lin,Jiabao Lei,Kui Jia*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在室内场景生成中存在编辑限制、物理不一致、人力成本高、单房间限制和材质质量差等问题。
...
### [80] [EdgeSpotter: Multi-Scale Dense Text Spotting for Industrial Panel Monitoring](https://arxiv.org/abs/2506.07112)
...
Method: 开发了基于Transformer的高效混合器学习多级特征依赖，设计了Catmull-Rom样条特征采样方法，并构建了工业面板监控数据集IPM。

Result: 在IPM数据集上的实验验证了方法的优越性能，边缘AI视觉系统的实际测试证明了其实用性。

Conclusion: EdgeSpotter在工业面板监控中表现出高效和鲁棒性，代码和演示已开源。
...
### [81] [Image segmentation and classification of E-waste for waste segregation](https://arxiv.org/abs/2506.07122)
...
Conclusion: 模型将整合到分拣机器人中，实现电子废物的自动化分类。
...
### [82] [Hi-VAE: Efficient Video Autoencoding with Global and Detailed Motion](https://arxiv.org/abs/2506.07136)
*Huaize Liu,Wenzhang Sun,Qiyuan Zhang,Donglin Di,Biao Gong,Hao Li,Chen Wei,Changqing Zou*

Main category: cs.CV

TL;DR: Hi-VAE是一种高效的视频自动编码框架，通过分层编码视频动态的粗到细运动表示，显著减少冗余并实现高压缩率。


<details>
  <summary>Details</summary>
Motivation: 现有视频自动编码方法在动态建模中存在冗余问题，导致压缩效率低下和训练成本高。

Method: Hi-VAE将视频动态分解为全局运动和细节运动两个潜在空间，使用自监督运动编码器压缩表示，并通过条件扩散解码器重建视频。

Result: 实验显示Hi-VAE压缩率高达1428倍，远超基线方法（如Cosmos-VAE的48倍），同时保持高质量重建和下游任务性能。
...
### [83] [Learning Compact Vision Tokens for Efficient Large Multimodal Models](https://arxiv.org/abs/2506.07138)
...
<details>
  <summary>Details</summary>
Motivation: 大型多模态模型（LMMs）因计算成本高和视觉令牌序列的二次复杂度面临挑战，需优化令牌处理效率。

Method: 通过STF融合空间相邻令牌以减少序列长度，并通过MBTF补充多粒度特征。

Result: 在8个视觉语言基准测试中，仅使用基线25%的视觉令牌，性能与基线相当或更优。
...
### [84] [GoTrack: Generic 6DoF Object Pose Refinement and Tracking](https://arxiv.org/abs/2506.07155)
*Van Nguyen Nguyen,Christian Forster,Sindi Shkodrani,Vincent Lepetit,Bugra Tekin,Cem Keskin,Tomas Hodan*

Main category: cs.CV

TL;DR: GoTrack是一种基于CAD的高效6DoF物体姿态精修与跟踪方法，无需对象特定训练，结合模型到帧和帧到帧注册，通过光流估计实现。


<details>
  <summary>Details</summary>
Motivation: 现有跟踪方法仅依赖分析-合成方法进行模型到帧注册，GoTrack通过集成帧到帧注册节省计算并稳定跟踪。

Method: 使用标准神经网络块（基于DINOv2的transformer）简化模型到帧注册，并采用轻量级现成光流模型处理帧到帧注册。

Result: GoTrack结合现有粗姿态估计方法，在标准6DoF物体姿态估计与跟踪基准上达到RGB-only的SOTA结果。

Conclusion: GoTrack提供了一种高效且准确的姿态跟踪解决方案，代码和模型已开源。
...
### [85] [Faster than Fast: Accelerating Oriented FAST Feature Detection on Low-end Embedded GPUs](https://arxiv.org/abs/2506.07164)
...
### [86] [Frame Guidance: Training-Free Guidance for Frame-Level Control in Video Diffusion Models](https://arxiv.org/abs/2506.07177)
...
Method: 提出Frame Guidance，基于帧级信号（如关键帧、风格参考图等），采用简单的潜在处理方法减少内存使用，并结合潜在优化策略实现全局一致性。

Result: 实验表明，Frame Guidance能在多种任务（如关键帧引导、风格化、循环）中生成高质量可控视频，兼容任意视频模型。

Conclusion: Frame Guidance为可控视频生成提供了一种高效、无需训练的解决方案，适用于多样化任务和输入信号。
...
### [87] [Hierarchical Feature-level Reverse Propagation for Post-Training Neural Networks](https://arxiv.org/abs/2506.07188)
*Ni Ding,Lei He,Shengbo Eben Li,Keqiang Li*

Main category: cs.CV

TL;DR: 本文提出了一种针对预训练神经网络的分层解耦后训练框架，通过重构中间特征图引入代理监督信号，提升模型透明度和训练灵活性。


<details>
  <summary>Details</summary>
Motivation: 解决端到端自动驾驶中黑盒模型的可解释性和安全性问题。

Method: 利用真实标签重构中间特征图，将特征级反向计算形式化为优化问题，并转化为线性方程组或最小二乘问题。

Result: 在多个标准图像分类基准测试中表现出优越的泛化性能和计算效率。

Conclusion: 该方法为神经网络训练提供了一种新颖且高效的范式，验证了其有效性和潜力。
...
### [88] [SAP-Bench: Benchmarking Multimodal Large Language Models in Surgical Action Planning](https://arxiv.org/abs/2506.07196)
...
<details>
  <summary>Details</summary>
Motivation: 当前基准测试无法充分评估手术决策任务所需的精确性和复杂性，尤其是在生命关键领域。

Method: 引入SAP-Bench数据集，包含1226个临床验证的手术动作片段，并提出MLLM-SAP框架，利用MLLMs生成手术动作建议。

Result: 评估了7种先进MLLMs，发现其在预测下一步手术动作方面存在显著性能差距。

Conclusion: SAP-Bench和MLLM-SAP为手术动作规划任务提供了有效的评估工具，并揭示了当前模型的局限性。
...
### [89] [TV-LiVE: Training-Free, Text-Guided Video Editing via Layer Informed Vitality Exploitation](https://arxiv.org/abs/2506.07205)
...
<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法主要关注风格转换等简单任务，复杂任务如对象添加和非刚性编辑尚未充分探索。

Method: 通过识别视频生成模型中的关键层（与RoPE相关），选择性注入特征实现编辑，并提取掩码区域用于对象添加。

Result: TV-LiVE在对象添加和非刚性编辑任务上优于现有方法。

Conclusion: TV-LiVE为复杂视频编辑任务提供了高效且可控的解决方案。
...
### [90] [Backdoor Attack on Vision Language Models with Stealthy Semantic Manipulation](https://arxiv.org/abs/2506.07214)
*Zhiyuan Zhong,Zhen Sun,Yepang Liu,Xinlei He,Guanhong Tao*

Main category: cs.CV

TL;DR: 论文提出了一种新型的跨模态后门攻击方法BadSem，利用语义不匹配作为隐式触发器，攻击视觉语言模型（VLMs），实验表明其攻击成功率高达98%，且现有防御策略难以应对。


<details>
  <summary>Details</summary>
Motivation: 现有后门攻击主要依赖单模态触发器，而VLMs的跨模态融合特性尚未被充分探索，因此研究基于语义不匹配的跨模态后门攻击具有重要意义。

Method: 提出BadSem攻击方法，通过故意在训练时错配图像-文本对注入后门，并构建SIMBad数据集支持语义操纵（如颜色和对象属性）。

Result: 在四种广泛使用的VLMs上，BadSem平均攻击成功率（ASR）超过98%，且能泛化到分布外数据集和跨模态传输。注意力可视化显示后门模型在语义敏感区域异常聚焦。

Conclusion: BadSem揭示了VLMs在语义层面的安全漏洞，现有防御策略无效，亟需研究更安全的部署方案。
...
### [91] [AugmentGest: Can Random Data Cropping Augmentation Boost Gesture Recognition Performance?](https://arxiv.org/abs/2506.07216)
*Nada Aboudeshish,Dmitry Ignatov,Radu Timofte*

Main category: cs.CV

TL;DR: 本文提出了一种综合数据增强框架，通过几何变换、随机裁剪等方法增强骨架数据集的多样性，显著提升了手势识别和动作识别的性能。


<details>
  <summary>Details</summary>
Motivation: 针对骨架数据集中数据多样性不足的问题，提出一种全面的数据增强方法，以模拟真实世界的变化，提升模型泛化能力。

Method: 整合几何变换、随机裁剪、旋转、缩放和强度变换等方法，生成每个样本的三个增强版本，从而四倍扩大数据集规模。

Result: 在多个基准数据集和模型上验证了框架的有效性，显著提升了模型性能，并在手势识别和动作识别任务中取得了最先进的结果。

Conclusion: 该框架不仅提升了模型性能，还为实际应用中的手势识别和动作识别提供了可扩展的解决方案。
...
### [92] [Hallucination at a Glance: Controlled Visual Edits and Fine-Grained Multimodal Learning](https://arxiv.org/abs/2506.07227)
...
<details>
  <summary>Details</summary>
Motivation: MLLMs在视觉语言任务中表现优秀，但在细粒度视觉差异任务中容易产生幻觉或遗漏语义变化，这归因于训练数据和学习目标的局限性。

Method: 提出了一种可控数据生成流程，构建了包含50K图像-文本对的微编辑数据集（MED），并设计了监督微调框架，引入特征级一致性损失。

Result: 在微编辑检测基准测试中，方法显著提升了差异检测准确率并减少了幻觉现象，同时在标准视觉语言任务（如图像描述和视觉问答）中表现一致提升。

Conclusion: 通过结合针对性数据和目标对齐，有效提升了MLLMs在细粒度视觉推理中的表现。
...
### [93] [Multi-Step Visual Reasoning with Visual Tokens Scaling and Verification](https://arxiv.org/abs/2506.07235)
*Tianyi Bai,Zengjie Hu,Fupeng Sun,Jiantao Qiu,Yizhen Jiang,Guangxin He,Bohan Zeng,Conghui He,Binhang Yuan,Wentao Zhang*

Main category: cs.CV

TL;DR: 论文提出了一种动态视觉推理框架，通过迭代优化视觉令牌提升多模态大语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs采用静态推理范式，限制了其动态适应和迭代优化的能力，而人类感知是动态和反馈驱动的。

Method: 提出基于马尔可夫决策过程的框架，结合推理器和验证器，通过多步直接偏好优化（DPO）训练验证器。

Result: 方法在多个视觉推理基准上显著优于现有方法，提高了准确性和可解释性。
...
### [94] [From Generation to Generalization: Emergent Few-Shot Learning in Video Diffusion Models](https://arxiv.org/abs/2506.07280)
*Pablo Acuaviva,Aram Davtyan,Mariam Hassan,Sebastian Stapf,Ahmad Rahimi,Alexandre Alahi,Paolo Favaro*

Main category: cs.CV

TL;DR: 视频扩散模型（VDMs）不仅是生成工具，还能通过少量示例微调适应新任务，展现出广泛的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 探索VDMs在生成视频之外的潜力，尤其是其内部隐含的视觉世界理解和结构化表示能力。

Method: 提出一个少样本微调框架，将任务转化为视觉转换，训练LoRA权重而不改变冻结VDM的生成接口。

Result: 模型在从低级视觉任务（如分割）到高级推理任务（如ARC-AGI）上表现出强大的泛化能力。

Conclusion: VDMs可视为适应性强的视觉学习器，有望成为未来视觉基础模型的核心。
...
### [95] [Multi-Step Guided Diffusion for Image Restoration on Edge Devices: Toward Lightweight Perception in Embodied AI](https://arxiv.org/abs/2506.07286)
*Aditya Chakravarty*

Main category: cs.CV

TL;DR: 本文提出了一种多步优化策略，显著提升了扩散模型在图像恢复任务中的性能，包括图像质量、感知准确性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如MPGD）在每一步去噪中仅应用单次梯度更新，限制了恢复的保真度和鲁棒性，尤其是在嵌入式或分布外场景中。

Method: 在每一步去噪时间步中引入多步优化策略，增加梯度更新次数。

Result: 实验表明，该方法在超分辨率和高斯去模糊任务中显著提升了LPIPS和PSNR指标，且延迟开销极小。

Conclusion: MPGD作为一种轻量级、即插即用的恢复模块，在无人机和移动机器人等嵌入式AI代理中具有实时视觉感知的潜力。
...
### [96] [FANVID: A Benchmark for Face and License Plate Recognition in Low-Resolution Videos](https://arxiv.org/abs/2506.07304)
*Kavitha Viswanathan,Vrinda Goel,Shlesh Gholap,Devayan Ghosh,Madhav Gupta,Dhruvi Ganatra,Sanket Potdar,Amit Sethi*

Main category: cs.CV

TL;DR: FANVID是一个新的视频基准数据集，用于低分辨率（LR）人脸和车牌识别任务，包含1,463个LR视频片段，定义了人脸匹配和车牌识别两项任务，并提供了基线方法和评估指标。


<details>
  <summary>Details</summary>
Motivation: 解决现实监控中低分辨率视频帧导致的人脸和车牌难以识别的问题，推动基于时序信息的识别模型发展。

Method: 构建FANVID数据集，包含LR视频片段，定义人脸匹配和车牌识别任务，使用预训练视频超分辨率、检测和识别技术作为基线方法。

Result: 基线方法在人脸匹配和车牌识别任务中分别取得0.58和0.42的分数，显示任务的可行性和挑战性。

Conclusion: FANVID旨在促进低分辨率识别中时序建模的创新，适用于监控、法医学和自动驾驶等领域。
...
### [97] [AllTracker: Efficient Dense Point Tracking at High Resolution](https://arxiv.org/abs/2506.07310)
...
TL;DR: AllTracker是一个通过估计查询帧与视频中其他帧之间的流场来估计长范围点轨迹的模型，提供高分辨率和密集（全像素）对应字段。


<details>
  <summary>Details</summary>
Motivation: 现有点跟踪方法无法提供高分辨率和密集对应字段，而现有光流方法仅能对应下一帧而非数百帧。

Method: 结合光流和点跟踪技术，通过低分辨率网格的迭代推断、2D卷积层空间传播信息和像素对齐注意力层时间传播信息。

Result: 模型快速且参数高效（1600万参数），在高分辨率下（768x1024像素，40G GPU）实现最先进的点跟踪精度。

Conclusion: 设计支持更广泛的数据集训练，对架构细节和训练方案进行了广泛消融研究，代码和模型权重已开源。
...
### [98] ["CASE: Contrastive Activation for Saliency Estimation](https://arxiv.org/abs/2506.07327)
*Dane Williamson,Yangfeng Ji,Matthew Dwyer*

Main category: cs.CV

TL;DR: 本文提出了一种诊断测试（CASE）来评估显著性方法的类别敏感性，发现现有方法在区分不同类别标签时表现不佳，并提出了更可靠的对比解释方法。


<details>
  <summary>Details</summary>
Motivation: 显著性方法在可视化模型预测时存在局限性，尤其是对类别标签的区分能力不足，需要更可靠的诊断和解释方法。

Method: 提出了一种诊断测试（CASE），通过对比实验评估显著性方法的类别敏感性，并设计了一种新的对比解释方法。

Result: 实验表明，许多广泛使用的显著性方法对类别标签不敏感，而CASE方法能生成更忠实且类别特定的解释。

Conclusion: CASE方法在类别敏感性和解释忠实度上优于现有方法，揭示了显著性方法的结构性缺陷。
...
### [99] [Hierarchical Scoring with 3D Gaussian Splatting for Instance Image-Goal Navigation](https://arxiv.org/abs/2506.07338)
*Yijie Deng,Shuaihang Yuan,Geeta Chandra Raju Bethala,Anthony Tzes,Yu-Shen Liu,Yi Fang*

Main category: cs.CV

TL;DR: 本文提出了一种新的实例图像目标导航（IIN）框架，通过分层评分范式选择最优视角，减少冗余，提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖随机采样视角，导致冗余和效率低下，缺乏优化的视角选择机制。

Method: 结合跨层级语义评分（基于CLIP）和局部几何评分，选择最优视角进行目标匹配。
...
### [100] [CBAM-STN-TPS-YOLO: Enhancing Agricultural Object Detection through Spatially Adaptive Attention Mechanisms](https://arxiv.org/abs/2506.07357)
*Satvik Praveen,Yoonsung Jung*

Main category: cs.CV

TL;DR: 论文提出CBAM-STN-TPS-YOLO模型，结合TPS和CBAM提升目标检测精度，适用于农业场景。
...
Method: 集成TPS到STN中实现非刚性空间变换，并利用CBAM抑制背景噪声和突出关键特征。

Result: 在PGP数据集上表现优于STN-YOLO，减少12%误检，提升精度和召回率。

Conclusion: 轻量级模型提升空间感知能力，适合实时边缘部署，适用于精准农业。
...
### [101] [Multiple Object Stitching for Unsupervised Representation Learning](https://arxiv.org/abs/2506.07364)
*Chengchao Shen,Dawei Liu,Jianxin Wang*

Main category: cs.CV

TL;DR: 本文提出了一种名为MOS的方法，通过拼接单目标图像生成多目标图像，从而提升无监督表示在多目标图像上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有对比学习方法在单目标图像上表现良好，但在多目标图像上性能较差。

Method: 通过拼接单目标图像生成多目标图像，利用预定的对象对应关系优化表示。

Result: 在ImageNet、CIFAR和COCO数据集上，MOS方法在单目标和多目标图像上均取得领先的无监督表示性能。

Conclusion: MOS方法通过对象对应关系优化表示，适用于复杂下游任务。
...
### [102] [C3S3: Complementary Competition and Contrastive Selection for Semi-Supervised Medical Image Segmentation](https://arxiv.org/abs/2506.07368)
*Jiaying He,Yitong Lin,Jiahe Chen,Honghui Xu,Jianwei Zheng*

Main category: cs.CV

TL;DR: 论文提出了一种名为C3S3的半监督医学图像分割模型，通过互补竞争和对比选择提升边界细节的准确性。
...
### [103] [Generative Models at the Frontier of Compression: A Survey on Generative Face Video Coding](https://arxiv.org/abs/2506.07369)
*Bolin Chen,Shanzhi Yin,Goluck Konuko,Giuseppe Valenzise,Zihan Zhang,Shiqi Wang,Yan Ye*

Main category: cs.CV

TL;DR: 本文综述了生成式人脸视频编码（GFVC）技术，分析了其在高保真、低码率视频通信中的优势，并探讨了标准化和应用前景。


<details>
  <summary>Details</summary>
Motivation: 随着深度生成模型的兴起，GFVC通过语义感知表示和逼真合成能力，显著提升了人脸视频压缩效率，超越了现有视频编码标准（如VVC）。本文旨在填补理论与工业标准化之间的关键空白。

Method: 综述了多种GFVC方法，比较了不同特征表示和优化策略，构建了大规模GFVC压缩视频数据库，并提出了标准化框架和低复杂度系统。

Result: GFVC在超低码率下实现高保真视频通信，优于VVC标准。数据库和标准化框架为未来应用奠定了基础。

Conclusion: GFVC在工业应用中潜力巨大，但仍需解决当前挑战，如标准化和质量评估，以推动未来发展。
...
### [104] [ARGUS: Hallucination and Omission Evaluation in Video-LLMs](https://arxiv.org/abs/2506.07371)
*Ruchit Rawal,Reza Shirkavand,Heng Huang,Gowthami Somepalli,Tom Goldstein*

Main category: cs.CV

TL;DR: ARGUS是一个用于评估视频大语言模型（VideoLLM）自由文本生成性能的基准测试，重点关注幻觉和遗漏问题。
...
Method: 通过比较VideoLLM生成的视频字幕与人工标注的真实字幕，量化幻觉（错误描述）和遗漏（重要细节缺失）的发生率。

Result: ARGUS提供了双重指标，分别衡量幻觉率和遗漏率，从而全面评估视频字幕性能。
...
### [105] [DINO-CoDT: Multi-class Collaborative Detection and Tracking with Vision Foundation Models](https://arxiv.org/abs/2506.07375)
*Xunjie He,Christina Dao Wen Lee,Meiling Wang,Chengran Yuan,Zefan Huang,Yufeng Yue,Marcelo H. Ang Jr*

Main category: cs.CV

TL;DR: 提出了一种多类协作检测与跟踪框架，通过全局空间注意力融合模块和视觉语义重识别模块提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有协作感知研究主要针对车辆类，缺乏多类解决方案，限制了实际应用。

Method: 设计了全局空间注意力融合模块（GSAF）、视觉语义重识别模块（REID）和速度自适应轨迹管理模块（VATM）。

Result: 在V2X-Real和OPV2V数据集上显著优于现有方法。

Conclusion: 该框架有效提升了多类对象的检测与跟踪性能，适用于多样化道路场景。
...
### [106] [Adapter Naturally Serves as Decoupler for Cross-Domain Few-Shot Semantic Segmentation](https://arxiv.org/abs/2506.07376)
...
Method: 提出域特征导航器（DFN）和SAM-SVN方法，利用模型结构自然解耦域信息。

Result: 在1-shot和5-shot场景下，分别超越现有方法2.69%和4.68% MIoU。

Conclusion: DFN和SAM-SVN方法有效解决了域差距和少样本微调问题，显著提升了性能。
...
### [107] [MrM: Black-Box Membership Inference Attacks against Multimodal RAG Systems](https://arxiv.org/abs/2506.07399)
*Peiru Yang,Jinhua Yin,Haoran Zheng,Xueying Bai,Huili Wang,Yufei Sun,Xintian Li,Shangguang Wang,Yongfeng Huang,Tao Qi*

Main category: cs.CV

TL;DR: 论文提出了一种针对多模态RAG系统的黑盒成员推理攻击框架MrM，通过多目标数据扰动和反事实攻击约束，有效泄露敏感信息。
...
Method: 提出MrM框架，包括对象感知数据扰动、反事实掩码选择和统计推理，以增强攻击效果。

Result: 在多个数据集和模型上验证了MrM的有效性，攻击性能稳定且适应性强。
...
### [108] [Compressed Feature Quality Assessment: Dataset and Baselines](https://arxiv.org/abs/2506.07412)
*Changsheng Gao,Wei Zhou,Guosheng Lin,Weisi Lin*

Main category: cs.CV

TL;DR: 论文提出了压缩特征质量评估（CFQA）问题，并创建了首个包含300个原始特征和12000个压缩特征的基准数据集，评估了三种常用指标的性能，发现现有指标无法完全捕捉语义失真。
...
Method: 提出CFQA问题，构建基准数据集，评估MSE、余弦相似度和中心核对齐三种指标的性能。

Result: 数据集具有代表性，但现有指标无法充分衡量语义失真，需更精细的指标。
...
### [109] [DPFormer: Dynamic Prompt Transformer for Continual Learning](https://arxiv.org/abs/2506.07414)
*Sheng-Kai Huang,Jiun-Feng Chang,Chun-Rong Huang*

Main category: cs.CV

TL;DR: 提出了一种动态提示变换器（DPFormer）解决持续学习中的稳定性-可塑性困境和任务间混淆问题，通过提示方案和统一分类模块实现高性能。


<details>
  <summary>Details</summary>
Motivation: 解决持续学习中的灾难性遗忘和任务间混淆问题，同时保持模型参数几乎固定。

Method: 使用动态提示变换器（DPFormer）和提示方案，结合二元交叉熵损失、知识蒸馏损失和辅助损失进行端到端训练。

Result: 在CIFAR-100、ImageNet100和ImageNet1K数据集上，不同类增量设置下表现最优。

Conclusion: DPFormer通过提示方案和统一分类模块有效解决了持续学习中的关键问题，性能优于现有方法。
...
### [110] [FAMSeg: Fetal Femur and Cranial Ultrasound Segmentation Using Feature-Aware Attention and Mamba Enhancement](https://arxiv.org/abs/2506.07431)
*Jie He,Minglang Chen,Minying Lu,Bocheng Liang,Junming Wei,Guiyan Peng,Jiaxi Chen,Ying Tan*

Main category: cs.CV

TL;DR: 提出了一种基于特征感知和Mamba增强的胎儿股骨和颅骨超声图像分割模型，解决了现有模型在噪声高、相似性强的超声图像中的适应性差和小目标分割锯齿效应问题。


<details>
  <summary>Details</summary>
Motivation: 超声图像分割对精确生物测量和评估至关重要，但现有模型难以适应高噪声和高相似性的超声对象，尤其是小目标分割时锯齿效应明显。

Method: 设计了纵向和横向独立视角扫描卷积块、特征感知模块，结合Mamba优化的残差结构，增强局部细节捕捉和上下文信息融合，抑制原始噪声干扰。

Result: FAMSeg网络在实验中实现了最快的损失下降和最佳分割性能，适用于不同大小和方向的图像。
...
### [111] [Prompt to Protection: A Comparative Study of Multimodal LLMs in Construction Hazard Recognition](https://arxiv.org/abs/2506.07436)
*Nishi Chaudhary,S M Jamil Uddin,Sathvik Sharath Chandra,Anto Ovid,Alex Albert*

Main category: cs.CV

TL;DR: 研究比较了五种多模态大语言模型（LLMs）在建筑工地视觉危险识别中的表现，发现提示策略（如逐步推理）显著影响性能，GPT-4.5和GPT-o3表现最佳。
...
Method: 对五种LLMs（Claude-3 Opus、GPT-4.5、GPT-4o、GPT-o3、Gemini 2.0 Pro）进行对比测试，采用零样本、少样本和逐步推理（CoT）三种提示策略。

Result: CoT提示策略效果最佳，GPT-4.5和GPT-o3在多数场景中表现最优，提示设计对模型准确性至关重要。

Conclusion: 研究为建筑安全领域的多模态LLMs应用提供了实用指导，强调了提示工程的重要性。
...
### [112] [PhysiInter: Integrating Physical Mapping for High-Fidelity Human Interaction Generation](https://arxiv.org/abs/2506.07456)
*Wei Yao,Yunlian Sun,Chang Liu,Hongwen Zhang,Jinhui Tang*

Main category: cs.CV

TL;DR: 论文提出了一种结合物理映射的方法，用于生成更符合物理约束的多人物运动，显著提升了运动质量和物理保真度。


<details>
  <summary>Details</summary>
Motivation: 现有运动捕捉技术和生成模型常忽略物理约束，导致运动中的穿模、滑动和漂浮等问题，尤其在多人交互场景中更为突出。

Method: 通过物理仿真环境中的运动模仿，将目标运动映射到物理有效空间，并引入运动一致性（MC）和基于标记的交互（MI）损失函数。

Result: 实验表明，该方法在生成运动质量上表现优异，物理保真度提升了3%-89%。

Conclusion: 物理映射方法有效解决了运动生成中的物理约束问题，尤其在多人交互场景中表现出色。
...
### [113] [GLOS: Sign Language Generation with Temporally Aligned Gloss-Level Conditioning](https://arxiv.org/abs/2506.07460)
*Taeryung Lee,Hyeongjin Nam,Gyeongsik Moon,Kyoung Mu Lee*

Main category: cs.CV

TL;DR: 论文提出了一种基于时间对齐的gloss级条件的SLG框架GLOS，解决了现有方法在词序和语义准确性上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有SLG方法因使用句子级条件导致词序错误和语义准确性低，无法捕捉手语的时间结构和词级语义。

Method: 提出GLOS框架，采用gloss级条件和时间对齐条件融合模块（TAC），实现细粒度控制和词序保持。

Result: GLOS在CSL-Daily和Phoenix-2014T数据集上表现优于现有方法，生成的手语词序正确且语义准确。

Conclusion: GLOS通过gloss级条件和TAC模块，显著提升了SLG的词序和语义准确性。
...
### [114] [DeepVideo-R1: Video Reinforcement Fine-Tuning via Difficulty-aware Regressive GRPO](https://arxiv.org/abs/2506.07464)
...
<details>
  <summary>Details</summary>
Motivation: GRPO在视频大语言模型中的应用研究较少，且存在依赖安全机制和优势消失问题，亟需改进。

Method: 提出Reg-GRPO将GRPO目标重构为回归任务，直接预测优势值，并设计难度感知数据增强策略。
...
### [115] [Ambiguity-Restrained Text-Video Representation Learning for Partially Relevant Video Retrieval](https://arxiv.org/abs/2506.07471)
*CH Cho,WJ Moon,W Jun,MS Jung,JP Heo*

Main category: cs.CV

TL;DR: 论文提出了一种名为ARL的框架，用于解决PRVR任务中文本与视频内容之间的模糊性问题，通过多正对比学习和双重三元组损失来学习语义关系。


<details>
  <summary>Details</summary>
Motivation: 传统的PRVR训练假设文本查询与视频是一对一关系，但实际中文本与视频内容存在模糊性，作者希望通过模型学习解决这一问题。

Method: 提出ARL框架，通过不确定性和相似性检测模糊对，并采用多正对比学习和双重三元组损失进行分层学习，同时探索视频内部的细粒度关系。

Result: 提出的方法在PRVR任务中表现出有效性。

Conclusion: ARL框架能够有效解决文本与视频内容的模糊性问题，提升PRVR任务的性能。
...
### [116] [CoCoA-Mix: Confusion-and-Confidence-Aware Mixture Model for Context Optimization](https://arxiv.org/abs/2506.07484)
*Dasol Hong,Wooju Lee,Hyun Myung*

Main category: cs.CV

TL;DR: 论文提出了一种混淆感知损失（CoA-loss）和置信感知权重（CoA-weights）的方法，通过混合模型（CoCoA-Mix）提升视觉语言模型在任务专用化和泛化能力上的表现。


<details>
  <summary>Details</summary>
Motivation: 解决提示调优中因冻结编码器导致的特征不对齐问题，从而提升任务专用化和泛化能力。

Method: 提出CoA-loss优化决策边界以提升专用化，使用CoA-weights的混合模型增强泛化能力。

Result: CoCoA-Mix在实验中表现优于现有方法，显著提升了专用化和泛化能力。

Conclusion: CoCoA-Mix通过混淆感知损失和置信感知权重，有效解决了提示调优中的核心挑战。
...
### [117] [Drive Any Mesh: 4D Latent Diffusion for Mesh Deformation from Video](https://arxiv.org/abs/2506.07489)
*Yahao Shi,Yang Liu,Yanmin Wu,Xing Liu,Chen Zhao,Jie Luo,Bin Zhou*

Main category: cs.CV

TL;DR: DriveAnyMesh是一种通过单目视频驱动网格的方法，解决了当前4D生成技术在渲染引擎中的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有4D生成技术（如隐式方法和骨骼方法）在渲染效率、手动操作和跨类别泛化方面存在问题，而动画化现有3D资产需要深入理解其结构。

Method: 提出了一种4D扩散模型，通过去噪潜在集序列并解码为网格动画，利用基于变压器的变分自编码器捕捉3D形状和运动信息。

Result: 实验表明，DriveAnyMesh能高效生成高质量动画，适用于复杂动作，并兼容现代渲染引擎。
...
### [118] [SpatialLM: Training Large Language Models for Structured Indoor Modeling](https://arxiv.org/abs/2506.07491)
*Yongsen Mao,Junhao Zhong,Chuan Fang,Jia Zheng,Rui Tang,Hao Zhu,Ping Tan,Zihan Zhou*

Main category: cs.CV

TL;DR: SpatialLM是一个处理3D点云数据的大语言模型，能生成结构化3D场景理解输出，如墙壁、门窗等建筑元素和语义类别对象框。
...
Method: 基于开源大语言模型微调，使用大规模合成数据集（12,328个室内场景）训练。

Result: 在布局估计任务上达到最优性能，3D物体检测结果具有竞争力。

Conclusion: 展示了增强大语言模型空间理解能力的可行路径。
...
### [119] [Genesis: Multimodal Driving Scene Generation with Spatio-Temporal and Cross-Modal Consistency](https://arxiv.org/abs/2506.07497)
...
Conclusion: Genesis在多模态生成中实现了高一致性和实用性，验证了其语义保真度。
...
### [120] [MoQAE: Mixed-Precision Quantization for Long-Context LLM Inference via Mixture of Quantization-Aware Experts](https://arxiv.org/abs/2506.07533)
...
<details>
  <summary>Details</summary>
Motivation: 优化大型语言模型（LLM）的长上下文推理中KV缓存的高内存消耗问题。

Method: 1. 将不同量化位宽配置视为专家，采用MoE方法选择最优配置；2. 设计轻量级路由器微调过程；3. 引入路由冻结（RF）和路由共享（RS）机制。

Result: 在多个基准数据集上，MoQAE在效率和效果上均优于现有KV缓存量化方法。

Conclusion: MoQAE通过混合精度量化和优化路由机制，显著提升了KV缓存的内存效率，同时保持了模型性能。
...
### [121] [Domain Randomization for Object Detection in Manufacturing Applications using Synthetic Data: A Comprehensive Study](https://arxiv.org/abs/2506.07539)
*Xiaomeng Zhu,Jacob Henningsson,Duruo Li,Pär Mårtensson,Lars Hanson,Mårten Björkman,Atsuto Maki*

Main category: cs.CV

TL;DR: 论文提出了一种综合数据生成管道，用于制造业目标检测的合成数据生成，并通过实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 解决制造业目标检测中合成数据生成的挑战，探索从模拟到现实的可行性。

Method: 提出包含对象特征、背景、光照、相机设置和后处理的综合数据生成管道，并引入SIP15-OD数据集进行测试。

Result: 使用合成数据训练的Yolov8模型在公开数据集上表现优异，mAP@50得分高达96.4%。

Conclusion: 提出的域随机化方法有效，能够覆盖接近真实数据的分布。
...
### [122] [APTOS-2024 challenge report: Generation of synthetic 3D OCT images from fundus photographs](https://arxiv.org/abs/2506.07542)
...
TL;DR: APTOS-2024挑战赛探索了从2D眼底图像生成3D OCT图像的可行性，通过创新方法和基准数据集推动了生成模型的发展。


<details>
  <summary>Details</summary>
Motivation: 解决OCT设备成本高和操作复杂的问题，利用2D眼底图像的便捷性提升眼科医疗的可及性。

Method: 挑战赛框架包括基准数据集、评估方法（基于图像和视频的距离指标），并分析了42个初步提交和9个决赛方案。

Result: 342个团队参与，领先方法结合了数据预处理、预训练、视觉基础模型和架构改进。

Conclusion: APTOS-2024挑战赛首次验证了从眼底图像生成3D OCT的可行性，为资源匮乏地区的眼科医疗提供了潜在解决方案。
...
### [123] [Synthesize Privacy-Preserving High-Resolution Images via Private Textual Intermediaries](https://arxiv.org/abs/2506.07555)
...
<details>
  <summary>Details</summary>
Motivation: 现有差分隐私图像生成方法难以生成高分辨率且忠实于原始数据的图像。

Method: SPTI将图像转换为文本描述，利用差分隐私文本生成方法生成隐私保护的文本，再通过文本到图像模型重建图像。

Result: 在LSUN Bedroom和MM CelebA HQ数据集上，SPTI的FID显著优于现有方法。

Conclusion: SPTI为生成高分辨率差分隐私图像提供了高效且兼容的框架，扩展了私有视觉数据的访问。
...
### [124] [Cross-channel Perception Learning for H&E-to-IHC Virtual Staining](https://arxiv.org/abs/2506.07559)
*Hao Yang,JianYu Wu,Run Fang,Xuelian Zhao,Yuan Ji,Zhiyu Chen,Guibin He,Junceng Guo,Yang Liu,Xinhua Zeng*

Main category: cs.CV

TL;DR: 论文提出了一种新的跨通道感知学习策略（CCPL），用于解决H&E到IHC染色中细胞核与细胞膜跨通道相关性被忽视的问题，通过双通道特征提取和特征蒸馏损失提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有H&E到IHC染色研究常忽略细胞核与细胞膜的跨通道相关性，限制了病理图像分析的准确性。

Method: CCPL将HER2免疫组化染色分解为Hematoxylin和DAB染色通道，利用Gigapath的Tile Encoder提取双通道特征并计算跨通道相关性，同时通过特征蒸馏损失和光学密度统计确保染色一致性。

Result: 实验结果表明，CCPL在PSNR、SSIM、PCC和FID等定量指标及病理学家评估中表现优异，能生成高质量的虚拟染色图像。

Conclusion: CCPL有效保留了病理特征，为多媒体医疗数据支持的自动化病理诊断提供了可靠工具。
...
### [125] [OpenDance: Multimodal Controllable 3D Dance Generation Using Large-scale Internet Data](https://arxiv.org/abs/2506.07565)
*Jinlu Zhang,Zixi Kang,Yizhou Wang*

Main category: cs.CV

TL;DR: 论文提出了OpenDance5D数据集和OpenDanceNet框架，用于解决音乐驱动舞蹈生成的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有研究因缺乏细粒度多模态数据和灵活的多条件生成能力，限制了生成的可控性和多样性。

Method: 构建OpenDance5D数据集（包含14种舞蹈风格的101小时多模态数据），并提出OpenDanceNet框架，基于掩码建模实现可控舞蹈生成。

Result: OpenDanceNet在实验中表现出高保真度和灵活的可控性。

Conclusion: OpenDance5D和OpenDanceNet为音乐驱动舞蹈生成提供了有效的解决方案。
...
### [126] [Towards the Influence of Text Quantity on Writer Retrieval](https://arxiv.org/abs/2506.07566)
*Marco Peer,Robert Sablatnig,Florian Kleber*

Main category: cs.CV

TL;DR: 论文研究了基于手写相似性的作者检索任务，探讨了文本量对检索性能的影响，发现少量文本下深度学习方法的优势。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注页面级检索，而本文旨在探索行级和词级检索中文本量对性能的影响。

Method: 评估了三种先进作者检索系统（包括手工和深度学习方法），在不同文本量下的表现。

Result: 实验表明，仅用一行文本时性能下降20-30%，但四行文本时仍能达到全页性能的90%以上；深度学习方法在低文本场景下表现更优。

Conclusion: 少量文本下深度学习方法优于手工特征，文本相关检索在低文本场景中仍能保持较高性能。
...
### [127] [LLM-driven Indoor Scene Layout Generation via Scaled Human-aligned Data Synthesis and Multi-Stage Preference Optimization](https://arxiv.org/abs/2506.07570)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在空间不一致、计算成本高或泛化能力不足的问题，需要一种更高效的室内布局生成方案。

Method: 通过'GPT synthesize, Human inspect'流程构建3D-SynthPlace数据集，并采用两阶段训练（SFT和DPO）优化LLM模型OptiScene。

Result: OptiScene在布局质量和生成成功率上优于传统方法，并在交互任务中表现优异。
...
### [128] [Learning Speaker-Invariant Visual Features for Lipreading](https://arxiv.org/abs/2506.07572)
*Yu Li,Feng Xue,Shujie Li,Jinrui Zhang,Shuang Yang,Dan Guo,Richang Hong*

Main category: cs.CV

TL;DR: SIFLip是一种新的唇读框架，通过解耦说话人特定特征提升模型泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有唇读方法提取的视觉特征包含说话人特定属性，导致虚假相关性，影响准确性和泛化能力。

Method: SIFLip采用隐式解耦和显式解耦模块，利用文本嵌入和说话人识别子任务分离说话人特征。

Result: 实验表明SIFLip在多个公开数据集上显著优于现有方法。

Conclusion: SIFLip通过解耦说话人特征有效提升了唇读模型的泛化性能。
...
### [129] [Uncertainty-o: One Model-agnostic Framework for Unveiling Uncertainty in Large Multimodal Models](https://arxiv.org/abs/2506.07575)
*Ruiyang Zhang,Hu Zhang,Hao Fei,Zhedong Zheng*

Main category: cs.CV

TL;DR: 论文提出Uncertainty-o框架，用于评估和量化大型多模态模型（LMMs）的不确定性，并通过实验验证其在多种任务中的有效性。


<details>
  <summary>Details</summary>
Motivation: 尽管LMMs被认为比纯语言模型更鲁棒，但其不确定性评估仍存在三个关键问题：统一评估方法、如何提示LMMs展示不确定性，以及如何量化不确定性以支持下游任务。
...
### [130] [Super Encoding Network: Recursive Association of Multi-Modal Encoders for Video Understanding](https://arxiv.org/abs/2506.07576)
*Boyu Chen,Siran Chen,Kunchang Li,Qinglin Xu,Yu Qiao,Yali Wang*

Main category: cs.CV

TL;DR: 提出了一种统一的超编码网络（SEN），通过递归关联多模态编码器，增强视频理解任务中的多模态交互。


<details>
  <summary>Details</summary>
Motivation: 当前多模态基础模型仅通过对比学习对齐不同模态的编码器，缺乏更深层次的多模态交互，而这对理解复杂视频场景至关重要。

Method: 将预训练的编码器视为“超级神经元”，设计递归关联（RA）块，以递归方式融合多模态信息。

Result: 在跟踪、识别、聊天和编辑等四项代表性视频任务中表现显著提升，例如像素级跟踪的Jaccard指数提高2.7%，文本对齐提升6.4%。
...
### [131] [Explore the vulnerability of black-box models via diffusion models](https://arxiv.org/abs/2506.07590)
*Jiacheng Shi,Yanfu Zhang,Huajie Shao,Ashley Gao*

Main category: cs.CV

TL;DR: 研究发现扩散模型API可能被用于生成合成图像，进而训练替代模型，实现模型提取和对抗攻击，且效果显著。
...
Conclusion: 扩散模型API存在安全威胁，需加强防护措施。
...
### [132] [SceneRAG: Scene-level Retrieval-Augmented Generation for Video Understanding](https://arxiv.org/abs/2506.07600)
*Nianbo Zeng,Haowen Hou,Fei Richard Yu,Si Shi,Ying Tiffany He*

Main category: cs.CV

TL;DR: SceneRAG是一个基于大语言模型的框架，通过处理ASR转录和时间元数据将视频分割为叙事一致的场景，并结合视觉和文本模态信息构建知识图谱，显著提升了长视频理解任务的性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于固定长度分块的RAG方法破坏了视频的上下文连续性，无法捕捉真实场景边界，因此需要一种更自然的方法来组织长视频内容。

Method: SceneRAG利用大语言模型分割视频为叙事一致的场景，通过轻量级启发式和迭代校正优化边界，并结合多模态信息构建动态知识图谱。

Result: 在LongerVideos基准测试中，SceneRAG以72.5%的胜率显著优于现有基线方法。
...
### [133] [SurgBench: A Unified Large-Scale Benchmark for Surgical Video Analysis](https://arxiv.org/abs/2506.07603)
...
<details>
  <summary>Details</summary>
Motivation: 手术视频理解对自动化术中决策、技能评估和术后质量改进至关重要，但大规模多样化数据集的缺乏阻碍了进展。

Method: 引入SurgBench框架，包含53百万帧的预训练数据集和覆盖72个任务的评估基准，用于测试模型性能。

Result: 实验表明，现有视频基础模型在多样化任务中泛化能力不足，而基于SurgBench-P的预训练显著提升了性能。
...
### [134] [DragNeXt: Rethinking Drag-Based Image Editing](https://arxiv.org/abs/2506.07611)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于拖拽的图像编辑方法存在模糊性和繁琐性问题，无法高质量完成编辑任务。

Method: 将拖拽编辑重新定义为变形、旋转和平移，提出Latent Region Optimization (LRO)框架，采用Progressive Backward Self-Intervention (PBSI)方法。
...
### [135] [Scaling Human Activity Recognition: A Comparative Evaluation of Synthetic Data Generation and Augmentation Techniques](https://arxiv.org/abs/2506.07612)
...
Method: 构建大规模虚拟IMU数据集，对比视频和语言两种生成方法与传统数据增强技术。
...
### [136] [Event-Priori-Based Vision-Language Model for Efficient Visual Understanding](https://arxiv.org/abs/2506.07627)
...
Method: EP-VLM通过事件数据引导RGB输入的稀疏化，并采用位置保留标记化策略处理稀疏输入。

Result: EP-VLM在Qwen2-VL-2B上实现50%的FLOPs节省，同时在RealWorldQA数据集上保持98%的原始精度。

Conclusion: 事件视觉先验可显著提升VLM效率，为边缘设备上的可持续视觉理解提供可能。
...
### [137] [HuSc3D: Human Sculpture dataset for 3D object reconstruction](https://arxiv.org/abs/2506.07628)
*Weronika Smolak-Dyżewska,Dawid Malarz,Grzegorz Wilczyński,Rafał Tobiasz,Joanna Waczyńska,Piotr Borycki,Przemysław Spurek*

Main category: cs.CV

TL;DR: HuSc3D数据集填补了3D重建领域在真实场景复杂性评估上的空白，通过白色雕塑和动态背景等挑战性数据，突显了现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有3D重建数据集多基于理想化或精心捕获的场景，未能反映真实世界中的动态背景和采集差异（如白平衡问题）。

Method: 提出HuSc3D数据集，包含六组高细节白色雕塑，具有复杂穿孔和低纹理变化，且图像数量差异显著。

Result: 评估显示HuSc3D能有效区分模型性能，揭示其对几何细节、颜色模糊和数据量变化的敏感性。

Conclusion: HuSc3D为3D重建模型在真实挑战下的评估提供了独特基准，弥补了传统数据集的不足。
...
### [138] [HieraEdgeNet: A Multi-Scale Edge-Enhanced Framework for Automated Pollen Recognition](https://arxiv.org/abs/2506.07637)
*Yuchong Long,Wen Sun,Ningxiao Sun,Wenxiao Wang,Chao Li,Shan Yin*

Main category: cs.CV

TL;DR: HieraEdgeNet是一种多尺度边缘增强框架，显著提升了微小目标（如花粉）的自动识别精度。
...
Result: 在120类花粉数据集上，mAP@.5达到0.9501，优于现有模型。
...
### [139] [Synthetic Visual Genome](https://arxiv.org/abs/2506.07643)
...
TL;DR: ROBIN是一种基于密集标注关系的多模态语言模型，通过合成高质量场景图数据集SVG和自蒸馏框架SG-EDIT，显著提升了视觉关系推理能力。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态语言模型在视觉理解方面取得进展，但精确推理和生成视觉关系仍具挑战性。

Method: 通过合成数据集SVG和自蒸馏框架SG-EDIT训练ROBIN模型，优化场景图生成。

Result: ROBIN-3B模型在关系理解基准测试中表现优异，甚至超越更大参数模型，达到88.9的指代表达理解分数。

Conclusion: 高质量场景图数据训练对提升视觉推理任务性能至关重要。
...
### [140] [FMaMIL: Frequency-Driven Mamba Multi-Instance Learning for Weakly Supervised Lesion Segmentation in Medical Images](https://arxiv.org/abs/2506.07652)
...
Method: 两阶段框架：1）Mamba编码器捕获长程依赖，频率域编码模块增强空间敏感性；2）通过CAM引导的软标签监督和自校正机制优化伪标签。

Result: 在公开和私有数据集上优于现有弱监督方法，无需像素级标注。

Conclusion: FMaMIL在数字病理学中具有潜力，验证了其有效性。
...
### [141] [ProSplat: Improved Feed-Forward 3D Gaussian Splatting for Wide-Baseline Sparse Views](https://arxiv.org/abs/2506.07670)
*Xiaohan Lu,Jiaye Fu,Jiaqi Zhang,Zetian Song,Chuanmin Jia,Siwei Ma*

Main category: cs.CV

TL;DR: ProSplat是一个两阶段前馈框架，用于提升宽基线条件下的3D高斯泼溅（3DGS）性能，通过扩散模型和几何一致性优化实现高保真渲染。


<details>
  <summary>Details</summary>
Motivation: 解决3DGS在宽基线场景下因纹理细节不足和几何不一致导致的性能下降问题。

Method: 第一阶段生成3D高斯基元，第二阶段通过扩散模型（MORI和DWEA优化）增强渲染视图，并采用分治训练策略。

Result: 在RealEstate10K和DL3DV-10K数据集上，PSNR平均提升1 dB。

Conclusion: ProSplat在宽基线条件下显著提升了3DGS的渲染质量。
...
### [142] [OpenSplat3D: Open-Vocabulary 3D Instance Segmentation using Gaussian Splatting](https://arxiv.org/abs/2506.07697)
*Jens Piekenbrinck,Christian Schmidt,Alexander Hermans,Narunas Vaskevicius,Timm Linder,Bastian Leibe*

Main category: cs.CV

TL;DR: 论文提出OpenSplat3D方法，扩展3D高斯泼溅（3DGS）的能力，实现无需手动标注的开放词汇3D实例分割。


<details>
  <summary>Details</summary>
Motivation: 3DGS虽在神经场景重建中表现优异，但缺乏语义理解能力。本文旨在通过结合特征泼溅技术和语言嵌入，实现开放词汇的3D实例分割。

Method: 利用特征泼溅技术将语义信息与高斯分布关联，结合Segment Anything Model的实例掩码和对比损失，并通过视觉语言模型的语言嵌入实现文本驱动的实例识别。

Result: 在LERF-mask、LERF-OVS和ScanNet++验证集上展示了方法的有效性。

Conclusion: OpenSplat3D成功实现了无需手动标注的开放词汇3D实例分割，为场景理解提供了灵活且高效的解决方案。
...
### [143] [NOVA3D: Normal Aligned Video Diffusion Model for Single Image to 3D Generation](https://arxiv.org/abs/2506.07698)
...
<details>
  <summary>Details</summary>
Motivation: 现有的方法在从预训练图像扩散模型中提取3D对象时，由于3D先验不足，导致多视角一致性不佳。

Method: NOVA3D利用预训练视频扩散模型的3D先验，结合几何信息进行多视角视频微调，并提出了Geometry-Temporal Alignment (GTA)注意力机制和去冲突几何融合算法。
...
### [144] [Adaptive Blind Super-Resolution Network for Spatial-Specific and Spatial-Agnostic Degradations](https://arxiv.org/abs/2506.07705)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法忽视了不同退化类型的多样性，采用统一网络模型处理多种退化，导致性能受限。

Method: 分类退化类型为空间无关和空间相关，并设计动态滤波网络，结合全局和局部分支分别处理两类退化。
...
### [145] [Consistent Video Editing as Flow-Driven Image-to-Video Generation](https://arxiv.org/abs/2506.07713)
*Ge Wang,Songlin Fan,Hangxu Liu,Quanjian Song,Hewei Wang,Jinfeng Xu*

Main category: cs.CV

TL;DR: FlowV2V提出了一种基于光流的视频编辑方法，通过分解任务为第一帧编辑和条件I2V生成，解决了复杂运动建模问题，显著提升了时间一致性和样本质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法难以处理复杂运动模式（如非刚性物体运动），因此需要一种更有效的方法来建模复杂运动并保持时间一致性。

Method: FlowV2V将视频编辑任务分解为第一帧编辑和条件I2V生成，利用伪光流序列对齐变形形状，确保编辑一致性。

Result: 在DAVIS-EDIT数据集上，FlowV2V在DOVER和warping error指标上分别提升了13.67%和50.66%，优于现有方法。

Conclusion: FlowV2V通过光流建模复杂运动，显著提升了视频编辑的时间一致性和质量，为复杂任务提供了新思路。
...
### [146] [ReverB-SNN: Reversing Bit of the Weight and Activation for Spiking Neural Networks](https://arxiv.org/abs/2506.07720)
*Yufei Guo,Yuhan Zhang,Zhou Jie,Xiaode Liu,Xin Tong,Yuanpei Chen,Weihang Peng,Zhe Ma*

Main category: cs.CV

TL;DR: ReverB-SNN通过反转权重和激活的比特位，结合实值激活和二进制权重，提升SNN的信息容量和准确性，同时保持高效性。


<details>
  <summary>Details</summary>
Motivation: 解决SNN中二进制激活映射信息不足导致的准确率下降问题。

Method: 采用实值激活和二进制权重，引入可训练因子自适应学习权重幅度，推理时通过重参数化恢复标准形式。

Result: 在多种网络架构和数据集上表现优于现有方法。

Conclusion: ReverB-SNN在保持高效性的同时显著提升了SNN的准确性和信息容量。
...
### [147] [ETA: Efficiency through Thinking Ahead, A Dual Approach to Self-Driving with Large Models](https://arxiv.org/abs/2506.07725)
*Shadi Hamdan,Chonghao Sima,Zetong Yang,Hongyang Li,Fatma Güney*

Main category: cs.CV

TL;DR: ETA系统通过异步设计和批量推理，解决了自动驾驶系统中大模型推理速度慢的问题，提升了性能并保持实时性。


<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶系统中大模型推理速度慢与实时性需求的矛盾。

Method: 提出ETA系统，通过异步设计将计算任务提前，结合小模型实时提取特征和大模型批量推理，并通过动作掩码机制整合特征。

Result: 在Bench2Drive CARLA Leaderboard-v2基准测试中，ETA将驾驶分数提升8%至69.53，推理速度保持在50毫秒。

Conclusion: ETA通过异步设计和特征整合，实现了大模型的高效实时推理，显著提升了自动驾驶系统的性能。
...
### [148] [SpikeSMOKE: Spiking Neural Networks for Monocular 3D Object Detection with Cross-Scale Gated Coding](https://arxiv.org/abs/2506.07737)
*Xuemei Chen,Huamin Wang,Hangchi Shen,Shukai Duan,Shiping Wen,Tingwen Huang*

Main category: cs.CV

TL;DR: 提出了一种基于脉冲神经网络（SNN）的低功耗单目3D目标检测方法SpikeSMOKE，通过跨尺度门控编码机制（CSGC）提升特征表达能力，并设计了轻量级残差块以减少计算量。


<details>
  <summary>Details</summary>
Motivation: 解决3D目标检测中能耗高的问题，利用SNN的低功耗特性，同时克服其离散信号导致的信息损失问题。

Method: 提出CSGC机制结合跨尺度融合和门控滤波，增强特征表达；设计轻量级残差块以减少计算量。

Result: 在KITTI数据集上，SpikeSMOKE性能显著提升（Easy +2.82, Moderate +3.2, Hard +3.17），能耗降低72.2%，轻量版参数和计算量分别减少3倍和10倍。

Conclusion: SpikeSMOKE为低功耗3D目标检测提供了有效解决方案，性能与能耗平衡良好。
...
### [149] [AssetDropper: Asset Extraction via Diffusion Models with Reward-Driven Optimization](https://arxiv.org/abs/2506.07738)
...
<details>
  <summary>Details</summary>
Motivation: 设计师需要标准化资产库，但现有生成模型未能显著提升这一领域。开放世界场景提供了丰富素材，但高效提取高质量资产仍具挑战。

Method: 引入AssetDropper框架，通过预训练的奖励模型实现闭环反馈，提取前视图并处理复杂场景（如透视变形和遮挡）。

Result: AssetDropper在资产提取任务中达到最先进水平，并通过合成和真实数据集验证了其有效性。

Conclusion: AssetDropper为设计师提供了开放世界资产调色板，并通过奖励驱动优化显著提升了资产提取的精确性。
...
### [150] [ArchiLense: A Framework for Quantitative Analysis of Architectural Styles Based on Vision Large Language Models](https://arxiv.org/abs/2506.07739)
*Jing Zhong,Jun Yin,Peilin Li,Pengyu Zeng,Miao Zhang,Shuai Lu,Ran Luo*

Main category: cs.CV

TL;DR: 论文提出了一种基于视觉语言模型的建筑风格分析框架ArchiLense，通过构建专业数据集ArchDiffBench，实现了自动识别和分类建筑风格，解决了传统方法的主观性和区域偏见问题。
...
Method: 1. 构建ArchDiffBench数据集，包含1,765张高质量建筑图像及风格标注。2. 提出ArchiLense框架，结合视觉语言模型和深度学习算法，实现自动识别、比较和分类建筑风格。
...
### [151] [Flow-Anything: Learning Real-World Optical Flow Estimation from Large-Scale Single-view Images](https://arxiv.org/abs/2506.07740)
*Yingping Liang,Ying Fu,Yutao Hu,Wenqi Shao,Jiaming Liu,Debing Zhang*

Main category: cs.CV

TL;DR: 提出Flow-Anything框架，通过单视角图像生成大规模真实光流数据，解决合成数据域差距问题。


<details>
  <summary>Details</summary>
Motivation: 解决光流估计中合成数据训练导致的真实世界应用性能限制。

Method: 1. 单视角图像转3D表示；2. 开发对象无关体积渲染和深度感知修复模块。

Result: 生成FA-Flow数据集，性能超越现有无监督和监督方法。

Conclusion: Flow-Anything为光流估计和下游视频任务提供高效基础模型。
...
### [152] [Difference Inversion: Interpolate and Isolate the Difference with Token Consistency for Image Analogy Generation](https://arxiv.org/abs/2506.07750)
*Hyunsoo Kim,Donghyun Kim,Suhyun Kim*

Main category: cs.CV

TL;DR: 论文提出了一种名为Difference Inversion的方法，通过提取A和A'之间的差异并应用到B上，生成符合A:A'::B:B'关系的图像B'，解决了现有方法对特定模型的依赖问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如视觉上下文学习或视觉指令）通常局限于特定模型（如InstructPix2Pix或Inpainting模型），可能导致偏见或编辑能力受限。本文旨在提出一种不依赖特定模型的通用方法。

Method: 提出Difference Inversion方法，通过1) Delta插值提取A和A'的差异，2) Token一致性损失和3) Token嵌入的零初始化确保训练准确性，并将差异与B的提示结合，生成B'。

Result: 实验表明，Difference Inversion在定量和定性上均优于现有基线，能够以模型无关的方式生成更可行的B'。

Conclusion: Difference Inversion是一种有效的模型无关方法，能够生成符合A:A'::B:B'关系的图像B'，解决了现有方法的局限性。
...
### [153] [Trend-Aware Fashion Recommendation with Visual Segmentation and Semantic Similarity](https://arxiv.org/abs/2506.07773)
...
Result: 在DeepFashion数据集上表现优异，ResNet-50达到64.95%的类别相似度，且流行度误差最低。

Conclusion: 该方法为个性化时尚推荐提供了可扩展的框架，有效结合视觉与流行度信息。
...
### [154] [Language-Vision Planner and Executor for Text-to-Visual Reasoning](https://arxiv.org/abs/2506.07778)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型在泛化性能上表现不足，VLAgent旨在通过规划与执行的自动化流程解决这一问题。

Method: VLAgent分两阶段：任务规划阶段通过上下文学习微调LLM生成步骤计划；执行阶段通过神经符号模块逐步优化推理结果。

Result: 在四个视觉推理基准测试中，VLAgent性能显著优于现有方法。

Conclusion: VLAgent通过优化模块（如SS-Parser和Plan Repairer）提升了多模态推理的准确性和泛化能力。
...
### [155] [Design and Evaluation of Deep Learning-Based Dual-Spectrum Image Fusion Methods](https://arxiv.org/abs/2506.07779)
*Beining Xu,Junxian Li*

Main category: cs.CV

TL;DR: 论文提出了一种高质量的双光谱数据集和综合评估框架，用于可见光和红外图像融合，并通过实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前可见光和红外图像融合方法缺乏标准化评估和高质量数据集，限制了该领域的发展。

Method: 构建了包含1,369对对齐图像的双光谱数据集，并提出结合融合速度、通用指标和下游任务性能的综合评估框架。

Result: 实验表明，针对下游任务优化的融合模型在目标检测中表现更优，尤其是低光和遮挡场景。

Conclusion: 论文贡献包括高质量数据集、任务感知评估框架和对现有融合方法的全面分析，为未来研究提供了参考。
...
### [156] [Re-ranking Reasoning Context with Tree Search Makes Large Vision-Language Models Stronger](https://arxiv.org/abs/2506.07785)
*Qi Yang,Chenghao Zhang,Lubin Fan,Kun Ding,Jieping Ye,Shiming Xiang*

Main category: cs.CV

TL;DR: 本文提出了一种名为RCTS的多模态RAG框架，通过构建推理上下文丰富的知识库和树搜索重排序方法，解决了现有LVLMs在VQA任务中知识稀缺和响应不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的大型视觉语言模型（LVLMs）在多模态检索增强生成（RAG）中仍面临知识推理示例稀缺和检索知识响应不一致的挑战。

Method: 提出RCTS框架，包括构建推理上下文丰富的知识库和基于蒙特卡洛树搜索与启发式奖励（MCTS-HR）的重排序方法。

Result: 在多个VQA数据集上实现了最先进的性能，显著优于上下文学习（ICL）和普通RAG方法。
...
### [157] [Image Reconstruction as a Tool for Feature Analysis](https://arxiv.org/abs/2506.07803)
*Eduard Allakhverdov,Dmitrii Tarasov,Elizaveta Goncharova,Andrey Kuznetsov*

Main category: cs.CV

TL;DR: 提出了一种通过图像重建解释视觉特征的新方法，比较了SigLIP和SigLIP2模型，发现基于图像任务预训练的编码器保留更多图像信息。


<details>
  <summary>Details</summary>
Motivation: 理解视觉编码器内部如何表示特征。
...
### [158] [Incorporating Uncertainty-Guided and Top-k Codebook Matching for Real-World Blind Image Super-Resolution](https://arxiv.org/abs/2506.07809)
...
Conclusion: UGTSR框架有效提升了超分辨率图像的质量，代码将公开发布。
...
### [159] [Looking Beyond Visible Cues: Implicit Video Question Answering via Dual-Clue Reasoning](https://arxiv.org/abs/2506.07811)
...
<details>
  <summary>Details</summary>
Motivation: 现有VideoQA方法依赖显式视觉证据，但在涉及符号意义或深层意图时性能下降，因此需要解决隐式视觉证据的问题。

Method: 提出了IRM框架，包含AIM（生成线索候选并推理关系）和VEM（增强视觉表示），通过双流建模处理隐式问题。
...
### [160] [Self-Cascaded Diffusion Models for Arbitrary-Scale Image Super-Resolution](https://arxiv.org/abs/2506.07813)
*Junseo Bang,Joonhee Lee,Kyeonghyun Lee,Haechang Lee,Dong Un Kang,Se Young Chun*

Main category: cs.CV

TL;DR: CasArbi是一种新型的自级联扩散框架，用于任意尺度图像超分辨率，通过逐步提升分辨率实现灵活上采样。
...
Conclusion: CasArbi通过渐进式上采样和扩散模型的有效结合，实现了灵活且高性能的任意尺度超分辨率。
...
### [161] [M2Restore: Mixture-of-Experts-based Mamba-CNN Fusion Framework for All-in-One Image Restoration](https://arxiv.org/abs/2506.07814)
*Yongzhen Wang,Yongjun Li,Zhuoran Zheng,Xiao-Ping Zhang,Mingqiang Wei*

Main category: cs.CV

TL;DR: M2Restore提出了一种基于Mixture-of-Experts的Mamba-CNN融合框架，用于高效且鲁棒的一体化图像恢复，解决了现有方法在动态变化退化场景中泛化能力不足及局部细节与全局依赖平衡不佳的问题。
...
Method: 1. 使用CLIP引导的MoE门控机制融合任务条件提示与CLIP语义先验；2. 设计双流架构结合CNN局部表征与Mamba长程建模；3. 引入边缘感知动态门控机制自适应平衡全局与局部恢复。

Result: 在多个图像恢复基准测试中，M2Restore在视觉质量和定量性能上均表现出优越性。

Conclusion: M2Restore通过创新的架构设计和门控机制，显著提升了图像恢复的泛化能力和细节恢复效果。
...
### [162] [R3D2: Realistic 3D Asset Insertion via Diffusion for Autonomous Driving Simulation](https://arxiv.org/abs/2506.07826)
...
Method: R3D2通过训练一个一步扩散模型，利用3D高斯喷绘生成的资产数据集，实现实时生成逼真的渲染效果（如阴影和一致光照）。
...
### [163] [Diffusion models under low-noise regime](https://arxiv.org/abs/2506.07841)
*Elizabeth Pavlova,Xue-Xin Wei*

Main category: cs.CV

TL;DR: 研究了扩散模型在低噪声条件下的行为，揭示了训练数据大小、数据几何和模型目标对去噪轨迹的影响。


<details>
  <summary>Details</summary>
Motivation: 填补扩散模型在小噪声条件下行为的理解空白，提升模型在实际应用中的可靠性和可解释性。

Method: 使用CelebA子集和高斯混合基准，系统分析低噪声扩散动态下的模型行为。

Result: 发现模型在数据流形附近的行为差异，量化了训练集大小、数据几何和模型目标对去噪轨迹的影响。

Conclusion: 为扩散模型在实际小扰动场景中的可靠性提供了新见解。
...
### [164] [F2Net: A Frequency-Fused Network for Ultra-High Resolution Remote Sensing Segmentation](https://arxiv.org/abs/2506.07847)
...
<details>
  <summary>Details</summary>
Motivation: 超高清遥感图像的语义分割在环境监测和城市规划中至关重要，但传统方法存在计算和优化挑战。

Method: F2Net将图像分解为高频和低频组件，分别处理细节和全局上下文，并通过混合频率融合模块和两个新目标（交叉频率对齐损失和平衡损失）优化训练。

Result: 在DeepGlobe和Inria Aerial基准测试中，F2Net分别达到80.22和83.39的mIoU，表现最优。

Conclusion: F2Net通过频率分解和融合，实现了高效且稳定的超高清图像语义分割。
...
### [165] [PolyVivid: Vivid Multi-Subject Video Generation with Cross-Modal Interaction and Enhancement](https://arxiv.org/abs/2506.07848)
*Teng Hu,Zhentao Yu,Zhengguang Zhou,Jiangning Zhang,Yuan Zhou,Qinglin Lu,Ran Yi*

Main category: cs.CV

TL;DR: PolyVivid是一个多主体视频定制框架，通过文本-图像融合模块和3D-RoPE增强模块实现身份一致性和交互性，优于现有方法。
...
Conclusion: PolyVivid在多主体视频生成中实现了灵活且身份一致的控制，性能优于现有方法。
...
### [166] [SAM2Auto: Auto Annotation Using FLASH](https://arxiv.org/abs/2506.07850)
*Arash Rocky,Q. M. Jonathan Wu*

Main category: cs.CV

TL;DR: SAM2Auto是一种全自动视频数据集标注管道，无需人工干预或特定数据集训练，显著减少标注时间和成本。
...
Method: 结合SMART-OD（自动掩码生成与开放世界目标检测）和FLASH（实时视频实例分割与对象一致性跟踪）。

Result: 实验验证表明，SAM2Auto的标注精度与人工相当，同时大幅提升效率。
...
### [167] [LogoSP: Local-global Grouping of Superpoints for Unsupervised Semantic Segmentation of 3D Point Clouds](https://arxiv.org/abs/2506.07857)
*Zihui Zhang,Weisheng Dai,Hongtao Wen,Bo Yang*

Main category: cs.CV

TL;DR: 本文提出LogoSP方法，通过结合局部和全局点特征进行无监督3D语义分割，利用频域中的全局模式生成高精度伪标签，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有无监督3D语义分割方法仅依赖局部特征，缺乏对全局语义先验的挖掘。

Method: LogoSP通过频域中的全局模式分组超点，生成语义伪标签训练分割网络。

Result: 在两个室内和一个室外数据集上，LogoSP大幅超越现有无监督方法，达到最优性能。

Conclusion: LogoSP证明了全局模式能有效表示无监督学习中的3D语义信息。
...
### [168] [Egocentric Event-Based Vision for Ping Pong Ball Trajectory Prediction](https://arxiv.org/abs/2506.07860)
*Ivan Alberico,Marco Cannici,Giovanni Cioffi,Davide Scaramuzza*

Main category: cs.CV

TL;DR: 提出了一种基于事件相机的实时乒乓球轨迹预测系统，利用高时间分辨率解决传统相机的延迟和运动模糊问题，并通过注视数据优化计算资源分配。


<details>
  <summary>Details</summary>
Motivation: 传统相机在高速度乒乓球运动中存在延迟和运动模糊问题，事件相机的高时间分辨率能提供更准确的轨迹预测。

Method: 收集包含3D真实轨迹的乒乓球比赛数据集，结合Meta Project Aria眼镜的传感器数据和事件流，利用注视数据实现注视区域的事件处理，优化检测性能并降低计算延迟。

Result: 系统在最坏情况下的总延迟为4.5毫秒，显著优于基于帧的30 FPS系统（66毫秒），并实现了10.81倍的轨迹计算效率提升。

Conclusion: 这是首个基于事件相机和注视数据的乒乓球轨迹预测方法，展示了高效率和低延迟的优势。
...
### [169] [VIVAT: Virtuous Improving VAE Training through Artifact Mitigation](https://arxiv.org/abs/2506.07863)
*Lev Novitskiy,Viacheslav Vasilev,Maria Kovaleva,Vladimir Arkhipkin,Denis Dimitrov*

Main category: cs.CV

TL;DR: VIVAT通过简单调整KL-VAE训练中的损失权重、填充策略和引入空间条件归一化，显著减少常见伪影，提升重建和生成质量。


<details>
  <summary>Details</summary>
Motivation: KL-VAE训练中常见的伪影（如颜色偏移、网格图案等）影响重建和生成质量，需要一种无需大幅架构改动的方法来解决。

Method: 提出VIVAT方法，通过调整损失权重、优化填充策略和引入空间条件归一化，系统性地减少五种常见伪影。

Result: 在多个基准测试中，VIVAT在图像重建指标（PSNR和SSIM）上达到SOTA，并提升文本到图像生成的CLIP分数。

Conclusion: VIVAT为优化KL-VAE训练提供了实用解决方案，同时保持了框架的简洁性。
...
### [170] [FreeGave: 3D Physics Learning from Dynamic Videos by Gaussian Velocity](https://arxiv.org/abs/2506.07865)
...
Method: 引入物理编码和发散自由模块，估计每高斯速度场，避免低效的PINN损失。
...
### [171] [Spatio-Temporal State Space Model For Efficient Event-Based Optical Flow](https://arxiv.org/abs/2506.07878)
*Muhammad Ahmed Humais,Xiaoqian Huang,Hussain Sajwani,Sajid Javed,Yahya Zweiri*

Main category: cs.CV

TL;DR: 提出了一种基于时空状态空间模型（STSSM）的高效事件相机运动估计算法，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 事件相机在低延迟运动估计中具有潜力，但现有深度学习方法计算效率不足，而异步事件方法又缺乏时空信息。

Method: 引入STSSM模块和新网络架构，利用状态空间模型高效捕捉事件数据的时空相关性。

Result: 在DSEC基准测试中，模型推理速度提升4.5倍，计算量减少8倍（相比TMA）或2倍（相比EV-FlowNet），性能竞争力强。

Conclusion: STSSM模块为事件相机运动估计提供了高效且高性能的解决方案。
...
### [172] [CrosswalkNet: An Optimized Deep Learning Framework for Pedestrian Crosswalk Detection in Aerial Images with High-Performance Computing](https://arxiv.org/abs/2506.07885)
...
<details>
  <summary>Details</summary>
Motivation: 随着航拍和卫星图像的普及，深度学习在交通资产管理、安全分析和城市规划中具有巨大潜力，但需要更精确和高效的检测方法。

Method: CrosswalkNet采用定向边界框（OBB）和改进的注意力机制、空间金字塔池化模块及余弦退火等技术，结合超过23,000个标注样本进行训练。

Result: 在麻省航拍图像上，模型精度达96.5%，召回率达93.3%，并在其他州数据集上无需微调即表现优异。
...
### [173] [EgoM2P: Egocentric Multimodal Multitask Pretraining](https://arxiv.org/abs/2506.07886)
*Gen Li,Yutong Chen,Yiqian Wu,Kaifeng Zhao,Marc Pollefeys,Siyu Tang*

Main category: cs.CV

TL;DR: 论文提出EgoM2P框架，通过高效时间标记器和掩码建模解决多模态自我中心视觉任务，支持多任务学习，性能优于专用模型。


<details>
  <summary>Details</summary>
Motivation: 自我中心视觉的多模态数据（如RGB视频、深度、相机位姿和视线）在增强现实、机器人和人机交互中至关重要，但数据异质性和缺失模态标签使其难以扩展。

Method: 引入高效时间标记器，提出EgoM2P掩码建模框架，学习时间感知多模态标记，支持多任务学习。

Result: EgoM2P在视线预测、相机跟踪和深度估计等任务中性能匹配或优于专用模型，且速度快一个数量级。

Conclusion: EgoM2P为自我中心视觉提供通用解决方案，将开源以推动研究。
...
### [174] [Video Unlearning via Low-Rank Refusal Vector](https://arxiv.org/abs/2506.07891)
...
TL;DR: 本文提出了一种针对视频扩散模型的去学习方法，仅需5对多模态提示对即可生成拒绝向量，用于消除模型中的有害概念，同时保持生成质量。
...
Method: 通过多模态提示对生成拒绝向量，采用低秩因子分解方法隔离目标概念，避免对其他语义的负面影响。

Result: 方法能有效消除多种有害内容（如裸露、暴力、版权等），且无需重新训练或原始数据。

Conclusion: 该方法通过直接嵌入拒绝方向到模型权重中，提高了对抗绕过尝试的鲁棒性，同时保持了视频生成质量。
...
### [175] [WeThink: Toward General-purpose Vision-Language Reasoning via Reinforcement Learning](https://arxiv.org/abs/2506.07905)
...
Result: 在14个多模态基准测试中显著提升性能，涵盖数学推理和通用任务。

Conclusion: 自动化数据管道和WeThink数据集有效提升多模态模型的通用推理能力。
...
### [176] [A Comparative Study of U-Net Architectures for Change Detection in Satellite Images](https://arxiv.org/abs/2506.07925)
*Yaxita Amin,Naimisha S Trivedi,Rashmi Bhattad*

Main category: cs.CV

TL;DR: 本文对34篇论文进行了综合分析，比较了18种U-Net变体在遥感变化检测中的应用潜力，并评估了各自的优缺点。
...
### [177] [Mimicking or Reasoning: Rethinking Multi-Modal In-Context Learning in Vision-Language Models](https://arxiv.org/abs/2506.07936)
*Chengyue Huang,Yuchen Zhu,Sichen Zhu,Jingyun Xiao,Moises Andrade,Shivang Chopra,Zsolt Kira*

Main category: cs.CV

TL;DR: 研究发现，当前的视觉语言模型（VLMs）在多模态上下文学习（MM-ICL）中依赖浅层启发式方法，而非真正的任务理解，且性能在分布偏移时下降。
...
Result: 实验表明，VLMs对演示级信息的利用有限，性能对演示数量、检索方法等因素不敏感。

Conclusion: 当前VLMs在多模态上下文学习中表现不足，需进一步改进以提升任务理解能力。
...
### [178] [Decoupling the Image Perception and Multimodal Reasoning for Reasoning Segmentation with Digital Twin Representations](https://arxiv.org/abs/2506.07943)
*Yizhen Li,Dell Zhang,Xuelong Li,Yiqing Shen*

Main category: cs.CV

TL;DR: DTwinSeger是一种新颖的多模态视觉-文本任务方法，通过数字孪生（DT）表示将感知与推理解耦，利用LLM进行推理，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法因图像标记化破坏空间关系的问题，提升多模态推理任务的性能。

Method: 将任务分为两阶段：首先生成保留空间关系的DT表示，然后利用LLM进行推理。

Result: 在两个图像RS基准和三个图像参考分割基准上达到最优性能。
...
### [179] [Creating a Historical Migration Dataset from Finnish Church Records, 1800-1920](https://arxiv.org/abs/2506.07960)
...
<details>
  <summary>Details</summary>
Motivation: 研究芬兰历史人口迁移模式，填补数字化档案研究的空白。

Method: 采用深度学习流程（布局分析、表格检测、单元格分类和手写识别）自动化提取数据。

Result: 生成包含600多万条目的结构化数据集，可用于研究迁移、城市化、家庭迁移及疾病传播。

Conclusion: 展示了如何将大量手写档案转化为结构化数据，支持历史与人口研究。
...
### [180] [SlideCoder: Layout-aware RAG-enhanced Hierarchical Slide Generation from Design](https://arxiv.org/abs/2506.07964)
...
<details>
  <summary>Details</summary>
Motivation: 手动制作幻灯片费时费力且需要专业知识，现有基于自然语言的LLM生成方法难以捕捉幻灯片设计的视觉和结构细节。

Method: 提出Slide2Code基准和SlideCoder框架，结合颜色梯度分割算法和分层检索增强生成方法，分解复杂任务并优化代码生成。

Result: SlideCoder在布局保真度、执行准确性和视觉一致性上表现优异，比现有基线方法提升40.5分。

Conclusion: SlideCoder是首个布局感知的检索增强框架，能高效生成可编辑幻灯片，并通过实验验证了其优越性。
...
### [181] [SpaCE-10: A Comprehensive Benchmark for Multimodal Large Language Models in Compositional Spatial Intelligence](https://arxiv.org/abs/2506.07966)
...
Method: 提出SpaCE-10基准，定义10种原子能力和8种组合能力，采用分层标注流程生成高质量QA对。

Result: 测试发现，最先进的MLLMs在空间智能上仍远落后于人类，计数能力不足是主要限制因素。

Conclusion: SpaCE-10为MLLMs的空间智能评估提供了新工具，揭示了现有模型的局限性。
...
### [182] [CyberV: Cybernetics for Test-time Scaling in Video Understanding](https://arxiv.org/abs/2506.07971)
*Jiahao Meng,Shuyang Sun,Yue Tan,Lu Qi,Yunhai Tong,Xiangtai Li,Longyin Wen*

Main category: cs.CV

TL;DR: 论文提出了一种名为CyberV的新框架，通过引入控制论循环（MLLM推理系统、传感器和控制器）来提升多模态大语言模型（MLLMs）对长或复杂视频的理解能力，无需重新训练。实验显示性能显著提升。
...
Method: 提出CyberV框架，通过控制论循环（传感器监控中间结果，控制器触发自校正和反馈）实现动态资源分配和自适应推理。

Result: CyberV显著提升了多个模型的性能（如Qwen2.5-VL-7B提升8.3%），甚至达到人类专家水平。

Conclusion: CyberV有效提升了MLLMs的动态视频理解能力，具有广泛适用性和鲁棒性。
...
### [183] [OneIG-Bench: Omni-dimensional Nuanced Evaluation for Image Generation](https://arxiv.org/abs/2506.07977)
*Jingjing Chang,Yixiao Fang,Peng Xing,Shuhan Wu,Wei Cheng,Rui Wang,Xianfang Zeng,Gang Yu,Hai-Bao Chen*

Main category: cs.CV

TL;DR: OneIG-Bench是一个全面的文本到图像（T2I）模型评估框架，涵盖多维度评估，包括提示-图像对齐、文本渲染精度、推理生成内容、风格化和多样性。


<details>
  <summary>Details</summary>
Motivation: 现有T2I模型的评估系统未能全面覆盖推理、文本渲染和风格等关键维度，限制了模型的深入分析。

Method: 通过设计OneIG-Bench框架，支持多维度细粒度评估，并允许用户灵活选择评估子集。

Result: OneIG-Bench提供了公开的代码库和数据集，支持可重复的评估研究和跨模型比较。

Conclusion: 该框架为T2I模型的全面评估提供了系统化解决方案，有助于识别模型性能的瓶颈和优势。
...
### [184] [Real-time Localization of a Soccer Ball from a Single Camera](https://arxiv.org/abs/2506.07981)
...
### [185] [CXR-LT 2024: A MICCAI challenge on long-tailed, multi-label, and zero-shot disease classification from chest X-ray](https://arxiv.org/abs/2506.07984)
...
TL;DR: CXR-LT 2024扩展了数据集并引入零样本学习，以提升肺部疾病分类性能，涵盖三项任务和多种先进方法。


<details>
  <summary>Details</summary>
Motivation: 解决开放长尾肺部疾病分类的挑战，提升现有技术的可测量性，并扩展数据集以更好地反映真实临床场景。

Method: 提供高质量基准数据，引入三项任务（长尾分类、黄金标准子集分类、零样本泛化），并整合多模态模型、生成方法和零样本学习策略。

Result: 数据集扩展至377,110张胸片和45种疾病标签，包括19种新罕见病，并提供了解决噪声标签和未见疾病的方法。

Conclusion: CXR-LT 2024为临床现实和泛化诊断模型的开发提供了宝贵资源，推动了胸部放射学的技术进步。
...
### [186] [Rethinking Crowd-Sourced Evaluation of Neuron Explanations](https://arxiv.org/abs/2506.07985)
...
<details>
  <summary>Details</summary>
Motivation: 现有神经元解释方法的可靠性难以评估，且众包评估成本高、噪声大。

Method: 引入重要性采样选择最有价值的输入，并采用贝叶斯方法聚合评分。

Result: 实现了约30倍的成本降低和约5倍的评分效率提升。

Conclusion: 该方法在大规模研究中验证了流行神经元解释方法的质量。
...
### [187] [Rethinking Cross-Modal Interaction in Multimodal Diffusion Transformers](https://arxiv.org/abs/2506.07986)
*Zhengyao Lv,Tianlin Pan,Chenyang Si,Zhaoxi Chen,Wangmeng Zuo,Ziwei Liu,Kwan-Yee K. Wong*

Main category: cs.CV

TL;DR: 论文提出了一种名为TACA的方法，通过动态调整跨模态注意力和时间步感知权重，显著提升了文本到图像生成模型的对齐效果。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的MM-DiT模型（如FLUX）在文本提示与生成内容之间的对齐上存在不足，主要问题包括跨模态注意力抑制和缺乏时间步感知权重。

Method: 提出了Temperature-Adjusted Cross-modal Attention (TACA)，通过温度缩放和时间步依赖的调整动态平衡多模态交互，并结合LoRA微调。

Result: 在T2I-CompBench基准测试中，TACA显著提升了文本-图像对齐效果，包括物体外观、属性绑定和空间关系。

Conclusion: 研究强调了平衡跨模态注意力对提升文本到图像扩散模型语义保真度的重要性。
...
### [188] [PairEdit: Learning Semantic Variations for Exemplar-based Image Editing](https://arxiv.org/abs/2506.07992)
*Haoguang Lu,Jiacheng Chen,Zhenguo Yang,Aurele Tohokantche Gnanha,Fu Lee Wang,Li Qing,Xudong Mao*

Main category: cs.CV

TL;DR: PairEdit是一种无需文本指导的视觉编辑方法，通过图像对学习复杂编辑语义，显著提升内容一致性。
...
### [189] [UA-Pose: Uncertainty-Aware 6D Object Pose Estimation and Online Object Completion with Partial References](https://arxiv.org/abs/2506.07996)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法需要完整3D模型或大量参考图像，而部分参考数据（如片段化的物体外观和几何）下的6D姿态估计仍具挑战性。

Method: 基于有限RGBD图像或单张2D图像初始化部分3D模型，引入不确定性区分可见与不可见区域，并通过不确定性感知采样策略在线补全物体。

Result: 在YCB-Video等数据集上，UA-Pose显著优于现有方法，尤其在物体观测不完整时表现突出。

Conclusion: UA-Pose通过不确定性建模和在线补全，有效解决了部分参考数据下的6D姿态估计问题。
...
### [190] [MADFormer: Mixed Autoregressive and Diffusion Transformers for Continuous Image Generation](https://arxiv.org/abs/2506.07999)
*Junhao Chen,Yulia Tsvetkov,Xiaochuang Han*

Main category: cs.CV

TL;DR: MADFormer结合自回归和扩散模型，通过分块生成和混合层设计优化高分辨率图像生成性能。
...
Result: 实验表明分块生成显著提升高分辨率图像性能，混合层设计优化质量与效率平衡。

Conclusion: 为未来混合生成模型提供实用设计原则。
...
### [191] [Aligning Text, Images, and 3D Structure Token-by-Token](https://arxiv.org/abs/2506.08002)
...
<details>
  <summary>Details</summary>
Motivation: 3D理解对设计3D环境和机器人导航至关重要，受语言和图像建模进展启发，探索自回归模型在结构化3D场景中的应用。

Method: 提出统一的LLM框架，结合量化形状编码，评估了四种核心3D任务和四种数据集。

Result: 模型在真实世界3D物体识别任务中表现有效。

Conclusion: 该框架为3D场景建模提供了新思路，并在多任务和多数据集中验证了其有效性。
...
### [192] [Audio-Sync Video Generation with Multi-Stream Temporal Control](https://arxiv.org/abs/2506.08003)
*Shuchen Weng,Haojie Zheng,Zheng Chang,Si Li,Boxin Shi,Xinlong Wang*

Main category: cs.CV

TL;DR: MTV是一个用于音频同步视频生成的框架，通过分离音频为语音、效果和音乐轨道，实现精细控制和语义对齐的视频生成。


<details>
  <summary>Details</summary>
Motivation: 音频与视觉世界紧密同步，是可控视频生成的理想控制信号，但现有方法在高质量视频生成和精确音频-视频同步方面表现不足。

Method: MTV框架将音频分为语音、效果和音乐轨道，分别控制唇部动作、事件时间和视觉情绪，并利用DEMIX数据集进行多阶段训练。

Result: MTV在视频质量、文本-视频一致性和音频-视频对齐六个标准指标上达到最先进性能。

Conclusion: MTV通过音频分离和多阶段训练实现了高质量、精确同步的视频生成，为音频驱动的视频生成提供了新方法。
...
### [193] [Dynamic View Synthesis as an Inverse Problem](https://arxiv.org/abs/2506.08004)
...
<details>
  <summary>Details</summary>
Motivation: 解决动态视图合成中的确定性反转问题，特别是在零终端信噪比（SNR）调度下的挑战。

Method: 引入K阶递归噪声表示和随机潜在调制，实现潜在空间的精确对齐和遮挡区域补全。

Result: 实验表明，通过噪声初始化阶段的结构化潜在操作，可以高效完成动态视图合成。

Conclusion: 动态视图合成可通过噪声初始化阶段的潜在操作实现，无需额外训练或模块。
...
### [194] [ZeroVO: Visual Odometry with Minimal Assumptions](https://arxiv.org/abs/2506.08005)
*Lei Lai,Zekai Yin,Eshed Ohn-Bar*

Main category: cs.CV

TL;DR: ZeroVO是一种新型视觉里程计算法，无需预定义或静态相机校准，通过几何感知网络、语言先验和半监督训练实现零样本泛化，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有视觉里程计方法依赖预定义或静态相机校准，限制了其泛化能力。ZeroVO旨在克服这一限制，提供更通用的解决方案。

Method: 1. 设计校准无关的几何感知网络；2. 引入语言先验增强语义特征提取；3. 开发半监督训练范式，利用未标记数据适应新场景。

Result: 在KITTI、nuScenes和Argoverse 2等基准测试中，性能提升超过30%，并在GTA合成数据集上验证了泛化能力。

Conclusion: ZeroVO无需微调或相机校准，显著提升了视觉里程计的泛化能力和实际应用潜力。
...
### [195] [Dreamland: Controllable World Creation with Simulator and Generative Models](https://arxiv.org/abs/2506.08006)
...
Method: 设计分层世界抽象，编码像素级和对象级语义与几何信息，作为模拟器与生成模型的中间表示。

Result: 实验显示，Dreamland在图像质量上提升50.8%，可控性提升17.9%，并支持现成生成模型的使用。

Conclusion: Dreamland通过分层抽象和早期对齐，显著提升了生成模型的实用性和可控性，具有广泛应用潜力。
...
### [196] [Hidden in plain sight: VLMs overlook their visual representations](https://arxiv.org/abs/2506.08008)
*Stephanie Fu,Tyler Bonnen,Devin Guillory,Trevor Darrell*

Main category: cs.CV

TL;DR: 论文比较了视觉语言模型（VLMs）与其视觉编码器的性能，发现VLMs在视觉任务中表现显著下降，接近随机水平。分析表明，瓶颈在于VLMs未能有效利用视觉信息，且受到语言模型的先验影响。


<details>
  <summary>Details</summary>
Motivation: 探索视觉语言模型（VLMs）在整合视觉和语言信息方面的能力，以诊断其失败模式并为未来研究提供评估方法。

Method: 通过一系列视觉中心基准测试（如深度估计、对应关系）比较VLMs与其视觉编码器的表现，并分析VLMs的视觉表征退化、任务提示的脆弱性及语言模型的作用。

Result: VLMs在视觉任务中表现显著低于视觉编码器，接近随机水平，主要原因是未能有效利用视觉信息且受语言模型先验影响。
...
### [197] [Self Forcing: Bridging the Train-Test Gap in Autoregressive Video Diffusion](https://arxiv.org/abs/2506.08009)
*Xun Huang,Zhengqi Li,Guande He,Mingyuan Zhou,Eli Shechtman*

Main category: cs.CV

TL;DR: Self Forcing是一种新的自回归视频扩散模型训练范式，通过自生成输出条件训练，解决了曝光偏差问题，实现了高效实时视频生成。


<details>
  <summary>Details</summary>
Motivation: 解决传统方法中模型在推理时因依赖自身不完美输出而产生的曝光偏差问题。

Method: 采用自回归展开和KV缓存策略，结合多步扩散模型和随机梯度截断，实现高效训练。

Result: 实验表明，该方法在单GPU上实现亚秒级延迟的实时视频生成，质量优于非因果扩散模型。

Conclusion: Self Forcing在视频生成质量和效率上均表现出色，为实时应用提供了可行方案。
...
### [198] [Vision Transformers Don't Need Trained Registers](https://arxiv.org/abs/2506.08010)
*Nick Jiang,Amil Dravid,Alexei Efros,Yossi Gandelsman*

Main category: cs.CV

TL;DR: 研究发现Vision Transformers中存在高范数令牌导致注意力图噪声的问题，提出无需重新训练的解决方案，通过转移高范数激活改善模型性能。


<details>
  <summary>Details</summary>
Motivation: 探索Vision Transformers中高范数令牌导致注意力图噪声的机制，并寻找无需重新训练的解决方案。

Method: 通过将高范数激活转移到额外的未训练令牌中，模拟注册令牌的效果，无需重新训练模型。

Result: 方法改善了注意力图和特征图，提升了多个下游视觉任务的性能，效果接近显式训练注册令牌的模型。

Conclusion: 测试时注册令牌为预训练模型提供了一种无需训练的解决方案，提升了模型的可解释性和性能。
...
### [199] [Play to Generalize: Learning to Reason Through Game Play](https://arxiv.org/abs/2506.08011)
*Yunfei Xie,Yinsong Ma,Shiyi Lan,Alan Yuille,Junfei Xiao,Chen Wei*

Main category: cs.CV

TL;DR: 通过视觉游戏学习（ViGaL）提升多模态大语言模型的泛化推理能力，无需依赖具体领域数据。


<details>
  <summary>Details</summary>
Motivation: 认知科学表明游戏能促进可迁移的认知技能，因此提出通过游戏训练提升模型的跨领域推理能力。

Method: 采用强化学习在简单街机游戏（如贪吃蛇）上对7B参数模型进行后训练。

Result: 模型在多模态数学基准（如MathVista）和跨学科问题（如MMMU）上表现显著提升，甚至超越专业模型。

Conclusion: 规则简单的游戏可作为可控、可扩展的预训练任务，解锁多模态大语言模型的泛化推理能力。
...
### [200] [StableMTL: Repurposing Latent Diffusion Models for Multi-Task Learning from Partially Annotated Synthetic Datasets](https://arxiv.org/abs/2506.08013)
...
<details>
  <summary>Details</summary>
Motivation: 多任务密集预测需要大量标注，而部分任务标注的学习方法仍有局限性，因此探索零样本设置下的多任务学习。

Method: 采用扩散模型的潜在回归，结合任务编码、任务条件化和统一潜在损失，设计多流模型和任务注意力机制。

Result: 在8个基准测试的7个任务上表现优于基线方法。

Conclusion: StableMTL通过潜在损失和任务注意力机制，实现了高效的多任务学习，扩展性强且性能优越。
...
### [201] [4DGT: Learning a 4D Gaussian Transformer Using Real-World Monocular Videos](https://arxiv.org/abs/2506.08015)
*Zhen Xu,Zhengqin Li,Zhao Dong,Xiaowei Zhou,Richard Newcombe,Zhaoyang Lv*

Main category: cs.CV

TL;DR: 4DGT是一种基于4D高斯的Transformer模型，用于动态场景重建，通过单目视频训练，统一静态和动态组件，实现高效渲染。


<details>
  <summary>Details</summary>
Motivation: 解决动态场景重建中复杂时间变化环境和对象寿命差异的问题，同时减少优化方法的长时间计算需求。

Method: 使用4D高斯作为归纳偏置，提出密度控制策略处理长时空输入，以滚动窗口方式处理64帧。

Result: 在真实视频中显著优于其他高斯网络，跨域视频中与优化方法精度相当，重建时间从小时级降至秒级。

Conclusion: 4DGT通过纯前馈推理高效重建动态场景，适用于长视频序列，训练数据仅为单目视频。
...
### [202] [CellCLIP -- Learning Perturbation Effects in Cell Painting via Text-Guided Contrastive Learning](https://arxiv.org/abs/2506.06290)
...
<details>
  <summary>Details</summary>
Motivation: 利用高内涵筛选数据研究细胞形态对扰动的响应，但现有方法因图像语义差异和扰动类别多样性难以直接应用。
...
### [203] [NeurNCD: Novel Class Discovery via Implicit Neural Representation](https://arxiv.org/abs/2506.06412)
*Junming Wang,Yi Shi*

Main category: cs.LG

TL;DR: NeurNCD是一种基于Embedding-NeRF和KL散度的新型类发现框架，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统显式表示（如3D分割图）存在离散、易噪声等问题，限制了新类发现的准确性。

Method: 结合Embedding-NeRF和KL散度，通过特征查询、调制和聚类实现高效特征增强和信息交换。

Result: 在NYUv2和Replica数据集上显著优于现有方法，无需密集标注或人工干预。

Conclusion: NeurNCD为开放世界中的新类发现提供了高效且通用的解决方案。
...
### [204] [Vision-QRWKV: Exploring Quantum-Enhanced RWKV Models for Image Classification](https://arxiv.org/abs/2506.06633)
*Chi-Sheng Chen*

Main category: cs.LG

TL;DR: 论文提出了一种混合量子-经典架构Vision-QRWKV，用于图像分类任务，通过将变分量子电路集成到RWKV中，提升了非线性特征变换能力。实验表明，量子增强模型在多数数据集上优于经典模型。


<details>
  <summary>Details</summary>
Motivation: 利用量子机器学习增强经典神经网络架构，特别是在处理复杂高维数据时，提升视觉任务的性能。

Method: 将变分量子电路（VQC）集成到RWKV的通道混合组件中，构建Vision-QRWKV模型，并在14个图像分类基准数据集上进行评估。

Result: 量子增强模型在多数数据集上表现优于经典模型，尤其是在具有细微或噪声类别区分的数据集上（如ChestMNIST、RetinaMNIST、BloodMNIST）。

Conclusion: Vision-QRWKV是量子增强RWKV在视觉领域的首次系统应用，展示了量子模型在轻量高效视觉任务中的潜力。
...
### [205] [Non-Intrusive Load Monitoring Based on Image Load Signatures and Continual Learning](https://arxiv.org/abs/2506.06637)
...
### [206] [The OCR Quest for Generalization: Learning to recognize low-resource alphabets with model editing](https://arxiv.org/abs/2506.06761)
*Adrià Molina Rodríguez,Oriol Ramos Terrades,Josep Lladós*

Main category: cs.LG

TL;DR: 论文提出了一种通过模型编辑增强低资源语言识别的方法，优于传统元学习，显著提升了新字母表和域外数据的迁移学习性能。
...
Method: 利用模型编辑技术，结合领域合并策略，避免对数据分布或原型需求的依赖。

Result: 实验表明，即使使用相同训练数据，新方法在迁移学习和域外评估中表现显著优于现有技术。
...
### [207] [Feature-Based Instance Neighbor Discovery: Advanced Stable Test-Time Adaptation in Dynamic World](https://arxiv.org/abs/2506.06782)
...
<details>
  <summary>Details</summary>
Motivation: 深度神经网络在训练与测试分布变化时性能下降，现有方法难以应对动态多分布场景。

Method: 提出FIND方法，包括LFD、FABN和S-FABN，分别处理特征解耦、自适应归一化和选择性优化。

Result: FIND在动态场景中准确率提升30%，同时保持计算效率。

Conclusion: FIND通过特征分区和自适应归一化，有效解决了动态分布变化下的性能问题。
...
### [208] [FREE: Fast and Robust Vision Language Models with Early Exits](https://arxiv.org/abs/2506.06884)
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（VLMs）在任务中表现优异，但模型体积大导致推理延迟问题，限制了实际应用。

Method: 采用早期退出（EE）策略，结合对抗训练和GAN框架，训练退出分类器以生成类似最终层的特征表示。
...
### [209] [Rewriting the Budget: A General Framework for Black-Box Attacks Under Cost Asymmetry](https://arxiv.org/abs/2506.06933)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法假设所有查询成本相同，而实际中某些查询可能成本更高（如触发额外审查的类别）。针对这一非对称成本场景，现有算法效果有限。

Method: 提出了非对称搜索（AS）和非对称梯度估计（AGREST），通过优化搜索策略和采样分布，减少高成本查询的依赖。
...
### [210] [Towards Physics-informed Diffusion for Anomaly Detection in Trajectories](https://arxiv.org/abs/2506.06999)
...
Method: 结合运动学约束的物理知识扩散模型，识别不符合物理规律的轨迹。

Result: 在海上和城市领域的真实数据集中，实现了更高的异常检测准确率和更低的估计误差。

Conclusion: 提出的物理知识扩散模型有效提升了异常轨迹检测的精度，为实际应用提供了可靠工具。
...
### [211] [Advancing Multimodal Reasoning Capabilities of Multimodal Large Language Models via Visual Perception Reward](https://arxiv.org/abs/2506.07218)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于RLVR的方法忽视了多模态感知能力的提升，而这是复杂推理的基础。通过McNemar测试发现现有方法在感知能力上效果不佳。

Method: 提出Perception-R1，通过视觉感知奖励激励MLLMs准确感知视觉内容。具体包括收集视觉注释作为奖励参考，并使用LLM评估一致性以分配奖励。

Result: 在多个多模态推理基准测试中，Perception-R1仅用1,442训练数据即达到最优性能。

Conclusion: Perception-R1有效提升了MLLMs的感知和推理能力，填补了现有方法的不足。
...
### [212] [Variational Supervised Contrastive Learning](https://arxiv.org/abs/2506.07413)
*Ziwen Wang,Jiajun Fan,Thao Nguyen,Heng Ji,Ge Liu*

Main category: cs.LG

TL;DR: VarCon通过变分推理改进对比学习，解决了嵌入分布无显式调控和依赖大批量负样本的问题，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有对比学习存在嵌入分布无显式调控和依赖大批量负样本的局限性，影响泛化能力。

Method: 提出VarCon，将监督对比学习重新表述为对潜在类变量的变分推理，最大化后验加权的ELBO，实现高效的类感知匹配和细粒度控制。

Result: 在CIFAR-10、CIFAR-100、ImageNet等数据集上达到SOTA性能，Top-1准确率79.36%（ImageNet-1K），并表现出更好的决策边界和少样本学习能力。

Conclusion: VarCon显著提升了对比学习的性能和泛化能力，为嵌入空间的组织提供了更优的解决方案。
...
### [213] [Language Embedding Meets Dynamic Graph: A New Exploration for Neural Architecture Representation Learning](https://arxiv.org/abs/2506.07735)
*Haizhao Jing,Haokui Zhang,Zhenhao Shang,Rong Xiao,Peng Wang,Yanning Zhang*

Main category: cs.LG

TL;DR: LeDG-Former通过结合语言嵌入和动态图表示学习，解决了现有方法忽视硬件属性和静态邻接矩阵的局限性，实现了跨硬件平台的零样本预测和更优的神经架构建模。


<details>
  <summary>Details</summary>
Motivation: 当前方法忽视了硬件属性信息且依赖静态邻接矩阵，限制了模型的实用性和编码效果。

Method: 提出LeDG-Former框架，结合语言嵌入（利用LLM处理神经架构和硬件规格）和动态图Transformer。

Result: 在NNLQP基准上超越现有方法，首次实现跨硬件延迟预测，并在NAS-Bench数据集上表现优异。

Conclusion: LeDG-Former通过创新方法解决了现有问题，提升了神经架构表示学习的性能和应用范围。
...
### [214] [Identifiable Object Representations under Spatial Ambiguities](https://arxiv.org/abs/2506.07806)
...
<details>
  <summary>Details</summary>
Motivation: 模块化对象中心表示对人类推理至关重要，但在空间模糊性（如遮挡和视角模糊）下难以获取。

Method: 引入多视角概率方法，聚合视角特定槽以捕获不变内容信息，同时学习解耦的全局视角级信息。

Result: 方法解决了空间模糊性，提供了可识别性理论保证，并在标准和新数据集上验证了鲁棒性和扩展性。

Conclusion: 该方法在无需视角标注的情况下，有效解决了空间模糊性问题，并展示了理论和实践优势。
...
### [215] [Diffusion Counterfactual Generation with Semantic Abduction](https://arxiv.org/abs/2506.07883)
*Rajat Rasal,Avinash Kori,Fabio De Sousa Ribeiro,Tian Xia,Ben Glocker*

Main category: cs.LG

TL;DR: 该论文提出了一种基于扩散模型的因果图像编辑框架，通过空间、语义和动态推理实现高质量的反事实图像生成。


<details>
  <summary>Details</summary>
Motivation: 现有自编码框架在可扩展性和保真度上存在不足，扩散模型在视觉质量和语义控制方面表现出色，因此探索其在反事实图像生成中的应用。

Method: 提出了一套扩散模型的因果机制，结合Pearl因果理论，将语义表征集成到扩散模型中，实现反事实推理编辑。

Result: 首次实现了扩散模型中高级语义身份保留，展示了语义控制在因果控制与身份保留之间的权衡。

Conclusion: 扩散模型在反事实图像编辑中具有潜力，能够平衡因果控制的忠实性与身份保留。
...
### [216] [Diffuse Everything: Multimodal Diffusion Models on Arbitrary State Spaces](https://arxiv.org/abs/2506.07903)
*Kevin Rojas,Yuchen Zhu,Sichen Zhu,Felix X. -F. Ye,Molei Tao*

Main category: cs.LG

TL;DR: 本文提出了一种新的多模态扩散模型框架，支持原生生成跨模态的耦合数据，无需依赖外部预处理协议。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部预处理协议（如分词器和变分自编码器）统一数据表示，但这对数据有限的应用存在问题。本文旨在解决这一问题。

Method: 提出了一种新颖的框架，支持在任意状态空间上构建多模态扩散模型，并引入解耦的噪声调度策略，实现无条件生成和模态条件生成。

Result: 在文本-图像生成和混合类型表格数据合成任务中验证了方法的有效性，表现具有竞争力。

Conclusion: 本文提出的框架为多模态数据生成提供了更灵活的解决方案，避免了对外部预处理的高精度依赖。
...
### [217] [Generative Modeling of Weights: Generalization or Memorization?](https://arxiv.org/abs/2506.07998)
*Boya Zeng,Yida Yin,Zhiqiu Xu,Zhuang Liu*

Main category: cs.LG

TL;DR: 生成模型在生成神经网络权重时主要依赖记忆训练数据，无法有效生成新颖且高性能的权重。


<details>
  <summary>Details</summary>
Motivation: 探索生成模型在合成神经网络权重方面的能力，评估其是否能生成不同于训练数据的新颖权重。

Method: 研究了四种代表性方法，分析其生成权重的表现，并与简单基线方法（如添加噪声或权重集成）进行比较。

Result: 发现当前方法主要通过记忆训练数据生成权重，无法超越简单基线方法，且无法通过调整模型或数据增强缓解记忆问题。
...
### [218] [QForce-RL: Quantized FPGA-Optimized Reinforcement Learning Compute Engine](https://arxiv.org/abs/2506.07046)
...
Method: 结合E2HRL减少RL动作，QuaRL量化加速硬件，分析模型参数和计算操作。

Result: 性能提升2.3倍，FPS提升2.6倍，适用于资源受限设备。

Conclusion: QForce-RL在性能和能效上优于现有技术，适合灵活部署。
...
### [219] [Contextual Experience Replay for Self-Improvement of Language Agents](https://arxiv.org/abs/2506.06698)
*Yitao Liu,Chenglei Si,Karthik Narasimhan,Shunyu Yao*

Main category: cs.AI

TL;DR: 论文提出了一种名为Contextual Experience Replay (CER)的无训练框架，通过动态记忆缓冲区积累和合成过去经验，提升语言模型代理在复杂任务中的适应能力。
...
Method: CER通过动态记忆缓冲区积累和合成环境动态和决策模式的经验，使代理能在新任务中检索并增强相关知识。

Result: 在VisualWebArena和WebArena基准测试中，CER分别达到31.9%和36.7%的成功率，相对GPT-4o基线提升51.0%。

Conclusion: CER有效提升了语言模型代理的适应能力，证明了其在复杂任务中的高效性和有效性。
...
### [220] [VisioMath: Benchmarking Figure-based Mathematical Reasoning in LMMs](https://arxiv.org/abs/2506.06727)
...
Method: VisioMath包含8,070张图像和1,800道多选题，每个选项为图像，评估现有模型的性能。

Result: 即使是先进的模型（如GPT-4o）在VisioMath上准确率仅为45.9%，显示当前模型在视觉相似选项推理上的局限性。

Conclusion: VisioMath为未来多模态推理研究提供了严格测试基准，推动技术进步。
...
### [221] [Meta-Adaptive Prompt Distillation for Few-Shot Visual Question Answering](https://arxiv.org/abs/2506.06905)
*Akash Gupta,Amos Storkey,Mirella Lapata*

Main category: cs.AI

TL;DR: 论文提出一种元学习方法，通过软提示和注意力映射模块提升小规模多模态模型的少样本学习能力，优于传统上下文学习方法。


<details>
  <summary>Details</summary>
Motivation: 现有大型多模态模型（LMMs）的上下文学习（ICL）性能不稳定，尤其是在小规模模型中，增加示例未必提升效果，可能因冗余图像信息干扰任务表现。

Method: 提出基于元学习的软提示蒸馏方法，结合注意力映射模块，从任务相关图像特征中提取软提示，并在测试时通过少量示例调整。

Result: 在VL-ICL Bench上的实验表明，该方法在视觉问答任务中优于ICL和其他提示调优方法，且对图像扰动具有鲁棒性。

Conclusion: 通过软提示和注意力映射的联合学习，该方法有效提升了LMMs在低数据条件下的任务适应能力。
...
### [222] [Long-Tailed Learning for Generalized Category Discovery](https://arxiv.org/abs/2506.06965)
*Cuong Manh Hoang*

Main category: cs.AI

TL;DR: 论文提出了一种在长尾分布中进行广义类别发现的新框架，通过自引导标记和表示平衡技术提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界的数据集通常是不平衡的，现有方法在人工平衡数据集上表现良好，但在不平衡数据上效果不佳。

Method: 提出自引导标记技术和表示平衡过程，前者生成伪标签以减少偏差，后者通过挖掘样本邻域提升尾类关注。

Result: 在公开数据集上的实验表明，该框架优于现有最优方法。

Conclusion: 新框架有效解决了长尾分布下的广义类别发现问题，性能显著提升。
...
### [223] [Mitigating Behavioral Hallucination in Multimodal Large Language Models for Sequential Images](https://arxiv.org/abs/2506.07184)
*Liangliang You,Junchi Yao,Shu Yang,Guimin Hu,Lijie Hu,Di Wang*

Main category: cs.AI

TL;DR: 论文提出SHE框架，解决多模态大语言模型在序列图像中的行为幻觉问题，通过两阶段方法检测和缓解幻觉，并引入新指标BEACH量化效果。
...
Conclusion: SHE有效解决了序列图像中的行为幻觉问题，为多模态模型的可靠性提供了新方法。
...
### [224] [Reinforcing Multimodal Understanding and Generation with Dual Self-rewards](https://arxiv.org/abs/2506.07963)
...
<details>
  <summary>Details</summary>
Motivation: 解决大型多模态模型在图像-文本对齐上的不足，避免生成与视觉输入矛盾或不符合文本提示的响应。

Method: 基于理解与生成是逆对偶任务的观察，提出自监督双奖励机制，通过反转输入-输出对计算双似然作为自我奖励进行优化。

Result: 在视觉理解和生成基准测试中显著提升模型性能，尤其在文本到图像任务中表现突出。

Conclusion: 自监督双奖励机制有效提升了模型的性能，无需依赖外部监督，为多模态任务提供了新思路。
...
### [225] [GUI-Reflection: Empowering Multimodal GUI Models with Self-Reflection Behavior](https://arxiv.org/abs/2506.08012)
...
<details>
  <summary>Details</summary>
Motivation: 现有GUI模型依赖无错误离线轨迹，缺乏反思和错误恢复能力，限制了其实际应用。

Method: 提出GUI-Reflection框架，包括GUI预训练、离线监督微调和在线反思调优三阶段训练，自动生成反思数据并设计任务套件。

Result: 框架成功赋予GUI代理自我反思和纠正能力，无需人工标注，提升了自动化适应性。

Conclusion: GUI-Reflection为更智能、鲁棒的GUI自动化铺平道路，相关资源将公开。
...
### [226] [A Culturally-diverse Multilingual Multimodal Video Benchmark & Model](https://arxiv.org/abs/2506.07032)
...
TL;DR: 该论文提出了一个多语言视频LMM基准ViMUL-Bench，涵盖14种语言，并开发了多语言视频LMM模型ViMUL，旨在促进文化和语言包容性研究。


<details>
  <summary>Details</summary>
Motivation: 现有的大型多模态模型（LMMs）主要集中在英语，缺乏对多语言和文化多样性的视频内容理解。

Method: 引入ViMUL-Bench基准，包含8k手动验证的样本，覆盖15个类别和14种语言；开发了ViMUL模型，并提供了120万样本的多语言视频训练集。

Result: ViMUL模型在高资源和低资源语言之间取得了更好的平衡，提升了视频理解能力。

Conclusion: ViMUL-Bench、ViMUL模型及训练数据将公开，以推动文化和语言包容性多语言视频LMM的研究。
...
### [227] [Lingshu: A Generalist Foundation Model for Unified Multimodal Medical Understanding and Reasoning](https://arxiv.org/abs/2506.07044)
...
TL;DR: 论文提出了一种针对医学领域的多模态大语言模型（MLLM）Lingshu，通过改进数据整理和训练策略，解决了现有医学MLLM的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有医学MLLM在医学应用中表现有限，主要由于数据与任务的不匹配、幻觉问题及缺乏医学推理能力。

Method: 提出全面的数据整理流程，构建多模态医学数据集，并通过多阶段训练和强化学习增强模型能力。

Result: Lingshu在多项医学任务中表现优于现有开源多模态模型。

Conclusion: Lingshu为医学MLLM的发展提供了有效解决方案，并通过MedEvalKit标准化评估框架。
...
### [228] [Flattery in Motion: Benchmarking and Analyzing Sycophancy in Video-LLMs](https://arxiv.org/abs/2506.07180)
...
<details>
  <summary>Details</summary>
Motivation: 随着Video-LLMs在需要多模态推理的实际应用中的普及，确保其事实一致性和可靠性至关重要。然而，模型倾向于迎合用户输入（即使与视觉证据矛盾）的行为损害了其可信度。目前缺乏针对视频语言领域的系统性基准和评估。

Method: 提出VISE基准，首次针对Video-LLMs设计，涵盖多样化问题格式、提示偏见和视觉推理任务，并结合语言学视角分析迎合行为。探索关键帧选择作为无训练的缓解策略。

Result: VISE提供了对Video-LLMs迎合行为的细粒度分析，并展示了关键帧选择在增强视觉基础、减少迎合偏见方面的潜力。

Conclusion: VISE填补了视频语言领域迎合行为研究的空白，为未来改进Video-LLMs的可信度提供了方向。
...
### [229] [Unblocking Fine-Grained Evaluation of Detailed Captions: An Explaining AutoRater and Critic-and-Revise Pipeline](https://arxiv.org/abs/2506.07631)
*Brian Gordon,Yonatan Bitton,Andreea Marzoca,Yasumasa Onoe,Xiao Wang,Daniel Cohen-Or,Idan Szpektor*

Main category: cs.CL

TL;DR: 论文提出了DOCCI-Critique基准和VNLI-Critique模型，用于评估和改进大视觉语言模型（VLMs）生成图像描述的事实准确性。


<details>
  <summary>Details</summary>
Motivation: 当前方法难以检测VLMs生成的段落长度描述中的细粒度错误，缺乏验证数据集。

Method: 引入DOCCI-Critique基准（1,400条标注数据）和VNLI-Critique模型，用于自动化事实分类和错误分析。

Result: VNLI-Critique在多个基准测试中表现优异，AutoRater与人类判断高度一致，Critic-and-Revise流程显著提升描述准确性。

Conclusion: 研究提供了关键基准和工具，提升了VLMs图像理解的细粒度评估标准。
...
TL;DR: Pendulum Tracker是一个基于计算机视觉的实时测量物理摆运动的工具，适用于教育平台SimuFísica，支持多种设备，并能导出数据。


<details>
  <summary>Details</summary>
Motivation: 为教育场景提供一种实时、准确且易用的物理摆运动测量工具，帮助学生和教师更直观地理解振荡运动。

Method: 利用OpenCV.js库通过设备摄像头自动检测摆的位置，实时显示角度-时间图并估算振荡周期。

Result: 实验结果表明，测量结果与理论预测高度一致，适用于周期测量、重力加速度计算和阻尼振荡分析。
...
TL;DR: 该研究利用大型语言模型（LLMs）模拟地震影响，结合多模态数据预测地震强度，结果显示与真实报告高度一致。
...
Method: 使用多模态数据集（地理空间、社会经济、建筑和街景图像数据），结合RAG和ICL技术，预测地震的修正麦卡利强度（MMI）。

Result: 在2014年纳帕和2019年里奇克莱斯特地震中，预测结果与USGS报告高度相关（0.88），RMSE低至0.77。

Conclusion: LLMs在模拟灾害影响方面具有潜力，可帮助加强事前规划。
...
### [232] [ResPF: Residual Poisson Flow for Efficient and Physically Consistent Sparse-View CT Reconstruction](https://arxiv.org/abs/2506.06400)
*Changsheng Fang,Yongtong Liu,Bahareh Morovati,Shuo Han,Yu Shi,Li Zhou,Shuyi Fan,Hengyong Yu*

Main category: eess.IV

TL;DR: 提出ResPF模型，结合PFGM++和残差融合模块，高效解决稀疏视图CT重建问题，提升重建质量和速度。


<details>
  <summary>Details</summary>
Motivation: 稀疏视图CT可减少辐射剂量，但重建问题复杂，现有方法缺乏物理可解释性或计算成本高。

Method: 基于PFGM++，引入条件指导和劫持策略减少采样成本，嵌入数据一致性保证重建质量，提出残差融合模块保持轨迹连续性。

Result: 在合成和临床数据集上，ResPF重建质量、推理速度和鲁棒性优于现有方法。
...
### [233] [SPC to 3D: Novel View Synthesis from Binary SPC via I2I translation](https://arxiv.org/abs/2506.06890)
...
Method: 采用模块化两阶段框架：第一阶段使用生成模型（如Pix2PixHD）将二进制SPC图像转换为RGB图像；第二阶段利用3D重建技术（如NeRF或3DGS）生成新视图。

Result: 通过实验验证，该框架在感知质量和几何一致性上显著优于基线方法。

Conclusion: 提出的两阶段框架有效解决了SPC图像信息丢失问题，为3D重建提供了高质量解决方案。
...
### [234] [Optimal Transport Driven Asymmetric Image-to-Image Translation for Nuclei Segmentation of Histological Images](https://arxiv.org/abs/2506.07023)
*Suman Mahapatra,Pradipta Maji*

Main category: eess.IV

TL;DR: 提出了一种新的深度生成模型，用于从组织学图像中分割细胞核，通过嵌入空间处理信息不对称问题，并引入可逆生成器和空间约束操作，优化网络复杂度和性能。


<details>
  <summary>Details</summary>
Motivation: 解决图像到图像翻译模型在信息不对称情况下表现不佳的问题，以支持疾病检测和诊断。

Method: 结合最优传输和测度理论，设计可逆生成器，避免显式循环一致性损失，并引入空间约束操作保持图像块的空间连续性。

Result: 模型在公开数据集上表现优于现有方法，实现了网络复杂度和性能的更好平衡。

Conclusion: 提出的深度生成模型为细胞核分割提供了一种高效且性能优越的解决方案。
...
### [235] [SiliCoN: Simultaneous Nuclei Segmentation and Color Normalization of Histological Images](https://arxiv.org/abs/2506.07028)
*Suman Mahapatra,Pradipta Maji*

Main category: eess.IV

TL;DR: 论文提出了一种新型深度生成模型，用于同时分割细胞核结构和标准化染色组织图像的颜色外观。


<details>
  <summary>Details</summary>
Motivation: 解决染色组织图像中颜色变化对细胞核分割的影响，并提升分割与颜色标准化的协同效果。

Method: 结合截断正态分布和空间注意力的深度生成模型，假设颜色外观信息与细胞核分割图独立。

Result: 模型在公开数据集上表现优异，优于现有先进算法。

Conclusion: 该模型具有通用性和适应性，颜色信息的变化不会影响细胞核分割结果。
...
### [236] [Transfer Learning and Explainable AI for Brain Tumor Classification: A Study Using MRI Data from Bangladesh](https://arxiv.org/abs/2506.07228)
*Shuvashis Sarker*

Main category: eess.IV

TL;DR: 研究开发了一种基于深度学习的自动脑肿瘤分类系统，结合可解释AI技术，在资源有限的地区（如孟加拉国）提高脑肿瘤诊断效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 脑肿瘤（尤其是恶性肿瘤）的快速诊断对改善患者预后至关重要，但手动MRI分析效率低且易出错，尤其在医疗资源有限的地区。
...
### [237] [A Comprehensive Analysis of COVID-19 Detection Using Bangladeshi Data and Explainable AI](https://arxiv.org/abs/2506.07234)
*Shuvashis Sarker*

Main category: eess.IV

TL;DR: 该研究利用深度学习模型VGG19在CXR图像中检测COVID-19，准确率达98%，并通过LIME和SMOTE增强模型透明度和数据平衡。
...
### [238] [A Narrative Review on Large AI Models in Lung Cancer Screening, Diagnosis, and Treatment Planning](https://arxiv.org/abs/2506.07236)
...
<details>
  <summary>Details</summary>
Motivation: 肺癌是全球高发且致命的疾病，亟需精准及时的诊断和治疗。大型AI模型在医学图像理解和临床决策中的进展为肺癌管理提供了新机遇。

Method: 系统调研了大型AI模型在肺癌领域的应用，分类为模态特定编码器、编码器-解码器框架和联合编码器架构，并评估了其在多模态学习任务中的性能。

Result: 模型在肺结节检测、基因突变预测、多组学整合和个性化治疗规划等方面表现出色，部分已进入临床验证阶段。

Conclusion: 大型AI模型具有优化肺癌个性化治疗的潜力，但需解决泛化性、可解释性和法规合规性等挑战，未来应构建可扩展、可解释且临床集成的AI系统。
...
### [239] [Text-guided multi-stage cross-perception network for medical image segmentation](https://arxiv.org/abs/2506.07475)
*Gaoyu Chen*
...
<details>
  <summary>Details</summary>
Motivation: 现有医学图像分割方法因目标与非目标区域对比度低导致语义表达弱，而文本引导方法存在跨模态交互不足和特征表达不充分的问题。

Method: 引入多阶段交叉注意力模块增强语义细节理解，使用多阶段对齐损失提升跨模态语义一致性。

Result: TMC在三个公开数据集（QaTa-COV19、MosMedData和Breast）上Dice分数分别为84.77%、78.50%和88.73%，优于UNet和现有文本引导方法。
...
### [240] [Fine-Grained Motion Compression and Selective Temporal Fusion for Neural B-Frame Video Coding](https://arxiv.org/abs/2506.07709)
*Xihua Sheng,Peilin Chen,Meng Wang,Li Zhang,Shiqi Wang,Dapeng Oliver Wu*

Main category: eess.IV

TL;DR: 论文提出了一种针对神经B帧编码的增强方法，改进了运动压缩和时间融合，显著提升了性能。
...
Method: 设计了细粒度运动压缩方法和选择性时间融合方法，包括交互式双分支运动自动编码器和超先验隐式对齐机制。

Result: 实验表明，提出的编解码器优于现有神经B帧编解码器，性能接近或超过H.266/VVC参考软件。

Conclusion: 提出的方法有效解决了B帧压缩的独特挑战，显著提升了性能。
...
### [241] [Experimental Evaluation of Static Image Sub-Region-Based Search Models Using CLIP](https://arxiv.org/abs/2506.06938)
*Bastian Jäckl,Vojtěch Kloda,Daniel A. Keim,Jakub Lokoč*

Main category: cs.MM

TL;DR: 研究探讨了在多模态文本-图像模型中，通过添加基于位置的提示来提升在高度同质化专业领域中的图像检索性能。


<details>
  <summary>Details</summary>
Motivation: 专业领域中用户通常只能提供模糊的文本描述，缺乏区分同质实体的专业知识，导致检索效果不佳。

Method: 收集了741个人工标注的数据集，包含短/长文本描述和感兴趣区域的边界框，评估CLIP模型在不同静态子区域上的检索性能。

Result: 简单的3x3分区和5网格重叠方法显著提升了检索效果，并对标注框的扰动保持鲁棒性。

Conclusion: 基于位置的提示可以有效弥补模糊文本查询的不足，提升专业领域的图像检索性能。
...
### [242] [Benchmarking Early Agitation Prediction in Community-Dwelling People with Dementia Using Multimodal Sensors and Machine Learning](https://arxiv.org/abs/2506.06306)
*Ali Abedi,Charlene H. Chu,Shehroz S. Khan*

Main category: eess.SP

TL;DR: 该研究利用多模态传感器数据和机器学习方法，开发并评估了预测社区居住老年痴呆患者躁动的模型，最高AUC-ROC达0.9720。


<details>
  <summary>Details</summary>
Motivation: 躁动是老年痴呆患者常见行为，及时预测可减轻护理负担并提高生活质量。

Method: 引入新的躁动相关特征，评估多种机器学习和深度学习模型，使用TIHM数据集。

Result: 最佳模型为轻梯度提升机，AUC-ROC为0.9720，AUC-PR为0.4320。

Conclusion: 该方法支持准确、可解释的躁动预测，有助于主动护理和居家养老。
...
### [243] [An Open-Source Python Framework and Synthetic ECG Image Datasets for Digitization, Lead and Lead Name Detection, and Overlapping Signal Segmentation](https://arxiv.org/abs/2506.06315)
...
<details>
  <summary>Details</summary>
Motivation: 为推进ECG分析中的深度学习任务，如ECG数字化、导联检测和波形分割，提供高质量合成数据集。

Method: 基于PTB-XL信号数据集，生成四种开放数据集，包括配对的ECG图像和时间序列信号、YOLO格式标注的导联区域和名称检测数据、以及单导联分割掩码。

Result: 发布了开源框架和四种数据集，支持多种ECG分析任务。

Conclusion: 该框架和数据集为ECG分析的深度学习研究提供了实用工具和数据支持。
...
### [244] [Heart Rate Classification in ECG Signals Using Machine Learning and Deep Learning](https://arxiv.org/abs/2506.06349)
*Thien Nhan Vo,Thanh Xuan Truong*

Main category: eess.SP

TL;DR: 研究比较了传统机器学习（手工特征）和深度学习（ECG图像转换）对心电信号分类的效果，发现LightGBM表现最佳，准确率达99%。


<details>
  <summary>Details</summary>
Motivation: 解决心电信号分类问题，比较手工特征和图像转换两种方法的优劣。
...
Conclusion: 手工特征更适合捕捉ECG信号的时空变化，未来可结合多导联信号提升分类效果。
...
### [245] [MedChat: A Multi-Agent Framework for Multimodal Diagnosis with Large Language Models](https://arxiv.org/abs/2506.07400)
...
<details>
  <summary>Details</summary>
Motivation: 解决通用LLM在医疗影像应用中因幻觉、解释性不足和领域知识缺乏导致的临床准确性下降问题。

Method: 设计多代理框架MedChat，结合专用视觉模型和角色化LLM代理，由导演代理协调，提升可靠性和交互性。

Result: MedChat减少幻觉风险，支持交互式诊断报告，适用于临床和教育场景。

Conclusion: MedChat通过多代理协作有效提升医疗影像诊断的准确性和实用性。
...
### [246] [HotelMatch-LLM: Joint Multi-Task Training of Small and Large Language Models for Efficient Multimodal Hotel Retrieval](https://arxiv.org/abs/2506.07296)
...
<details>
  <summary>Details</summary>
Motivation: 解决传统旅游搜索引擎需要用户从目的地开始并编辑搜索参数的局限性。

Method: 结合多任务优化、非对称密集检索架构和图像处理技术。

Result: 在四个测试集上表现优于VISTA和MARVEL，主查询类型得分为0.681。

Conclusion: 多任务优化和架构设计提升了模型性能，具有通用性和可扩展性。
...
### [247] [SMaRCSim: Maritime Robotics Simulation Modules](https://arxiv.org/abs/2506.07781)
*Mart Kartašev,David Dörner,Özer Özkahraman,Petter Ögren,Ivan Stenius,John Folkesson*

Main category: cs.RO

TL;DR: SMaRCSim是一个仿真工具包，旨在解决水下机器人开发中的快速测试和团队协作问题。


<details>
  <summary>Details</summary>
Motivation: 现有仿真工具无法满足水下机器人学习、多类型机器人协作及任务规划的需求。

Method: 开发了SMaRCSim仿真包，支持学习算法、多类型机器人协作和任务规划集成。

Result: SMaRCSim为水下机器人功能开发提供了快速测试和团队协作的解决方案。

Conclusion: SMaRCSim填补了现有仿真工具的不足，为水下机器人领域带来新功能。
...
### [248] [Active Illumination Control in Low-Light Environments using NightHawk](https://arxiv.org/abs/2506.06394)
...
Method: 提出在线贝叶斯优化方法，动态调整光照强度和曝光时间，并设计基于特征检测的指标作为优化目标。

Result: 实验表明，特征检测和匹配性能提升47-197%，视觉估计更可靠。

Conclusion: NightHawk能有效改善地下环境中的机器人视觉性能。
...
### [249] [Edge-Enabled Collaborative Object Detection for Real-Time Multi-Vehicle Perception](https://arxiv.org/abs/2506.06474)
*Everett Richards,Bipul Thapa,Lena Mashayekhy*

Main category: cs.RO

TL;DR: 论文提出了一种基于边缘计算和多车协作的实时目标检测框架ECOD，通过PACE和VOTE算法提升CAV的感知能力，实验显示其分类准确率比传统方法高75%。


<details>
  <summary>Details</summary>
Motivation: 传统车载感知系统因遮挡和盲区精度有限，云端方案延迟高，无法满足实时需求，需开发低延迟、高精度的协作感知框架。

Method: 提出ECOD框架，集成PACE（多车数据聚合）和VOTE（共识投票分类）算法，在边缘服务器实时运行。

Result: 实验表明ECOD显著提升分类准确率（比传统方法高75%），同时实现低延迟实时处理。

Conclusion: 边缘计算可有效增强协作感知能力，适用于延迟敏感的自动驾驶系统。
...
### [250] [DriveSuprim: Towards Precise Trajectory Selection for End-to-End Planning](https://arxiv.org/abs/2506.06659)
*Wenhao Yao,Zhenxin Li,Shiyi Lan,Zi Wang,Xinglong Sun,Jose M. Alvarez,Zuxuan Wu*

Main category: cs.RO

TL;DR: DriveSuprim提出了一种基于选择的方法，通过渐进式候选过滤、旋转增强和自蒸馏框架，提升了自动驾驶车辆在复杂环境中的安全性和轨迹质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在预测安全轨迹时面临优化挑战，尤其是在罕见或未充分代表的场景中，难以精确选择最佳轨迹。

Method: 采用粗到细的渐进式候选过滤、旋转增强方法和自蒸馏框架，以提高鲁棒性和训练稳定性。

Result: DriveSuprim在NAVSIM v1和v2上分别达到93.5% PDMS和87.1% EPDMS，展示了卓越的安全关键能力和轨迹质量。

Conclusion: DriveSuprim通过创新方法解决了选择式轨迹预测的挑战，显著提升了自动驾驶的安全性和性能。
...
### [251] [Generalized Trajectory Scoring for End-to-end Multimodal Planning](https://arxiv.org/abs/2506.06664)
*Zhenxin Li,Wenhao Yao,Zi Wang,Xinglong Sun,Joshua Chen,Nadine Chang,Maying Shen,Zuxuan Wu,Shiyi Lan,Jose M. Alvarez*

Main category: cs.RO

TL;DR: GTRS提出了一种统一的多模态规划框架，结合粗粒度和细粒度轨迹评估，解决了静态和动态轨迹评分方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 解决静态轨迹词汇表在细粒度适应上的不足，以及动态生成轨迹在广泛分布捕捉上的缺陷。

Method: GTRS包括扩散式轨迹生成器、词汇泛化技术和传感器增强策略。
...
### [252] [RoboCerebra: A Large-scale Benchmark for Long-horizon Robotic Manipulation Evaluation](https://arxiv.org/abs/2506.06677)
*Songhao Han,Boxiang Qiu,Yue Liao,Siyuan Huang,Chen Gao,Shuicheng Yan,Si Liu*

Main category: cs.RO

TL;DR: RoboCerebra是一个新基准，用于评估机器人操作中的高级推理能力，结合了视觉语言模型（VLM）的语义推理和长期规划能力。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注反应性策略（System 1），忽视了VLM在语义推理和长期规划（System 2）中的潜力，亟需一个更复杂的基准来填补这一空白。

Method: 提出了RoboCerebra基准，包括大规模模拟数据集、分层框架（VLM规划器与VLA控制器结合）和评估协议，通过GPT生成任务指令并由人类执行构建数据集。

Result: RoboCerebra提供了更长的动作序列和更密集的标注，显著优于现有基准，并分析了VLM在System 2模块中的表现。

Conclusion: RoboCerebra推动了更通用和高效的机器人规划器的开发，为VLM在长期任务中的应用提供了新方向。
...
### [253] [SpikePingpong: High-Frequency Spike Vision-based Robot Learning for Precise Striking in Table Tennis Game](https://arxiv.org/abs/2506.06690)
*Hao Wang,Chengkai Hou,Xianglong Li,Yankai Fu,Chenxuan Li,Ning Chen,Gaole Dai,Jiaming Liu,Tiejun Huang,Shanghang Zhang*

Main category: cs.RO

TL;DR: SpikePingpong系统结合尖峰视觉与模仿学习，实现了高精度机器人乒乓球控制，解决了高速动态任务中的视觉预测和战略规划问题。


<details>
  <summary>Details</summary>
Motivation: 高速动态物体的控制是机器人领域的挑战，乒乓球作为测试平台，需要快速拦截和精确轨迹调整。

Method: 系统采用SONIC（尖峰相机模块）和IMPACT（战略规划模块），结合20 kHz尖峰相机和神经网络模型。

Result: 实验显示，系统在30 cm和20 cm精度任务中分别达到91%和71%的成功率，优于现有方法38%和37%。

Conclusion: SpikePingpong为高速动态任务中的机器人控制提供了新视角，实现了战术游戏策略的稳健实施。
...
### [254] [Multimodal Spatial Language Maps for Robot Navigation and Manipulation](https://arxiv.org/abs/2506.06862)
*Chenguang Huang,Oier Mees,Andy Zeng,Wolfram Burgard*

Main category: cs.RO

TL;DR: 提出多模态空间语言地图（VLMaps和AVLMaps），融合预训练多模态特征与3D环境重建，支持自然语言命令翻译为空间目标，并增强多模态目标导航能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法在环境映射、空间精度和多模态信息利用方面存在不足，需一种更全面的空间地图表示方法。

Method: 构建视觉-语言地图（VLMaps）及其扩展音频-视觉-语言地图（AVLMaps），结合预训练多模态特征与3D环境重建，利用大型语言模型（LLMs）实现自然语言命令翻译和多模态目标导航。

Result: 实验表明，多模态空间语言地图支持零样本空间和多模态目标导航，在模糊场景中召回率提升50%，适用于移动机器人和桌面操作器。

Conclusion: 多模态空间语言地图通过融合多模态特征和3D重建，显著提升了导航和交互能力，尤其在模糊环境中表现优异。
...
### [255] [MapBERT: Bitwise Masked Modeling for Real-Time Semantic Mapping Generation](https://arxiv.org/abs/2506.07350)
*Yijie Deng,Shuaihang Yuan,Congcong Wen,Hao Huang,Anthony Tzes,Geeta Chandra Raju Bethala,Yi Fang*

Main category: cs.RO

TL;DR: MapBERT是一个新颖的框架，利用BitVAE和掩码变换器来生成完整的语义地图，解决了现有方法在实时性和泛化性上的不足。


<details>
  <summary>Details</summary>
Motivation: 空间感知对具身代理至关重要，但现有方法在稀疏、不平衡的物体类别和多样空间尺度下表现不佳，难以实时生成未观察区域。

Method: 提出MapBERT，结合BitVAE编码语义地图为紧凑位令牌，使用掩码变换器推断缺失区域，并引入对象感知掩码策略增强物体中心推理。

Result: 在Gibson基准测试中，MapBERT实现了最先进的语义地图生成，平衡了计算效率和未观察区域的准确重建。

Conclusion: MapBERT通过建模未观察空间的分布，显著提升了语义地图生成的性能，适用于实际机器人任务。
...
### [256] [BitVLA: 1-bit Vision-Language-Action Models for Robotics Manipulation](https://arxiv.org/abs/2506.07530)
*Hongyu Wang,Chuyan Xiong,Ruiping Wang,Xilin Chen*

Main category: cs.RO

TL;DR: BitVLA是首个1位三元（{-1, 0, 1}）视觉-语言-动作（VLA）模型，通过蒸馏感知训练策略压缩视觉编码器至1.58位，在内存受限设备上表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决VLA模型因规模增大在资源受限机器人系统上部署的挑战。

Method: 提出BitVLA模型，采用1位三元参数和蒸馏感知训练策略压缩视觉编码器。

Result: 在LIBERO基准测试中性能接近4位后训练量化的OpenVLA-OFT，内存占用仅29.8%。

Conclusion: BitVLA在内存受限边缘设备上具有部署潜力，代码和模型已开源。
