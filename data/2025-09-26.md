...
Motivation: 现有HDR-3DGS方法需要从不同曝光的多视角LDR图像中学习，这既繁琐又容易因物体运动模糊和校准误差而失败。SeHDR旨在解决从单曝光图像学习HDR表示的病态问题。

Method: 首先从单曝光LDR输入学习基础3D高斯，然后估计具有相同几何但不同线性颜色的多个3D高斯，最后通过可微分神经曝光融合将基础和高斯估计集成到HDR高斯中进行新视图渲染。

Result: 大量实验表明，SeHDR在性能上优于现有方法以及精心设计的基线模型。

Conclusion: SeHDR成功实现了从单曝光多视角LDR图像学习HDR场景表示，为解决这一具有挑战性的问题提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个统一的框架来处理多种图像生成任务，扩展传统文本到图像系统，使其成为更交互式和多维度的创意工具。

Method: 使用高效的扩散变换器和强大的VAE减少图像令牌数量，通过数十亿文本-图像对进行预训练，结合多模态后训练和多种推理加速技术（对抗蒸馏、分布匹配、量化、推测解码）。

Result: 在文本到图像和多模态图像编辑任务上达到最先进水平，生成2K图像仅需1.8秒，在复杂任务中表现出卓越的多模态能力。

Conclusion: Seedream 4.0将传统文本到图像系统扩展为更交互式和多维度的创意工具，推动了生成AI在创意和专业应用领域的边界。
...
TL;DR: 提出了一种通用的神经空间（NS）框架，通过编码器-解码器架构预计算跨视觉和成像任务的特征，使多个下游AI模块能够共享相同的特征空间，从而提高效率并改善跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的AI模型通常针对特定高精度任务定制，但在需要执行一系列模块化任务的应用中效率低下，因为每个任务都需要映射到不同的潜在域。

Method: 使用轻量级CNN骨干网络构建编码器-解码器框架，学习具有变换感知和泛化能力的表示，使多个下游模块能够在共享的特征空间中运行。

Result: 实验表明，成像和视觉模块（如去马赛克、去噪、深度估计和语义分割）可以在神经空间中高效执行。

Conclusion: 该架构减少了冗余，改善了跨域泛化能力，并为高效的多任务视觉流水线奠定了基础，相比大型Transformer骨干网络更轻量且硬件适应性更广。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于掩码的虚拟试穿方法存在局限性：生成理想掩码困难、需要背景知识、模型依赖性强，且在某些场景下无法实现（如将长袖卷起）。

Method: 利用视觉语言模型和图像分割模型自动生成二进制掩码，基于用户提供的图像和自由文本样式指令，无需精确绘制掩码，自动化执行多轮图像生成。

Result: InstructVTON与现有虚拟试穿模型兼容，实现了最先进的样式控制效果。

Conclusion: 该系统简化了终端用户体验，解决了传统掩码方法无法处理的复杂试穿场景问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像编辑方法在个性化图像合成任务中存在几何控制不足、风格一致性差的问题，且通常需要大量训练。

Method: 首先将2D对象转换为3D，在3D层面进行交互式编辑，然后从指定视角重新渲染为2D图像，结合扩散适配器实现几何、风格和内容控制。

Result: 该方法能够生成具有精确几何控制和风格一致性的编辑图像，无需额外训练。

Conclusion: FreeInsert通过3D几何信息实现了训练自由的图像编辑，为个性化对象插入提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像扩散模型在合成真实人类照片方面取得了显著进展，但面临场景退化、控制不足和感知身份不理想等问题。

Method: 提出CustomEnhancer框架，采用三重流融合生成方法，识别并组合两个兼容的反向潜在空间来操纵个性化模型的关键空间；同时引入ResInversion方法通过预扩散机制进行噪声校正。

Result: 实验表明CustomEnhancer在场景多样性、身份保真度和无训练控制方面达到最先进水平，ResInversion比NTI方法快129倍。

Conclusion: 该框架实现了对个性化模型生成过程的全面无训练控制，为个性化提供了精确控制，同时大幅提高了效率。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在关键应用中易受对抗攻击影响，而现有的参数高效微调方法如LoRA无法有效应对这些安全威胁，CLIP作为众多下游模型的基础尤其脆弱

Method: 提出动态对抗课程学习框架DAC-LoRA，基于一阶平稳条件和TRADES损失函数，构建渐进式挑战的攻击课程，可集成到标准PEFT流程中

Result: DAC-LoRA在保持清洁准确率的同时显著提升了对抗鲁棒性，实现了轻量级且广泛适用的鲁棒性增强

Conclusion: 该框架证明了对抗训练可以有效地集成到参数高效微调中，为视觉语言模型提供了实用的鲁棒性增强方案
...
<details>
  <summary>Details</summary>
Motivation: 现有VQA模型容易受到训练数据偏差影响，过度依赖表面模式，难以泛化到多样化的问题和图像

Method: 提出IOG-VQA模型，包含对象交互自注意力机制（捕捉图像中对象间复杂交互）和基于GAN的去偏框架（生成无偏数据分布）

Result: 在VQA-CP v1和VQA-CP v2数据集上的实验表明，模型在处理有偏和不平衡数据分布方面表现优异

Conclusion: 同时解决对象交互和数据集偏差对于推进VQA任务至关重要
...
<details>
  <summary>Details</summary>
Motivation: 视频序列中常存在结构化噪声和背景伪影，传统RPCA方法的稀疏性假设难以捕捉真实视频数据的丰富变异性。

Method: 提出Nuclear Diffusion方法，整合低秩时间建模与扩散后验采样，通过结合基于模型的时间模型和深度生成先验来实现高保真视频恢复。

Result: 在心脏超声去雾任务中，该方法在对比度增强（gCNR）和信号保持（KS统计量）方面均优于传统RPCA方法。

Conclusion: 结合基于模型的时间模型与深度生成先验的方法在视频恢复方面具有巨大潜力，能够实现更高保真度的恢复效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在扩散过程中加入模拟器投影层来确保物理合理性，但由于模拟器的顺序性导致计算成本高昂且无法并行化。

Method: 将模拟器运动投影解释为扩散过程中的引导形式，提出SimDiff模型，通过条件化环境参数（如重力、风力）直接整合到去噪过程中。

Result: SimDiff能够高效生成物理合理的运动，无需推理时重复调用模拟器，并能对不同物理系数进行细粒度控制，同时展现出对未见环境参数组合的组合泛化能力。

Conclusion: SimDiff通过将模拟器约束转化为扩散引导，实现了高效且可控的物理合理人体运动生成，具有良好的泛化性能。
...
<details>
  <summary>Details</summary>
Motivation: Vision Transformers会产生高范数伪影，这些伪影在知识蒸馏过程中会主导目标函数，导致学生模型过度拟合伪影而忽视信息信号，限制了大型模型的增益。

Method: 提出奇异零空间引导的能量重分配(SiNGER)，通过零空间引导的扰动在精炼教师特征时抑制伪影同时保留信息，使用基于LoRA的适配器高效实现扰动。

Result: 实验表明SiNGER能持续提升学生模型性能，在多个下游任务中达到最先进水平，并产生更清晰、更可解释的表征。

Conclusion: SiNGER有效解决了知识蒸馏中伪影抑制和信息保留的权衡问题，为Vision Transformers的知识蒸馏提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有概念擦除方法难以在精确移除目标概念的同时最小化图像质量退化，需要更精准的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有视频概念转移方法缺乏对视频不同组件的细粒度控制能力，难以实现精确可控的视频编辑。

Method: 提出空间分解（前景、背景、运动流）+双到单流DiT架构，引入基于随机掩码的自监督预训练策略，并采用Chain-of-Prompt机制进行时间步分解的三阶段去噪过程。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常分别学习相机和物体运动，这可能导致对相机与物体之间相对运动的混淆，难以在相机和物体运动同时存在时保持视频的一致性和泛化性。

Method: 利用稳定扩散网络学习参考运动映射图，结合提取的语义物体先验，通过图像到视频网络生成能够准确遵循指定相机轨迹并保持物体运动一致性的视频。

Result: 大量实验验证表明，该方法在性能上大幅优于当前最先进的方法。

Conclusion: 所提出的方法通过整合相机和物体运动为像素运动，有效解决了视频生成中相机轨迹引导的挑战，实现了更好的运动一致性和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 文本-图像生成发展迅速，但评估输出是否真正捕捉提示中的对象、属性和关系仍是核心挑战。当前评估严重依赖自动指标，但这些指标往往基于惯例或流行度而非经过人类判断验证。由于该领域的评估和进展报告直接依赖于这些指标，理解它们反映人类偏好的程度至关重要。

Method: 进行了广泛的研究，分析广泛使用的组合文本-图像评估指标。分析超越了简单的相关性，检查了这些指标在不同组合挑战中的行为，并比较了不同指标家族与人类判断的一致性。

Result: 结果显示没有单一指标在所有任务中表现一致：性能随组合问题类型而变化。VQA指标虽然流行但并非普遍优越，而某些嵌入指标在特定情况下更强。仅图像指标对组合评估贡献有限，因为它们是为感知质量而非对齐设计的。

Conclusion: 这些发现强调了仔细和透明选择指标的重要性，既是为了可信的评估，也是为了将它们用作生成中的奖励模型。指标选择应根据具体任务类型进行定制。
...
<details>
  <summary>Details</summary>
Motivation: 现有语义匹配方法存在两个主要问题：(i) 几何歧义：依赖2D基础模型特征难以区分对称结构，需要额外微调但缺乏泛化性；(ii) 最近邻规则：像素级匹配忽略了跨图像不可见性和流形保持。这需要几何感知的像素描述符和整体密集对应机制。

Method: 方法包括：(i) 保留VGGT内在优势：重用早期特征阶段、微调后期特征、添加语义头实现双向对应；(ii) 在数据稀缺下适应语义匹配：通过循环一致性训练策略、合成数据增强和渐进式训练方案（含混叠伪影缓解）。

Result: 大量实验表明，该方法在几何感知、匹配可靠性和流形保持方面表现优异，超越了之前的基线方法。

Conclusion: 通过利用3D几何基础模型VGGT并针对语义匹配场景进行适应性改进，本文提出的方法有效解决了现有语义匹配技术的局限性，实现了更优的性能。
...
<details>
  <summary>Details</summary>
Motivation: 高分辨率医学视频对准确诊断至关重要，但硬件限制和生理约束使其难以获取。现有VSR模型在处理医学视频时面临相机抖动、噪声、帧突变等挑战，容易产生伪影和扭曲特征误导医生。

Method: 提出MedVSR框架：1）交叉状态空间传播（CSSP）将远距离帧投影为状态空间模型中的控制矩阵，选择性传播一致信息特征实现有效对齐；2）内部状态空间重建（ISSR）模块结合长距离空间特征学习和大核短距离信息聚合来增强组织结构。

Result: 在包括内窥镜和白内障手术在内的四个医学场景数据集上的实验表明，MedVSR在重建性能和效率方面显著优于现有VSR模型。

Conclusion: MedVSR通过专门设计的对齐和重建机制，有效解决了医学视频超分辨率的独特挑战，为临床诊断提供了更可靠的视频增强方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前大规模文本到视频生成的主要瓶颈是物理一致性和可控性。现有模型往往产生不真实的运动（如物体向上坠落）或速度和方向的突变，且缺乏精确的参数控制能力。

Method: 提出NewtonGen框架，核心是训练神经牛顿动力学(NND)，通过联合利用数据先验和动力学指导，实现物理一致的视频合成。

Result: 该方法能够生成物理一致的视频，并提供精确的参数控制能力。

Conclusion: 通过将数据驱动合成与可学习物理原理相结合，NewtonGen解决了当前文本到视频生成中的物理一致性和可控性问题。
...
<details>
  <summary>Details</summary>
Motivation: 为了让高质量图像生成技术能够在各种消费级设备上部署使用，解决计算资源限制的问题

Method: 采用重新制定的分布匹配目标进行多步蒸馏，引入时间步共享减少梯度噪声和分时步微调提高提示对齐，结合文本编码器重构和专门量化等管道优化

Result: SD35-Flash在包括大规模用户研究在内的广泛评估中，始终优于现有的多步方法

Conclusion: 该框架使高级生成式AI真正实现了实际部署的普及化，从手机到桌面电脑都能高效运行
...
<details>
  <summary>Details</summary>
Motivation: 传统聚类方法仅依赖输入数据的相似性，难以捕捉领域特定的结构或语义约束，限制了在复杂领域中的应用效果。

Method: 扩展VAE架构，嵌入LLM生成的规则、语义表示和数据驱动特征到统一潜在空间，通过包含重建损失、KL散度、一致性惩罚和违规惩罚的损失函数强制约束合规。

Result: 在飞机和汽车数据集上的实验表明，规则引导的聚类能产生更具操作意义和可解释性的聚类结果（如分离无人机、统一隐形飞机、区分SUV和轿车），同时提升传统聚类指标。

Conclusion: DARTVAE通过结合规则编码和学习表示，比纯数据驱动模型实现更有意义和一致的聚类结果，但面临LLM规则生成可能产生幻觉或冲突、规则过多导致过拟合、复杂领域扩展困难等挑战。
...
Method: 使用掩码变换器自编码器方法重建缺失的FHR信号，该方法能够捕捉数据的空间和频率成分，适用于不同时长的数据缺失情况。

Result: 该方法在信号修复和预测方面表现出鲁棒性，能够处理不同持续时间的缺失数据。

Conclusion: 该方法可应用于回顾性研究数据集，支持基于AI的风险算法开发，未来有望集成到可穿戴FHR监测设备中，实现更早、更稳健的风险检测。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像模型遗忘方法主要依赖MSE损失，但这种方法只是f-散度的一个特例，缺乏灵活性。

Method: 提出统一的f-散度框架，允许使用任意f-散度进行遗忘，分析不同散度对算法收敛性和遗忘质量的影响。

Result: 该框架提供了选择最优散度的灵活性，能够在激进遗忘和概念保留之间实现更好的平衡。

Conclusion: f-散度统一框架为机器遗忘提供了更灵活和优化的解决方案，优于传统的MSE方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有的机器人世界模型存在推理速度慢和生成轨迹物理合理性不足的问题，主要原因是帧到帧生成方法在相似帧上进行昂贵计算，且忽视了关键转换的语义重要性。

Method: KeyWorld首先通过迭代简化机器人运动轨迹识别重要转换，获取真实关键帧；然后训练DiT模型从文本任务描述推理生成这些物理意义关键帧；最后使用轻量级插值器高效重建完整视频。

Result: 在LIBERO基准测试中，KeyWorld相比帧到帧生成基线实现了5.68倍加速，关注运动感知关键帧进一步提高了生成视频的物理有效性，特别是在复杂任务上。
