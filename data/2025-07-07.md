...
Motivation: 由于3D高斯基元将光照和几何信息编码为外观，直接插入高斯混合环境中的网格会出现光照不匹配问题，需要解决这一问题。

Method: 开发GBake工具，从高斯泼溅场景中烘焙反射探针。

Result: 实现了传统3D网格在高斯泼溅环境中的真实反射映射。

Conclusion: GBake工具成功解决了高斯泼溅环境中传统3D网格的光照不匹配问题。
...
<details>
  <summary>Details</summary>
Motivation: 解决在实时渲染中处理具有离散微表面（如闪光或闪烁效果）材质的挑战。

Method: 基于实时区域光照下的闪光渲染技术，结合环境贴图滤波方法，快速计算微表面反射概率，并采用双门高斯近似进行采样。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅方法在静态面部表现优秀，但在可动画化虚拟形象中仍存在非线性变形和细节不足的问题，需要更高效的表示方法。

Method: 提出HyperGaussians，扩展3D高斯至高维多元高斯，并通过可学习的局部嵌入提升表现力；采用‘逆协方差技巧’解决计算效率问题。

Result: 在4个面部数据集上的19个受试者测试中，HyperGaussians在数值和视觉上均优于3DGS，尤其在高频细节（如眼镜框、牙齿、复杂面部动作和镜面反射）上表现突出。

Conclusion: HyperGaussians为高质量可动画化面部虚拟形象提供了一种高效且表现力强的解决方案，显著提升了细节渲染和动态效果。
...
<details>
  <summary>Details</summary>
Motivation: 旨在创建视觉逼真且功能丰富的3D虚拟环境，适用于AR/VR、游戏、机器人和数字孪生等领域。

Method: 通过场景理解、3D模型检索、材质增强和物理引擎集成，构建可编辑且兼容标准图形管线的场景。

Result: 在Scan2CAD基准测试中实现最先进的相似性性能，并能处理复杂光照和遮挡的材质转移。
...
Method: 提出SurgVisAgent，结合先验模型、上下文少样本学习和链式思维推理，实现定制化图像增强。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（如ControlNet++）仅关注最终去噪步骤，忽略了中间生成阶段，限制了其效果。
...
Conclusion: InnerControl通过全步骤对齐损失提升了控制保真度和生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统自回归模型在图像生成中难以捕获全局关系的问题，以及视觉分词器对全局信息处理有限的问题。

Method: 提出Hita，结合可学习的整体查询和局部补丁标记，采用序列结构和轻量级融合模块优化信息流。

Result: 在ImageNet基准测试中达到2.59 FID和281.9 IS，并在零样本风格迁移和图像修复中表现优异。

Conclusion: Hita通过改进全局信息捕获和自回归生成对齐，显著提升了图像生成性能。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的动态场景运动复杂且高度动态，现有方法（如神经辐射场或3D高斯泼溅）难以建模大尺度运动，限制了其应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有VidLMs主要关注陆地场景，忽视了水下观察的高需求应用。

Method: 通过结合人类专业知识和AI模型构建数据集，涵盖水下环境挑战、数据多样性和任务多样性，并设计评估指标。

Result: 实验表明，在UVLM上微调的VidLMs显著提升水下理解能力，同时对现有空中VidLM基准也有轻微改进。

Conclusion: UVLM为水下观察提供了标准化评估工具，并展示了跨场景应用的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然能生成高质量图像，但缺乏图像特定语义表示的能力，而自动编码器可以提供这种能力。通过结合两者，论文旨在建立一种既能生成高质量图像又能学习图像特定表示的方法。

Method: 1. 结合扩散模型和自动编码器，学习图像特定表示并组织潜在空间。2. 通过回归高分辨率图像块的位置信息，构建适合区分脑组织类型的潜在空间。3. 利用潜在表示和扩散模型的生成能力，设计无监督的撕裂伪影修复技术。4. 通过表示引导和扩散模型的可控噪声与去噪能力，实现无监督的JPEG伪影修复。

Result: 该方法成功构建了结构化的潜在空间，能够识别脑图像的区域特定模式，并实现了无监督的撕裂伪影和JPEG伪影修复。

Conclusion: 通过结合扩散模型和自动编码器，论文提供了一种既能生成高质量图像又能学习图像特定表示的方法，并在脑图像分析和伪影修复中展示了其有效性。
...
Method: 采用预训练扩散模型从单张参考照片迁移化妆模式，通过粗到细的策略（Coherent Duplication和Refinement Module）实现一致性和高质量化妆效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖成对数据集且可能导致过增强，实际应用中数据集收集困难。

Method: IGDNet包含分解模块和去噪模块，前者分离光照与反射分量，后者通过光照引导的像素自适应校正增强非均匀光照区域。
...
<details>
  <summary>Details</summary>
Motivation: 现有的人体姿态估计方法在视觉模糊和遮挡情况下难以恢复合理的近距离交互动作，即使是大型基础模型（如SAM）也无法准确区分人类语义。

Method: 训练扩散模型学习人类社交距离和姿态先验知识，结合双分支优化框架重建动作和外观，并设计多种约束（如3D高斯、2D关键点和网格穿透）辅助优化。

Result: 在多个基准测试中表现优于现有方法，并构建了带有伪真实交互注释的数据集。

Conclusion: 该方法能够从复杂环境中捕获的视频中估计准确的交互动作，为未来姿态估计和人类行为理解研究提供了支持。
...
<details>
  <summary>Details</summary>
Motivation: 解决有限数据下扩散模型过拟合、先验知识丢失和文本对齐退化的问题。
...
Method: 提出CanonSwap框架，先消除运动信息，在统一规范空间修改身份，再重新整合到原视频空间；设计了部分身份调制模块和细粒度同步指标。

Result: 实验表明，CanonSwap在视觉质量、时间一致性和身份保持上显著优于现有方法。

Conclusion: CanonSwap通过解耦运动与外观信息，成功解决了视频人脸交换中的动态属性保持问题。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有文本到图像模型在人像生成中局部细节（如面部和手部）表现不足的问题。

Method: 构建全局和局部学习目标，基于MPD准则优化参数更新策略，实现公平的多目标优化。

Result: 显著提升了人像生成中局部细节的质量，同时保持整体生成效果。
...
Conclusion: 该框架实现了无需训练的结构丰富且外观丰富的生成。
...
Method: 结合 TriMap 视频扩散模型生成 RGB、法线和分割图，并通过语言量化压缩器 (LQC) 编码语言嵌入，实现跨场景泛化。

Result: 实验证明 LangScene-X 在质量和泛化能力上优于现有方法。

Conclusion: LangScene-X 通过生成一致的新观察，实现了从稀疏视图重建和开放词汇场景理解的突破。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）在开放环境中的安全漏洞问题，尤其是视觉模态引发的有害响应，需要更现实和有效的攻击方法。

Method: 提出VisCo攻击，利用四种视觉策略动态生成辅助图像，结合毒性模糊和语义优化，构建视觉中心越狱场景。

Result: VisCo在MM-SafetyBench上对GPT-4o的毒性分数为4.78，攻击成功率为85%，显著优于基线方法。

Conclusion: VisCo攻击展示了视觉模态在越狱场景中的关键作用，为MLLMs的安全性研究提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有文本到视频（T2V）和图像到视频（I2V）方法在动态运动信号和空间约束集成上的不足，提升编辑灵活性和运动控制。

Method: 提出AnyI2V框架，支持多种条件图像输入（如网格、点云），无需训练，结合LoRA和文本提示实现风格迁移和编辑。
...
<details>
  <summary>Details</summary>
Motivation: 视频生成模型因迭代去噪过程导致推理速度慢、计算成本高，限制了其广泛应用。解决这一瓶颈对推动视频合成技术的普及至关重要。

Method: 提出EasyCache，引入轻量级、运行时自适应的缓存机制，动态重用先前计算的变换向量，避免推理中的冗余计算。无需离线分析或参数调优。
...
Method: RefTok通过未量化的参考帧对帧集进行编码和解码，保持运动连续性和对象外观。

Result: 在4个视频数据集上，RefTok平均提升36.7%的指标，并在生成任务中优于更大规模的模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有System 2 Thinking方法存在模态或问题特定性、需要额外监督等限制，论文探索是否可以通过无监督学习实现泛化。

Method: 训练EBTs为输入和候选预测对分配能量值，通过梯度下降能量最小化进行预测。

Result: EBTs在训练和推理中表现优于Transformer++和Diffusion Transformers，且在下游任务中泛化能力更强。

Conclusion: EBTs是一种有潜力的新范式，可提升模型的学习和推理能力。
...
Result: 实验表明，该方法压缩比是规则压缩方法SZ3的10倍，比同类学习方法的性能高63%。
...
Result: 在多个数据集上表现优于或接近现有方法，成功解缠CelebA中的面部属性。

Conclusion: L-VAE有效平衡了解缠和重构，性能优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 解决深度学习在医学影像中数据稀缺和隐私限制的问题，同时克服联邦学习的高通信成本和单一任务限制。
...
<details>
  <summary>Details</summary>
Motivation: 现有持续领域自适应预训练方法存在高计算成本、数据顺序敏感性和无法为特定任务提供定制模型的问题。

Method: 利用LoRA模块实现高效、并行且对领域顺序鲁棒的预训练，并利用累积知识为特定任务提供定制模型。

Result: DoMIX在持续领域自适应预训练中表现高效，且可扩展到标准LLM微调场景。
