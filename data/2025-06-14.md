...
TL;DR: 论文提出了一种名为Token Perturbation Guidance (TPG)的新方法，通过直接扰动扩散网络中的中间令牌表示来提升生成质量，无需额外训练且适用于条件与非条件生成。


<details>
  <summary>Details</summary>
Motivation: Classifier-free guidance (CFG)需要特定训练且仅适用于条件生成，TPG旨在解决这些限制。

Method: TPG通过规范保持的扰动操作直接作用于中间令牌表示，提供稳定有效的指导信号。

Result: 在SDXL和Stable Diffusion 2.1上的实验显示，TPG在无条件生成中FID提升近2倍，同时在提示对齐上与CFG效果接近。

Conclusion: TPG是一种通用的、条件无关的指导方法，能够为更广泛的扩散模型带来类似CFG的优势。
...
TL;DR: 通过利用低质量、合成和分布外图像提升扩散模型质量的方法。
...
Result: 在合成损坏图像上验证框架，并在ImageNet FID和文本到图像生成任务中取得最优性能。

Conclusion: 噪声可缓解高质量分布与混合分布间的初始偏差，理论分析支持了方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有虚拟试衣方法局限于正面视角且缺乏实时性，而基于单件服装的方法虽有所改进，但仍存在数据采集成本高、服装与人体对齐不准确的问题。

Method: 使用真实人体采集服装数据集，无需定制机器人模特；提出混合人物表示，结合简化的DensePose图以提升对齐精度。
...
TL;DR: Edit360是一个无需调优的框架，将2D编辑扩展到多视角一致的3D编辑，解决了现有方法在视角限制和一致性上的问题。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在2D图像生成和编辑上取得进展，但扩展到3D资产仍具挑战性，尤其是需要多视角一致性的细粒度编辑。现有方法通常限制在预定的视角，缺乏灵活性。

Method: 基于视频扩散模型，Edit360通过Anchor-View Editing Propagation机制，选择锚点视角进行2D编辑，并在潜在和注意力空间中传播编辑，确保多视角一致性。

Result: Edit360实现了从任意视角的用户定制编辑，并重建高质量3D资产，支持可定制的3D内容创作。
...
TL;DR: 该论文提出了一种结合Stable Diffusion、GPT-2和混合音频管道的60秒电影自动生成方法，支持高分辨率和高帧率，适用于多种应用场景。


<details>
  <summary>Details</summary>
Motivation: 生成式人工智能的进步推动了多媒体创作的变革，本文旨在通过文本输入自动合成高质量的电影内容。

Method: 采用五场景框架，结合Stable Diffusion生成图像、GPT-2构建叙事，混合音频管道（gTTS和YouTube音乐），并通过线性帧插值和后期处理优化结果。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在生成高保真图像和提示引导生成方面表现出色，但在处理广泛语义提示时，确保生成样本的多样性仍具挑战性。

Method: SPARKE方法利用条件熵进行多样性引导，动态调整相似提示的多样性测量，并通过简化熵计算将复杂度从O(n^3)降至O(n)。

Result: 实验表明，SPARKE在不显著增加计算成本的情况下，提高了生成数据的提示感知多样性。

Conclusion: SPARKE为扩散模型提供了一种高效的多样性控制方法，适用于大规模生成场景。
...
TL;DR: 本文提出了一种针对个性化视觉语言检索的方法，通过低秩适配语言编码器的参数，有效整合个人与通用知识，并在两个基准测试中取得最优性能。
...
Method: 采用正则化低秩适配语言编码器的最后一层参数，结合参数加法策略整合多个个人概念。

Result: 在DeepFashion2和ConCon-Chi基准测试中，个性化检索准确率比现有方法提升4%-22%。

Conclusion: 该方法通过低秩适配和参数加法，有效平衡个人概念学习与通用知识保留，实现了高性能的个性化检索。
...
Method: 利用立体基础模型估计相机姿态并重建密集点云；通过多尺度2D特征编码颜色属性；设计基于自注意力的点交互网络增强特征表示；通过轻量级MLP解码高斯参数。
...
TL;DR: 本文提出了一种统一的图像处理框架，用于解释自注意力机制及其组件（如位置编码和残差连接）的作用，并引入两种独立的架构修改，旨在提高可解释性，同时提升模型在语言和视觉任务中的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 自注意力机制虽然成功，但其理论基础尚不完善，现有框架缺乏对其组件的深入解释。本文旨在填补这一空白，通过图像处理框架提供更深入的理解。

Method: 开发了一个统一的图像处理框架，解释自注意力及其组件的功能，并引入两种独立的架构修改。

Result: 实验表明，基于图像处理的修改不仅提高了模型的可解释性，还显著提升了准确性和鲁棒性，尤其是在长序列理解方面。

Conclusion: 本文通过图像处理框架为自注意力机制提供了更深入的理论解释，同时展示了其在实际任务中的性能提升潜力。
...
TL;DR: ReconMOST是一种基于扩散模型的数据驱动框架，用于多层海水温度重建，解决了传统方法在稀疏数据和计算成本上的限制。


<details>
  <summary>Details</summary>
Motivation: 准确重建海洋温度对全球气候动态和海洋气象研究至关重要，但现有方法受限于数据稀疏性和计算复杂性。

Method: 通过预训练无条件扩散模型学习物理一致的海洋温度分布，利用稀疏观测数据引导反向扩散过程进行重建。

Result: 在CMIP6和EN4数据上，MSE值分别为0.049（引导）、0.680（重建）和0.633（总体），表现出高精度和鲁棒性。

Conclusion: ReconMOST扩展了ML在海洋温度重建中的应用，具有高精度和泛化能力，适用于全球多层重建。
...
<details>
  <summary>Details</summary>
Motivation: 尽管多模态基础模型在图像理解和生成方面取得了进展，但其性能仍不及专门化模型。Pisces旨在通过解决视觉特征和训练过程的差异来提升统一模型的性能。

Method: Pisces采用解耦的视觉编码架构和针对多模态生成优化的训练技术，结合精细的数据处理、预训练和微调。

Result: Pisces在20多个图像理解基准测试中表现优异，并在图像生成基准测试GenEval上展现出强大的生成能力。

Conclusion: Pisces展示了图像理解与生成之间的协同作用，并通过分离视觉编码器推动了统一多模态模型的进展。
...
TL;DR: T2I-PAL利用文本生成图像以减少模态差距，结合热图和可学习原型提升多标签识别性能，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决CLIP模型中图像与文本模态差距问题，提升仅用文本进行参数高效微调时的图像识别性能。

Method: 利用预训练文本生成模型生成图像减少模态差距，结合类热图和可学习原型增强局部特征表示，并融合提示调优和适配器学习。

Result: 在多个基准测试中平均性能提升3.47%，优于现有方法。

Conclusion: T2I-PAL无需全语义标注图像，减少人工标注负担，且兼容现有CLIP框架，性能显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统基于GAN的方法在生成图像时的复杂性和计算强度问题。

Method: 使用轻量级神经网络将CSI幅度映射到LDM的潜在空间，结合文本引导的扩散模型生成高分辨率图像。

Result: 在计算效率和感知质量上优于基线方法，并支持文本引导的可控性。
...
TL;DR: TexTailor是一种新方法，通过文本描述生成一致的物体纹理，解决了现有方法在多视角纹理合成中的一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现有文本到纹理合成方法在多视角下纹理属性逐渐偏移，且相机位置固定，导致纹理一致性差。

Method: 采用重采样方案整合已合成纹理信息，微调深度感知扩散模型，并提出性能保持损失和自适应相机位置调整。
...
<details>
  <summary>Details</summary>
Motivation: 医学影像领域缺乏文本到图像的潜在扩散模型研究，且现有模型未能有效对齐临床信息与影像。

Method: 提出了一种微调框架，用于改进预训练模型的多模态对齐能力，适用于下游任务如短语定位。

Result: 在标准数据集（MS-CXR）上取得最优性能，并在外部数据（VinDr-CXR）上表现稳健。

Conclusion: 该方法为医学影像领域的文本到图像任务提供了有效的解决方案，并具有实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在图像生成和语义任务之间存在分离，SymmFlow旨在通过统一框架解决这一问题。

Method: 采用对称学习目标，联合建模正向和反向变换，引入新训练目标以保留语义信息，支持灵活条件。

Result: 在CelebAMask-HQ和COCO-Stuff上分别达到FID 11.9和7.0，同时在语义分割和分类任务中表现优异。

Conclusion: SymmFlow在多任务中实现了高效且高性能的统一建模，代码将开源。
...
Method: 设计基于提示的数据引擎和奖励引导的训练策略，利用预训练视觉语言模型反馈。

Result: 在VBench-2.0基准测试中，平均性能提升约4%，仅需4 GPU小时。

Conclusion: GigaVideo-1在无需人工标注和少量真实数据下，展示了高效和有效性。
...
TL;DR: 本文提出了一种轻量级卷积神经网络（CNN）结合挤压激励块注意力（SE）的深度伪造检测模型，旨在解决现有模型存储和内存消耗大的问题。


<details>
  <summary>Details</summary>
Motivation: 深度伪造技术对信息真实性和安全性构成威胁，现有检测方法难以应对其复杂性和真实性。

Method: 采用轻量级CNN结合SE块注意力机制，动态调整通道特征，提升模型效率和效果。

Result: 在Style GAN数据集上，模型分类准确率达94.14%，AUC-ROC得分为0.985。

Conclusion: 该方法为深度伪造检测提供了高效且资源友好的解决方案，具有实际应用潜力。
...
Conclusion: UAC框架有效解决了传统对抗攻击的局限性，为CAPTCHA安全提供了新思路。
...
TL;DR: 论文提出了一种名为SbCA的统一模型，用于解决超大规模图像超分辨率任务中的性能下降问题，通过分解图像为矢量图形并迭代优化细节，显著提升了超分辨率质量。


<details>
  <summary>Details</summary>
Motivation: 现有ASISR方法在超出训练数据范围的超大上采样因子下性能显著下降，导致模糊问题。

Method: 提出SbCA模型，利用笔画矢量放大器和细节补全模块分解图像为矢量图形并恢复缺失细节，通过循环策略迭代优化。

Result: 实验表明，SbCA在超大上采样任务（如×100）中显著优于现有方法，生成的高分辨率图像质量更高。
...
<details>
  <summary>Details</summary>
Motivation: 视频抠图在资源受限设备上的部署面临计算量大和量化后精度下降的挑战，现有PTQ方法在视频抠图中尚未成熟。

Method: 提出两阶段PTQ策略（块重建优化和全局校准）、统计驱动的全局仿射校准（GAC）方法和光流辅助（OFA）组件。

Result: PTQ4VM在不同位宽下均达到最先进精度，4位量化下性能接近全精度模型，且计算量减少8倍。
...
<details>
  <summary>Details</summary>
Motivation: 解决Transformer在视频生成中的高计算复杂性问题，并扩展Mamba架构在多模态和时空建模中的应用。

Method: 提出MM-DiM块，结合多模态信息重组和时空建模；引入奖励学习策略提升单帧视觉质量。
...
TL;DR: 本文提出了一种直接从视频中学习上下文图像编辑的方法，通过多模态序列标注和块因果扩散变换器，实现了多任务学习和多轮编辑能力。
...
Result: 模型在上下文图像编辑中表现优异，在多轮编辑基准测试中达到SOTA，同时展示了多概念组合、故事生成和编辑链应用的能力。
...
Conclusion: 发布FLUX-Reason基线模型（MMMG-Score 34.45），以推动进一步研究。
...
TL;DR: GenWorld是一个高质量、大规模的真实世界模拟数据集，用于AI生成视频检测，解决了现有数据集不足的问题。提出的SpannDetector模型通过多视角一致性检测，显著提升了检测效果。


<details>
  <summary>Details</summary>
Motivation: 视频生成技术的快速发展威胁了真实世界信息的可信度，亟需可靠的AI生成视频检测工具，但高质量真实世界数据集的缺乏阻碍了进展。

Method: 提出GenWorld数据集，模拟真实场景，采用多种先进视频生成模型，并包含多样化的生成器和提示模态。提出SpannDetector模型，利用多视角一致性检测AI生成视频。

Result: 实验表明，SpannDetector在检测高质量AI生成视频方面表现优异，揭示了基于物理合理性的可解释检测方向。

Conclusion: GenWorld数据集和SpannDetector模型为AI生成视频检测领域提供了重要工具，推动了该领域的发展。
...
Result: 在Stable Diffusion 3和FLUX.1等模型上验证了方法的有效性，实现了生成质量的提升和特定视觉风格的操控。

Conclusion: 首次在扩散模型中进行了头级注意力扰动分析，揭示了注意力层的可解释性，并为设计有效扰动策略提供了实用方法。
...
Method: 分析采样轨迹的低维几何规律，提出动态规划方法调整采样时间表。

Result: 发现采样轨迹具有低维性和相似形状，动态规划方法显著提升生成性能。

Conclusion: 采样轨迹的低维几何规律为优化扩散生成模型提供了新思路。
...
Result: 在Ising和TASEP模型上验证了方法的有效性，优于现有基线；应用于扩散模型时揭示了潜在空间的分形相变结构。
...
Conclusion: ReGuidance首次为DPS提供了理论保证，并有效提升了性能。
...
### [35] [Prompt-Guided Latent Diffusion with Predictive Class Conditioning for 3D Prostate MRI Generation](https://arxiv.org/abs/2506.10230)
*Emerson P. Grabke,Masoom A. Haider,Babak Taati*

Main category: eess.IV

TL;DR: 论文提出了一种名为CCELLA的双头条件方法，结合非医学大型语言模型和病理分类，解决了医学潜在扩散模型（LDM）在数据稀缺和性能限制上的问题。


<details>
  <summary>Details</summary>
Motivation: 医学LDM训练常受限于数据稀缺、短提示文本编码器或非医学LDM的复用，导致性能和科学可及性受限。

Method: 提出CCELLA方法，通过交叉注意力将非医学大型语言模型编码的文本特征与时间步嵌入的病理分类同时条件化LDM U-Net，并设计了联合损失函数和数据高效的LDM训练框架。

Result: 在有限的前列腺MRI数据集上，3D FID得分为0.025，显著优于基准模型的0.071。合成图像用于分类器训练时，准确率从69%提升至74%。

Conclusion: CCELLA方法在数据稀缺和标注有限的情况下，显著提升了医学图像合成的质量和科学可及性。
...
### [36] [Conditional diffusion models for guided anomaly detection in brain images using fluid-driven anomaly randomization](https://arxiv.org/abs/2506.10233)
*Ana Lawry Aguila,Peirong Liu,Oula Puonti,Juan Eugenio Iglesias*

Main category: eess.IV

TL;DR: 提出了一种基于条件扩散模型的新框架，用于脑MRI中的异常检测和健康图像重建，通过合成伪病理图像改进重建效果，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决无监督异常检测中模型无法准确重建健康组织或去除异常的问题，尤其是在罕见疾病数据稀缺的情况下。

Method: 采用条件扩散模型框架，结合流体驱动的异常随机化生成伪病理图像，增强健康图像重建的指导。

Result: 在合成和真实病理数据集上表现优异，优于变分自编码器、条件/无条件潜在扩散模型，甚至部分超越有监督修复方法。

Conclusion: 提出的方法在无监督异常检测中表现出色，为罕见疾病诊断提供了有效工具。
...
### [37] [SWDL: Stratum-Wise Difference Learning with Deep Laplacian Pyramid for Semi-Supervised 3D Intracranial Hemorrhage Segmentation](https://arxiv.org/abs/2506.10325)
*Cheng Wang,Siqi Chen,Donghua Mi,Yang Chen,Yudong Zhang,Yinsheng Li*

Main category: eess.IV

TL;DR: SWDL-Net是一种新的半监督学习框架，结合拉普拉斯金字塔和深度卷积上采样，用于颅内出血（ICH）的医学图像分割，在仅有2%标记数据的情况下优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割通常需要大量手动标注数据，但ICH的标注过程繁琐且昂贵，半监督学习（SSL）成为解决标记数据稀缺的有前景的方法。

Method: 提出SWDL-Net框架，结合拉普拉斯金字塔（边缘锐化）和深度卷积上采样（细节精确），通过差异学习机制整合两者的互补优势。

Result: 在271例ICH数据集和公开基准测试中，SWDL-Net在仅有2%标记数据的情况下表现优于现有方法；在BHSD数据集（5%标记数据）上也验证了其优越性。
...
### [38] [EQ-TAA: Equivariant Traffic Accident Anticipation via Diffusion-Based Accident Video Synthesis](https://arxiv.org/abs/2506.10002)
*Jianwu Fang,Lei-Lei Li,Zhedong Zheng,Hongkai Yu,Jianru Xue,Zhengguo Li,Tat-Seng Chua*

Main category: cs.MM

TL;DR: 论文提出了一种名为AVD的模型，通过生成事故视频片段来解决交通场景中事故预测（TAA）的挑战，避免了繁琐的标注需求，并提出了EQ-TAA方法提升性能。


<details>
  <summary>Details</summary>
Motivation: 交通场景中的事故预测（TAA）面临数据标注困难、因果部分难以识别以及数据偏差等问题，需要一种无需额外标注的方法。

Method: 提出了AVD模型，通过生成事故视频片段（从正常片段到事故片段）来合成额外数据，并结合EQ-TAA方法使用对比损失进行训练。

Result: 实验表明，AVD和EQ-TAA在性能上优于现有方法。

Conclusion: AVD和EQ-TAA为TAA提供了一种无需标注的高效解决方案，具有竞争力。
...
<details>
  <summary>Details</summary>
Motivation: 现有HER2评估模型通常单独分析H&E或IHC图像，而临床依赖它们的协同解释。双模态获取常因工作流程复杂性和成本限制受阻。

Method: 1) 动态分支选择器；2) 双向跨模态GAN；3) 混合训练协议。

Result: 单模态H&E预测准确率从71.44%提升至94.25%，双模态准确率达95.09%，IHC输入可靠性为90.28%。

Conclusion: 该框架在资源有限的环境中具有显著潜力，通过弹性架构实现高效且精确的HER2评估。
...
TL;DR: 提出了一种基于扩散的框架，通过多模态情感绑定策略和注意力潜在扩散模型，解决了音频驱动3D面部动画中情感表达单一和确定性映射的问题。
...
Conclusion: 该方法通过多模态融合和扩散模型显著提升了3D面部动画的情感表达多样性和自然性。
...
### [41] [AniMaker: Automated Multi-Agent Animated Storytelling with MCTS-Driven Clip Generation](https://arxiv.org/abs/2506.10540)
*Haoyuan Shi,Yunxin Li,Xinyu Chen,Longyue Wang,Baotian Hu,Min Zhang*

Main category: cs.MA

TL;DR: AniMaker是一个多智能体框架，通过高效生成多候选视频片段和故事感知的片段选择，解决了多场景、多角色故事视频生成的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成方法在生成连贯的多场景故事视频时存在叙事断裂和节奏问题，且单一片段质量低下会影响整体动画的逻辑和视觉连贯性。
...
### [42] [A Navigation Framework Utilizing Vision-Language Models](https://arxiv.org/abs/2506.10172)
...
### [43] [Eye, Robot: Learning to Look to Act with a BC-RL Perception-Action Loop](https://arxiv.org/abs/2506.10968)
*Justin Kerr,Kush Hari,Ethan Weber,Chung Min Kim,Brent Yi,Tyler Bonnen,Ken Goldberg,Angjoo Kanazawa*

Main category: cs.RO

TL;DR: EyeRobot是一个机器人系统，通过强化学习训练眼球注视策略，实现手眼协调以完成任务。
...
Method: 开发机械眼球，收集360度摄像头数据，通过BC-RL循环联合训练手和眼的策略。
