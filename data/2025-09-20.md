...
Motivation: 现有视频扩散模型虽然具有丰富的空间智能潜力，但由于可控性有限和几何不一致性，难以实际应用于3D/4D任务。传统方法需要重新训练或微调，这会破坏预训练知识并带来高计算成本。

Method: 提出三个核心模块：1）Intra-Step Recursive Refinement：在推理时递归优化网络预测；2）Flow-Gated Latent Fusion：利用光流相似性分离运动和外观；3）Dual-Path Self-Corrective Guidance：通过比较引导和无引导路径自适应校正轨迹漂移。

Result: 在多个基准测试上的广泛实验验证了该方法在真实性、轨迹一致性和视觉保真度方面的优越性，实现了精确的运动控制和逼真的内容生成。

Conclusion: WorldForge引入了一种新颖的即插即用可控视频合成范式，为利用生成先验进行空间智能任务提供了新视角，无需训练即可实现细粒度的轨迹对齐引导。
...
<details>
  <summary>Details</summary>
Motivation: 现有tokenizer要么专注于重建要么专注于理解，且通常只处理单一模态。需要一种能够统一处理多种视觉输入并同时支持重建和理解任务的框架。

Method: 采用纯Transformer架构，使用4D旋转位置编码处理任意分辨率和时长的视觉输入。通过无对抗训练目标结合感知和Gram矩阵损失，采用渐进式训练课程从单图像扩展到视频和3D。

Result: 图像：0.21 rFID和82.2% ImageNet准确率；视频：3.01 rFVD和32.6% MSRVTT检索率；3D：28.19 PSNR和90.9%分类准确率。在下游任务中表现优异。

Conclusion: AToken展示了统一视觉tokenization在构建下一代多模态AI系统中的潜力，能够同时支持生成和理解任务，为多模态系统提供了强大基础。
...
Method: 开发了边缘引导注意力机制，从联合编码的边缘特征和中间特征激活中推导自适应调制图，用于归一化和重加权响应；集成到轻量级残差设计中，使用结合像素级、感知和对抗项的复合目标函数进行训练。

Result: 在标准SISR基准测试中，相比SRGAN、ESRGAN和先前的边缘注意力基线方法，在可比较的模型复杂度下，结构清晰度和感知质量均获得一致提升。

Conclusion: 该公式化方法提供了参数高效的边缘先验注入路径、通过定制多术语损失实现稳定的对抗精炼，以及在不依赖更深或过度参数化架构的情况下增强边缘保真度，证明了原则性边缘条件调制在推进感知超分辨率方面的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常经验性重复迭代去噪过程，无法高效地处理不同水平或模式的噪声，需要一种适配性的迭代去噪方法。

Method: 提出基于评分流泵模型的适配性迭代去噪方法：首先估计噪声变异并确定适配性去噪调度，然后迭代调用训练网络更新点云。设计了网络架构和两阶段采样策略来支持特征融合和梯度融合。

Result: 方法在合成数据集和真实扫描数据集上都获得了更优的结果，能够生成清洁平滑的去噪点云，同时更好地保持形状边界和细节。

Conclusion: 该方法不仅在质量和数量上都超过了现有最优方法，而且能够有效处理不同模式的噪声，为点云去噪任务提供了一种适配性的迭代解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉定位方法面临困境：HD地图精度高但成本昂贵难以扩展，SD地图方法主要关注图像与地图的BEV匹配，忽略了普遍存在但噪声严重的GPS信号。GPS虽然易得但在城市环境中存在多径误差问题。

Method: 提出DiffVL框架，使用扩散模型将视觉定位重构为GPS去噪任务。核心思路是将噪声GPS轨迹在视觉BEV特征和SD地图的条件下视为隐含真实位姿分布，通过迭代扩散细化来恢复真实位姿。联合建模GPS、SD地图和视觉信号，学习逆转GPS噪声扰动。

Result: 在多个数据集上的实验表明，该方法相比BEV匹配基线方法达到了最先进的精度水平，实现了亚米级定位精度，且不依赖HD地图。

Conclusion: 这项工作证明扩散模型可以通过将噪声GPS作为生成先验来实现可扩展的定位，这是对传统基于匹配方法的范式转变，为视觉定位提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于指令的图像编辑方法在处理挑战性编辑任务时表现不佳，因为现有数据集编辑类型和样本数量有限，且传统数据集构建方法存在噪声图像-标题对，可能引入偏见并限制模型在复杂编辑场景中的能力。

Method: 采用新颖的数据集构建流程，利用两个多模态大语言模型（MLLMs）分别生成视觉自适应编辑指令和制作高保真度编辑图像，构建包含107K高质量图像编辑样本的MultiEdit数据集。
...
Result: 定量和定性评估表明，使用多个参考图像能带来明显优势，实现了卓越的上色性能。

Conclusion: DACoN通过基础模型和CNN特征融合的方法，有效解决了动画线条图上色的技术挑战，支持任意数量参考图像，显著提升了上色质量。
...
<details>
  <summary>Details</summary>
Motivation: 单目视频重建人体化身面临几何信息不足的挑战，现有3D高斯泼溅方法由于自由形式的3D高斯基元而难以保持表面细节。

Method: 提出网格引导的2D高斯泼溅，将2D高斯基元直接附加到模板网格面上，并利用基础模型（如Sapiens）补充视觉信息，通过选择性梯度隔离的协调训练策略解决多模态优化冲突。

Result: 实验证明该方法在重建质量上优于现有方法，在几何精度和外观保真度方面有显著提升，同时提供丰富的语义信息，支持新颖视角和姿态下时空一致的渲染。

Conclusion: 通过增强表示和协调信息蒸馏的结合，FMGS-Avatar显著推进了3D单目人体化身重建技术，实现了高质量的几何细节保持和语义信息整合。
...
<details>
  <summary>Details</summary>
Motivation: 解决当前GAN反向数据萌蓬方法对预训练SR网络和类别信息的依赖问题，提高方法的通用性和适用性

Method: 首先提取高梯度补丁并使用CLIP特征进行图像分类，然后微调温渌模型学习选择补丁的分布并合成萌蓬训练图像

Result: 仅使用0.68%原始数据训练Transformer SR模型，性能下降仅0.3dB，萌蓬模型微调耗时4小时，SR模型训练耗时1小时，远少于全数据训练11小时的时间

Conclusion: 该方法在保持独制性能的同时显著提高了数据利用效率和训练效率，为深度网络训练提供了更加高效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像扩散模型在3D CT生成方面应用有限，主要依赖简化的提示词，忽略了完整放射学报告中的丰富语义细节，导致文本图像对齐和临床保真度降低。

Method: 提出Report2CT框架，整合三个预训练医学文本编码器（BiomedVLP CXR BERT、MedEmbed和ClinicalBERT）来捕捉临床上下文，使用放射学报告和体素间距信息条件化3D潜在扩散模型，在CT RATE数据集的20000个CT体积上进行训练。

Result: 生成的CT体积具有解剖一致性和优异的视觉质量，多编码器条件化提高了CLIP分数，表明更好地保留了放射学报告中的细粒度临床细节。在MICCAI 2025 VLM3D挑战赛中排名第一，在所有评估指标上达到最先进性能。

Conclusion: 通过利用完整放射学报告和多编码器文本条件化，Report2CT推进了3D CT合成，能够生成临床保真度高、质量优异的合成数据。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法假设模型会对所有降级类型过拟合，但通过仔细研究发现模型主要过拟合噪声降级，这主要因为噪声的降级模式与其他降级类型存在显著差异。

Method: 提出了一种针对性的特征去噪框架，包含两个模块：噪声检测模块用于识别噪声降级，以及去噪模块用于抑制模型对噪声的过拟合。该框架可以无缝集成到现有超分辨模型中，无需改动网络结构。

Result: 在5个传统测试集和数据集上进行了实验，包括合成和真实场景。实验结果显示，该框架在性能上显著超过了以往的规则化基础方法。

Conclusion: 通过针对噪声降级过拟合问题的特征去噪框架，有效提升了超分辨模型的泛化能力，为解决未知降级情况下的图像超分辨提供了一种通用且高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着人像图像在计算机视觉中的广泛使用，需要保护个人身份信息。同时，匿名化后的图像仍需保持在下游计算机视觉任务中的可用性。

Method: 利用潜在滿散模型的结构填充能力，设计适应性属性导向模块，在反向去噪过程中应用梯度纠正，使生成图像的面部属性与合成目标图像的属性对齐。支持局部匿名化，允许用户指定保留不变的面部区域。

Result: 在CelebA-HQ和FFHQ公开数据集上进行的大量实验显示，该方法在性能上超过了现有最先进方法，且不需要额外的模型训练。

Conclusion: 该框架提供了一种高效的人像匿名化方案，能够在保护隐私的同时维持图像的实用性，为计算机视觉应用中的个人信息保护提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 实际宇宙任务中采用数据驱动的空间设备位置估计方法遇到困难，主要因为对其决策过程缺乏理解。需要一种方法来可视化识别网络所使用的关键3D特征线索。

Method: 训练基于NeRF的图像生成器，通过位置估计网络的反向传播梯度来强化生成器渲染出空间设备位置估计网络主要利用的3D特征。

Result: 实验证明该方法能够恢复关键的3D线索，并为位置估计网络的监督方式与其隐式表征之间的关系提供了更深入的见解。

Conclusion: 该研究提供了一种有效的可视化方法，能够揭示数据驱动空间设备位置估计网络的决策基础，有助于推进这类方法在实际宇宙任务中的应用。
...
<details>
  <summary>Details</summary>
Motivation: 解决室内3D建模手动工作繁重的问题，现有生成方法在视觉质量、多样性、语义一致性和用户控制方面存在挑战，缺乏专门的大规模高质量数据集

Method: 构建包含12,328个结构化标注场景的大规模合成数据集，开发多视角多模态扩散模型SpatialGen，基于3D布局和参考图像生成外观、几何和语义信息

Result: 模型能够从任意视角生成保持空间一致性的多模态输出，在实验中表现优于先前方法

Conclusion: 通过开源数据和模型推动室内场景理解和生成领域发展，SpatialGen为自动化3D室内场景生成提供了有效解决方案
...
Method: 改进合成数据生成管道，包含常被忽略的前向散射项，考虑非均匀介质，并收集真实浑浊环境下的BUCKET数据集
...
<details>
  <summary>Details</summary>
Motivation: 现有双向模型图像编辑方法需要手动暴力调整多个相互依赖的超参数，调参空间大导致计算成本高

Method: 将超参数搜索模式为马尔可夫决策过程，推出基于近端策略优化的强化学习框架，动态调整去噪步骤中的超参数

Result: 实验表明方法显著减少了搜索时间和计算开销，较暴力搜索方法更高效

Conclusion: 该强化学习框架提高了双向模型图像编辑的实际部署可行性
...
<details>
  <summary>Details</summary>
Motivation: 现有方法如COLMAP在动态场景中存在运行时间长、依赖真实运动掩码等限制，而且常需要额外的监督信息（如真实焦距、运动掩码、3D点云等），这些在普通摄像的RGB视频中通常无法获得。

Method: 方法包含三个关键组件：(1)表示追踪滤波器，建立突出的同相关系；(2)离群点感知的聚合优化，通过自适应降权移动离群点来优化相机参数；(3)两阶段优化策略，通过交换软优化限制和凸最小值来提高稳定性和速度。

Result: 在4个实际数据集（NeRF-DS, DAVIS, iPhone, TUM-dynamics）和1个合成数据集（MPI-Sintel）上进行实验，结果显示该方法仅使用单个RGB视频作为监督，就能更高效和更准确地估计相机参数。通过将相机估计输入到4D重建方法进一步验证了准确性。

Conclusion: 该方法能够在仅使用单个RGB视频作为唯一监督的情况下，实现更高效和更准确的动态场景相机参数优化，充分解决了现有方法的限制。
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG攻击主要依赖操控用户查询，但实际中用户输入往往固定或保护，而指令提示作为广泛重用、公开共享且少有审计的组件，成为更实际的攻击向量

Method: 提出AIP攻击方法：1)使用多样化查询生成策略模拟实际语言变化；2)基于遗传算法的聚合优化方法，在攻击成功率、清洁任务效用和隐藏性之间征求平衡

Result: 实验结果显示AIP攻击能够达到95.23%的攻击成功率(ASR)，同时保持正常的良性功能

Conclusion: 这一发现曝露了RAG系统中一个重要但被忽视的漏洞，强调了重新评估共享指令提示安全性的必要性
...
<details>
  <summary>Details</summary>
Motivation: 解决亲属验证中的年龄差距问题，传统人脸老化模型存在种族偏见，影响照片相似度，需要生成无偏见的老化图像来提高验证准确性。

Method: 提出RA-GAN模型，包含RACEpSp和特征混合器两个新模块，生成无种族偏见的人脸老化图像，用于同年龄父母-子女亲属关系验证。

Result: RA-GAN在所有年龄段平均比SAM-GAN提升13.14%，在60+年龄段比CUSP-GAN提升9.1%的种族准确性。在KinFaceW-I数据集上，父子、父女、母子、母女关系的验证准确率分别提升5.22%、5.12%、1.63%、0.41%。

Conclusion: RA-GAN能有效生成无种族偏见的人脸老化图像，显著提升亲属验证准确率，特别是在处理年龄差距较大的亲属关系时效果显著。
...
<details>
  <summary>Details</summary>
Motivation: 探索多模态大语言模型在零样本时空视频定位任务中的应用，解决MLLMs在整合文本查询线索（如属性、动作）进行推理时的能力不足问题。

Method: 提出分解时空高亮(DSTH)策略和时序增强组装(TAS)策略。DSTH将查询分解为属性和动作子查询，使用logit引导重注意模块学习空间和时间提示；TAS通过原始帧和时序增强帧的组装预测来提升时序一致性。

Result: 在三个常见STVG基准测试中超越了最先进方法，证明了该方法的有效性。

Conclusion: 该框架成功释放了MLLMs在时空视频定位任务中的推理能力，为零样本STVG提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究表明高质量视觉表示对图像生成的重要性，以及生成模型在图像理解方面的局限性。作为最初为自然语言设计的生成范式，自回归模型在视觉领域面临类似挑战。

Method: 提出了自监督训练框架ST-AR，通过在训练过程中引入自监督目标来解决局部条件依赖、步间语义不一致和空间不变性缺失三个关键问题。

Result: ST-AR显著提升了自回归模型的图像理解能力并改善了生成质量，在保持相同采样策略的情况下，为LlamaGen-L带来约42%的FID提升，为LlamaGen-XL带来49%的FID提升。

Conclusion: 自监督目标可以有效解决自回归模型在视觉领域的关键问题，ST-AR框架在不依赖预训练表示模型的情况下显著提升了图像理解和生成性能。
...
Method: 提出两阶段预训练：1）自我中心视频生成预训练，在1200万自我中心操作视频上训练图像到视频模型；2）人类中心轨迹感知建模，联合预测未来关键点轨迹。还提出ActionVAE压缩动作序列为紧凑潜在嵌入

Result: 在相同下游机器人数据集上微调后，RynnVLA-001优于最先进的基线模型
...
<details>
  <summary>Details</summary>
Motivation: 现有方法偏重于完整无噪的观测数据，而忽视了视线外对象和传感噪声带来的挑战，这在实际应用中存在安全风险。

Method: 提出了增强版的视觉-定位去噪模块，利用相机标定建立视视-定位映射关系，以无监督方式有效去陪传感噪声数据。

Result: 在Vi-Fi和JRDB数据集上进行了评估，该方法在轨迹去噪和预测方面都达到了最先进的性能，显著超越了之前的基准方法。

Conclusion: 这是首次将视视-定位投影集成到视线外对象轨迹去噪中的工作，为该领域的未来发展排埋了基础。
...
<details>
  <summary>Details</summary>
Motivation: 传统MVS方法通过逐步细化深度图来提升效率，而扩散模型在生成任务中表现出色，因此探索将扩散模型引入MVS任务中
...
Conclusion: 扩散模型成功应用于MVS任务，提出的框架在效率和精度方面都取得了优异表现，为3D几何重建提供了新的有效方法
...
<details>
  <summary>Details</summary>
Motivation: 针对视觉语言模型在开放词汇语义分割中的源自由领域自适应需求，旨在在不访问源数据的情况下提升模型在新领域的性能

Method: 采用师生范式，结合词汇对齐策略改进伪标签生成；使用LoRA进行高效微调；提出Top-K类别选择机制降低内存需求

Result: 在CityScapes数据集上实现6.11 mIoU显著提升，在零样本分割基准测试中表现优异

Conclusion: VocAlign为开放词汇设置下的源自由自适应设立了新标准，在保持计算效率的同时显著提升了性能
...
<details>
  <summary>Details</summary>
Motivation: 现有的物理集成机器学习方法只能考虑单一偏微方程，导致模型误定和逆逆性问题，无法处理多重机制导致的疾病子型。

Method: 将反应-扩散偏微方程集成到变分自动编码器（VAE）混合模型框架中，支持从神经影像数据中推断可解释的隐变量子型（如扩散率和反应速率）。

Result: 在合成数据集上评估了方法的性能，并在阿尔茫海默病进展的正电子发射断层扫描（PET）数据中展示了发现机制子型的潜力。

Conclusion: 该方法能够从稀疏高维神经影像数据中学习多重物理机制的混合模型，为神经退血性疾病的子型分析提供了更好的解释性和应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 传统空气质量监测系统空间覆盖有限且可访问性差，需要更直观、可解释的污染可视化方法来支持环境决策和公众参与

Method: 结合统计纹理分析和监督学习进行污染分类，利用视觉语言模型引导的图像生成技术合成不同污染程度的可视化场景

Result: 在城市场景数据集上验证了方法有效性，能够准确估计污染水平并生成语义一致的可视化输出

Conclusion: 该系统为智能环境应用提供基础，未来将通过绿色CNN架构和FPGA增量学习实现实时边缘部署，提升可扩展性和能效
